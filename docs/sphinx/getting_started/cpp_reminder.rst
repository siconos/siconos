...
So to build an object of class *CHILD* the syntax will be::

  auto my_object_name = std::make_shared<CHILD>(some args ...)

For example, suppose you want to build a LagrangianDS, which belongs
to the base class DynamicalSystem, then do::

  auto nameOfMyDS = std::make_shared<LagrangianDS>(someParameters);
...
::
   ...
   auto v1= std::make_shared<siconos::algebra::SiconosVector>(4);

   # build a sparse vector of size 4
   auto vsparse= std::make_shared<siconos::algebra::SiconosVector>(4, Siconos::SPARSE)

   # build a block vector, which contains 3 dense blocks of size 5
   auto vblock = std::make_shared<siconos::algebra::BlockVector>(3, 5);

   # build a block vector, which contains 2 sparse
   auto v2 = std::make_shared<siconos::algebra::SiconosVector>(4, Siconos::SPARSE)
   auto v3 = std::make_shared<siconos::algebra::SiconosVector>(7, Siconos::SPARSE)
   auto vblock = std::make_shared<siconos::algebra::BlockVector>(v2, v3);

   int row = 3, col = 3;
   // row X col Dense matrix:
   auto m= std::make_shared<siconos::algebra::SiconosMatrix>(row,col);
   // row X col matrix, all elements initialized with a scalar value:
   double a = 4.4;
   auto m = std::make_shared<siconos::algebra::SiconosMatrix>(row,col,a);
   // row X row Symmetric matrix:
   auto m = std::make_shared<siconos::algebra::SiconosMatrix>(row,row, Siconos::SYMMETRIC);
   // Read from a file
   auto m2 = std::make_shared<siconos::algebra::SiconosMatrix>("mat.dat",1); // 1: ascii, 0:binary
   // Build an empty vector and insert some existing vectors.
   auto V0 = std::make_shared<siconos::algebra::BlockVector>();
   // Pointer insertion: 
   V0->insertPtr(v1); 
...
Note that a BlockVector can also contain some other BlockVector::
  ...
  auto V1 = std::make_shared<siconos::algebra::BlockVector>();
  V1->insertPtr(V0);
  V1->insertPtr(v1);
...
::
   ...
   auto v= std::make_shared<siconos::algebra::SiconosVector>(3); // v = [0 0 0]
...
::
   ...
   auto A = std::make_shared<siconos::algebra::SiconosMatrix>(10,10,SPARSE);
   (*A)(0,0) = 12; // WRONG
...
::
   ...
   auto A = std::make_shared<siconos::algebra::SiconosMatrix>(*B);
   auto x= std::make_shared<siconos::algebra::SiconosVector>(*y);
...
To handle a specific block, use "[ ]" or getVector and getVectorPtr functions::
  ...
  auto v3= std::make_shared<siconos::algebra::SiconosVector>(3);  // v3 = [0 0 0]
  auto v4= std::make_shared<siconos::algebra::SiconosVector>(4);  // v4 = [0 0 0 0]
  ...
  *v3 = *vB->getVectorPtr(0);
  ...
  // Assignment:
  auto v5= std::make_shared<siconos::algebra::SiconosVector>(3);  // v5 = [0 0 0]
