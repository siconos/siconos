...
#include <algorithm>
#include <iostream>
Ropeway::Ropeway() { m_down = false; }

Ropeway::~Ropeway() {}

void Ropeway::compute(const Cable &a_meca, const std::vector<Pile> &a_piles, int nb_nodes,
...
void Ropeway::prepareSupport(std::vector<std::shared_ptr<Support>> &a_supports,
...
void Ropeway::addSupport(const Rope &a_rope, std::vector<std::shared_ptr<Support>> &a_supports,
                         int &a_pulleyIdx) const
{
...
int Ropeway::computeMesh(std::vector<Point> &a_q, std::vector<Point> &a_R,
...
const Pile &Ropeway::get_FirstPile() { return m_ropes.front().get_pile0(); }

const Pile &Ropeway::get_LastPile() { return m_ropes.back().get_pile0(); }
...
const Cable &Ropeway::get_meca0() const { return m_ropes.front().get_meca(); }
...
void Ropeway::set_Down(bool a_value) { m_down = a_value; }
