...
siconos::mechanics::fem::FiniteElementModel::vertexToNode(std::shared_ptr<MVertex> v) {
...
unsigned int siconos::mechanics::fem::FiniteElementModel::init() {
  ...
  for (auto &fe : elements()) {
    fe->setGlobalToLocalMatrix(_mesh->dim());
  }
...
void siconos::mechanics::fem::FiniteElementModel::AssembleElementaryMatrix(
    std::shared_ptr<siconos::algebra::SiconosMatrix> M, siconos::algebra::SimpleMatrix &Me,
    FElement &fe) {
  int node1_cnt = 0;
...
void siconos::mechanics::fem::FiniteElementModel::computeElementaryMassMatrix(
  ...
  for (const auto &gp : fe.GaussPoints(integrationOrder)) {
    if (_mesh->dim() == 2 and fe.family() == FiniteElementFamily::isoparametric) {
               ...
               fe.family() == FiniteElementFamily::isoparametric)  // Ugly
    {
      ...
      for (int i = 0; i < nnodes; i++) {
        for (int j = 0; j < nnodes; j++) {
          ...
          Me.setValue(
              i * 3 + 2, j * 3 + 2,
              coeff * N[i] * N[j] + Me.getValue(i * 3 + 2,
...
void siconos::mechanics::fem::FiniteElementModel::computeBeamElementaryMassMatrix_direct(
    siconos::algebra::SimpleMatrix &Me, FElement &fe, std::map<unsigned int, std::shared_ptr<Material>> &materials) {
  int dim = _mesh->dim();
  ...
  if (dim == 2)
  {
    ...
    Me_loc->setValue(3,0,Me_loc->getValue(0,3));

    Me_loc->setValue(1,1,massDensity*A*length*156/420);
    Me_loc->setValue(1,2,massDensity*A*length2*21/420);
    Me_loc->setValue(1,4,massDensity*A*length*54/420);
    Me_loc->setValue(1,5,massDensity*A*length2*(-13)/420);
    Me_loc->setValue(2,1,Me_loc->getValue(1,2));
    Me_loc->setValue(4,1,Me_loc->getValue(1,4));
    Me_loc->setValue(5,1,Me_loc->getValue(1,5));

    Me_loc->setValue(2,2,massDensity*A*length3*4/420);
    Me_loc->setValue(2,4,massDensity*A*length2*13/420);
    Me_loc->setValue(2,5,massDensity*A*length3*(-3)/420);
    Me_loc->setValue(4,2,Me_loc->getValue(2,4));
    Me_loc->setValue(5,2,Me_loc->getValue(2,5));

    Me_loc->setValue(3,3,massDensity*A*length*2/6);

    Me_loc->setValue(4,4,massDensity*A*length*156/420);
    Me_loc->setValue(4,5,massDensity*A*length2*(-21)/420);
    Me_loc->setValue(5,4,Me_loc->getValue(4,5));
...
void siconos::mechanics::fem::FiniteElementModel::computeElementaryStiffnessMatrix_direct(
   ...
   * this could be simplified by explicit formulae
   */
 if (_mesh->dim() == 2 and fe.family() == FiniteElementFamily::isoparametric)  // Ugly
  {
...
void siconos::mechanics::fem::FiniteElementModel::computeBeamElementaryStiffnessMatrix_direct(
    siconos::algebra::SimpleMatrix &Ke, FElement &fe, std::map<unsigned int, std::shared_ptr<Material>> &materials) {
  int dim = _mesh->dim();
  ...
  if (dim == 2)
  {
    ...
    Ke_loc->setValue(3,0,Ke_loc->getValue(0,3));

    Ke_loc->setValue(1,1,shearing);
    Ke_loc->setValue(1,2,shearOnBend);
    Ke_loc->setValue(1,4,-shearOnshear);
    Ke_loc->setValue(1,5,shearOnBend);
    Ke_loc->setValue(2,1,Ke_loc->getValue(1,2));
    Ke_loc->setValue(4,1,Ke_loc->getValue(1,4));
    Ke_loc->setValue(5,1,Ke_loc->getValue(1,5));

    Ke_loc->setValue(2,2,bending);
    Ke_loc->setValue(2,4,-shearOnBend);
    Ke_loc->setValue(2,5,bendOnBend);
    Ke_loc->setValue(4,2,Ke_loc->getValue(2,4));
    Ke_loc->setValue(5,2,Ke_loc->getValue(2,5));

    Ke_loc->setValue(3,3,axial);

    Ke_loc->setValue(4,4,shearing);
    Ke_loc->setValue(4,5,-shearOnBend);
  ...
  else if (dim==3)
  {
    ...
    for(int a=0;a<2;a++){
      for (int i=0; i<3;i++){
        for (int j=0; j<3;j++){
          Te->setValue(6*a + i,6*a + j,R->getValue(i,j));
        }
      }
    }
    for(int a=0;a<2;a++){
      for (int i=0; i<3;i++){
        Te->setValue(3+6*a + i,3+6*a + i,ones->getValue(i,i));
      }
    }
    ...
    Ke_loc->setValue(1,1,shearing);
    Ke_loc->setValue(1,5,shearOnBend);
    Ke_loc->setValue(1,7,-shearOnshear);
    Ke_loc->setValue(1,11,shearOnBend);
    Ke_loc->setValue(2,2,shearing);
    Ke_loc->setValue(2,4,-shearOnBend);
    ...
    Ke_loc->setValue(9,3,Ke_loc->getValue(3,9));

    Ke_loc->setValue(4,4,bending);
    Ke_loc->setValue(4,8,shearOnBend);
    Ke_loc->setValue(4,10,bendOnBend);
    Ke_loc->setValue(8,4,Ke_loc->getValue(4,8));
    Ke_loc->setValue(10,4,Ke_loc->getValue(4,10));
    Ke_loc->setValue(5,5,bending);
    Ke_loc->setValue(5,7,-shearOnBend);
    Ke_loc->setValue(5,11,bendOnBend);
    Ke_loc->setValue(7,5,Ke_loc->getValue(5,7));
    Ke_loc->setValue(11,5,Ke_loc->getValue(5,11));

    Ke_loc->setValue(6,6,axial);

    Ke_loc->setValue(7,7,shearing);
    Ke_loc->setValue(7,11,-shearOnBend);
    Ke_loc->setValue(11,7,Ke_loc->getValue(7,11));
    Ke_loc->setValue(8,8,shearing);
    Ke_loc->setValue(8,10,shearOnBend);
    Ke_loc->setValue(10,8,Ke_loc->getValue(8,10));

    Ke_loc->setValue(9,9,torsion);
...
void siconos::mechanics::fem::FiniteElementModel::computeElementary_B_Matrix(FElement& fe, siconos::algebra::SimpleMatrix& B, double length)
{
  int ndof = fe.ndof();
  auto &nodes = fe.nodes();
  int nnodes = nodes.size();
  ...
  for (const auto &gp : fe.GaussPoints(3)) {
    if (_mesh->dim() == 1)
    {
      ...
      for (int n = 0; n < nnodes; n++) {
        B.setValue(cpt, 0, 6*gp_eta/(length*length));
        B.setValue(cpt, 1, (3*gp_eta - 1)/length);
        B.setValue(cpt, 2, -6*gp_eta/(length*length));
        B.setValue(cpt, 3, (3*gp_eta + 1)/length);
      }
...
void siconos::mechanics::fem::FiniteElementModel::computeBeamElementaryBMatrix_direct(FElement& fe, siconos::algebra::SimpleMatrix& B,  std::map<unsigned int, std::shared_ptr<Material>> &materials)
{
  int ndof = fe.ndof();
  ...
  if (dim==2)
  {
    ...
    for (const auto &gp : fe.GaussPoints(integrationOrder)) {
      ...
      B.setValue(gp_cnt,2, sqrt(gp_w)*(3*gp_eta-1)/length);
      B.setValue(gp_cnt,4, sqrt(gp_w)*(-6)*gp_eta/length2);
      B.setValue(gp_cnt,5, sqrt(gp_w)*(3*gp_eta+1)/length);
      gp_cnt++;
    }
  ...
  {
    B.setValue(0,0, - 1/length); // axial
    B.setValue(0,6,   1/length); // axial

    B.setValue(3,3, - 1/length); // twisting
    B.setValue(3,9,   1/length); // twisting
  ...
  std::map<unsigned int, 	std::shared_ptr<Material> > & mat)
{
  ...
  int elem_cnt=0;
  std::shared_ptr<siconos::algebra::SimpleMatrix> Be;
  /* loop over the elements */
  for(std::shared_ptr<FElement> fe : elements())
  {
    if (fe->type() == FiniteElementType::B2){
      ...
      if (dim==2)
        ...
        Be = std::make_shared<siconos::algebra::SimpleMatrix>(6, 12);
    ...
    else
    {
      ...
      Be = std::make_shared<siconos::algebra::SimpleMatrix>(dimStress, fe->ndof());
...
siconos::mechanics::fem::FiniteElementModel::contactingNodes(int contact_entity_tag) {
  ...
  auto contacting_nodes = std::make_shared<std::list<std::shared_ptr<FENode>>>();
  ;
  ...
  for (auto &e : _mesh->elements()) {
    if (e->tags(0) == contact_entity_tag) {
      for (auto &v : e->vertices()) {
        ...
        {
          std::cout << "node number in contact zone " << n->num() << " vertex number "
                    << v->num() << std::endl;
