      subroutine n2qn1 (n, x, f, g, dxmin, df1, epsabs, imp, io,
     &mode, iter, nsim, binf, bsup, iz, rz, reverse)
      implicit double precision (a-h,o-z)
      save
      dimension x(n),g(n),dxmin(n),bsup(n),binf(n)
      dimension iz(*),rz(*)
      logical plantage, modifx, reverse
      double precision gpmopt
      if(reverse) indic=4
      if(reverse .and. mode.gt.7) go to 1111
      if (imp.gt.0) then
      nw=n*(9+n)/2
      ni=2*n+1
 ...
 1111 continue
      call n2qn1a (n,x,f,g,dxmin,epsabs,gpmopt,df1,mode,
     &iter,nsim,imp,io,rz,rz(nd),rz(nww),rz(nww1),
     &rz(nga),binf,bsup,iz(nindi),iz(nibloc),reverse,iz(ni))
      if(mode.eq.8 .or. mode.eq.9) return
 ...
 1005 format (5x,"bornes",
     ...
     &(9x,"| ",5(i5,": ",i2," |")))
      ...
      subroutine n2qn1a (n,x,f,g,dxmin,acc,gpmopt,
     &df1,mode,niter,nsim,iprint,lp,h,d,w,w1,ga,
     &binf,bsup,indi,ibloc,reverse,iz)
      implicit double precision (a-h,o-z)
      save
      dimension x(n),g(n),dxmin(n),h(*),d(n),w(n),w1(n)
      dimension binf(n),bsup(n),ga(n),iz(*)
      dimension ibloc(n),indi(n)
 ...
 1024 format (1x)
      if(reverse) indic=4
      if(reverse .and. mode.eq.8) goto 8888
      if(reverse .and. mode.eq.9) goto 9999
      do 10 i=1,n
   10 ga(i)=g(i)
      alfa=0.99d0
      beta=1.d-4
 ...
 1020 format (1x,79("-")/" n2qn1:",i4," iters",i6," simuls","   f=",
     &1pd15.7/)
      ...
      iecri=iecri+1
c     if (.not.reverse .and. iecri.eq.-iprint) then
c     iecri=0
c     indic=1
...
c     endif
  510 continue
      ...
      go to 540
  511 modbnd=1
      call fmani1(modbnd,n,ga,w,indi)
      wii=zero
      do i=1,nr
      wi=w(i)
      wiii=wi*dxmin(i)
 ...
 1022 format (" n2qn1:",i4," iters",i6," simuls","   f=",d15.7,
     &"   borne",i4,"  desactivee")
      ...
      logic=6
      ibloc(ncs)=0
      call fretc1(modbnd,n,ncs,nr,h,w,indi,indic2)
      indic2=0
      dnr=dsqrt(dble(float(nr)))
      acc1=acc*dnr
      if(modbnd.eq.0) go to 511
      mode=7
 ...
 9999 continue
      call nlis0 (n,fuclid,x,f,dga,ro,romin,romax,d,g,
     &alfa,beta,iprint,lp,logic,nfun,nsim,w,mode,reverse)
      if(mode.eq.9) return
  ...
  575 theta=one
     ...
     &      "update","y'*s = ", ys*ro,"s'*H*s = ", shs*ro*ro
            goto 900
         endif
         ol=ys/shs/ro
     ...
     &   "n2qn1: OL factor for matrix initialization ", ol
      ...
      modbnd = 1
      call fmani1(modbnd,n,d,w,indi)
      ir = -nr
      call fmani1(modbnd,n,ga,d,indi)
      do i = 1,nr
      d(i) = -d(i)
      enddo
     ...
     &   " >>> n2qn1: the updated BFGS matrix is rank deficient"
      ...
      goto 500
  900 if (mode.ne.5.and.mode.ne.3.and.mode.ge.0.and.mode.le.7)
     & go to 910
      moda=mode
      if (reverse) then
         mode=8
...
c        call simul (indic,n,x,f,ga,izs,rzs,dzs)
 ...
 8888 mode=moda
  ...
  998 ga(i)=g(i)
      ...
      subroutine nlis0 (n,prosca,x,f,fpn,t,tmin,tmax,d,g,
     &amd,amf,imp,io,logic,nap,napmax,xn,mode,reverse)
      save
      external prosca
      integer n,imp,io,logic,nap,napmax,mode
