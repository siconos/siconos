				      ...
				      Standard_Real& fval) 
{
  ...
  /*OB : here we just compute FV = f(P)*/
  if( F->Value(*P, *FV) ) {
...
mymath_FunctionSetRoot::mymath_FunctionSetRoot(math_FunctionSetWithDerivatives& F,
...
void mymath_FunctionSetRoot::Delete()
{}

void mymath_FunctionSetRoot::SetTolerance(const math_Vector& Tolerance)
{
  for (Standard_Integer i = 1; i <= Tol.Length(); i++) {
    Tol(i) = Tolerance(i);
  }
...
void mymath_FunctionSetRoot::Perform(math_FunctionSetWithDerivatives& F,
				   const math_Vector& StartingPoint,
	     		           const math_Vector& InfBound,
			           const math_Vector& SupBound) 
{
...
Standard_Boolean mymath_FunctionSetRoot::IsSolutionReached(math_FunctionSetWithDerivatives& ) {
...
void mymath_FunctionSetRoot::Dump(Standard_OStream& o) const {
...
void mymath_FunctionSetRoot::Root(math_Vector& Root) const{
...
void mymath_FunctionSetRoot::FunctionSetErrors(math_Vector& Err) const{
  StdFail_NotDone_Raise_if(!Done, " ");
