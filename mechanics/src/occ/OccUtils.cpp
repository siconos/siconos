...
void siconos::mechanics::occ::occ_move(TopoDS_Shape& shape, const siconos::algebra::SiconosVector& q) {
...
void siconos::mechanics::occ::occ_distanceFaceFace(const OccContactFace& csh1, const OccContactFace& csh2,
                                        ...
                                        Standard_Real& nZ, Standard_Real& MinDist) {
  // need the 2 sp pointers to keep memory
  std::shared_ptr<const TopoDS_Face> pface1 = csh1.contact();
  std::shared_ptr<const TopoDS_Face> pface2 = csh2.contact();
  ...
  if (measure.IsDone()) {
    ...
    for (Standard_Integer i = 1; i <= nb_solutions; ++i) {
      if (measure.SupportTypeShape2(i) == BRepExtrema_IsInFace) {
        ...
        gp_Dir normal = cadmbtb::FaceNormal(face2, u, v);
...
void siconos::mechanics::occ::occ_distanceFaceEdge(const OccContactFace& csh1, const OccContactEdge& csh2,
                                        Standard_Real& X1, Standard_Real& Y1,
                                        Standard_Real& Z1, Standard_Real& X2,
                                        Standard_Real& Y2, Standard_Real& Z2,
                                        Standard_Real& nX, Standard_Real& nY,
  ...
  // need the 2 sp pointers to keep memory
  std::shared_ptr<const TopoDS_Face> pface1 = csh1.contact();
  std::shared_ptr<const TopoDS_Edge> pedge2 = csh2.contact();
  ...
  if (measure.IsDone()) {
    ...
    for (Standard_Integer i = 1; i <= nb_solutions; ++i) {
      ...
      if (measure.SupportTypeShape1(i) == BRepExtrema_IsInFace) {
        ...
        gp_Dir normal = cadmbtb::FaceNormal(face1, u, v);
        normal.Coord(nX, nY, nZ);
        X1 = p1.X();
        X2 = p2.X();
        Y1 = p1.Y();
