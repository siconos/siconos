...
std::shared_ptr<btCollisionObject> siconos::collision::bullet::internal::
    SiconosBulletCollisionManager_impl::createCollisionObjectHelper(
        ...
        std::shared_ptr<StaticBody> staticBody, std::shared_ptr<btConvexShape> childShape) {
  ...
  auto record = std::make_shared<BR>(base, ds, shape, btshape, btobject, contactor, staticBody,
                                     childShape);
...
#endif
    if (auto rbds = std::dynamic_pointer_cast<RigidBodyDS>(record->ds)) {
      if (rbds->useContactorInertia()) {
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::updateShape(
  ...
  if (box->version() != record->shape_version) {
    ...
    if (auto rbds = std::dynamic_pointer_cast<RigidBodyDS>(record->ds)) {
      if (rbds->useContactorInertia()) updateContactorInertia(rbds, btbox);
    }
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::updateShape(
  ...
  if (cyl->version() != record->shape_version) {
    ...
    if (auto rbds = std::dynamic_pointer_cast<RigidBodyDS>(record->ds)) {
      if (rbds->useContactorInertia()) updateContactorInertia(rbds, btcyl);
    }
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::updateShape(
  ...
  if (cone->version() != record->shape_version) {
    ...
    if (auto rbds = std::dynamic_pointer_cast<RigidBodyDS>(record->ds)) {
      if (rbds->useContactorInertia()) updateContactorInertia(rbds, btcone);
    }
    if (record->btobject->getBroadphaseHandle()) {
      _collisionWorld->updateSingleAabb(&*record->btobject);
      //      _collisionWorld->getBroadphase()->getOverlappingPairCache()->
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::updateShape(
  ...
  if (capsule->version() != record->shape_version) {
    ...
    btcapsule->setMargin((capsule->insideMargin() + capsule->outsideMargin()) *
                         _options->worldScale);
    if (auto rbds = std::dynamic_pointer_cast<RigidBodyDS>(record->ds)) {
      if (rbds->useContactorInertia()) updateContactorInertia(rbds, btcapsule);
    }
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::updateShape(
  ...
  if (ch->version() != record->shape_version) {
    ...
    btch->setMargin((ch->insideMargin() + ch->outsideMargin()) * _options->worldScale);
    if (auto rbds = std::dynamic_pointer_cast<RigidBodyDS>(record->ds)) {
      if (rbds->useContactorInertia()) updateContactorInertia(rbds, btch);
    }
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::
  ...
  // Real Bullet (3D) shape
  auto childShape2 = std::make_shared<btCylinderShapeZ>(
      btVector3(btScalar(SCALING * 1), btScalar(SCALING * 1), btScalar(_options->Depth2D)));

  // "Wrap" 3D into 2D (see https://pybullet.org/Bullet/BulletFull/classbtConvex2dShape.html)
  auto btconvex2d1 = std::make_shared<btConvex2dShape>(childShape2.get());
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::updateShape(
  ...
  if (disk->version() != record->shape_version) {
    ...
    if (auto rbds = std::dynamic_pointer_cast<RigidBody2dDS>(record->ds)) {
      if (rbds->useContactorInertia()) update2DContactorInertia(rbds, btconvex2d);
    }
    if (record->btobject->getBroadphaseHandle()) {
      _collisionWorld->updateSingleAabb(&*record->btobject);
      // _collisionWorld->getBroadphase()->getOverlappingPairCache()->
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::
  ...
  // Real bullet (3D) object
  auto childShape0 = std::make_shared<btBoxShape>(
      btVector3(btScalar(SCALING * 1), btScalar(SCALING * 1), btScalar(SCALING * 1)));

  // Wrapped into 2d bullet object
  auto btconvex2d = std::make_shared<btConvex2dShape>(childShape0.get());
...
void siconos::collision::bullet::internal::SiconosBulletCollisionManager_impl::
  ...
  auto btconvex2d = std::make_shared<btConvex2dShape>(btch.get());
