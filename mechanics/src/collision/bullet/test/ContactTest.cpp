// -*- compile-command: "make -C ~/projects/siconos/bld/mechanics && valgrind --leak-check=full
// --suppressions=$HOME/projects/siconos/cmake/valgrind.supp
// ~/projects/siconos/bld/mechanics/src/collision/bullet/test/testContact ContactTest" -*- make
// --no-print-directory -C ~/projects/siconos/bld/mechanics &&
// ~/projects/siconos/bld/mechanics/src/collision/bullet/test/testContact ContactTest::t2 |
// grep pos, | cut -d, -f2-8 | plot.py -s
...
static BounceResult bounceTest(std::string moving, std::string ground,
  ...
  if (moving == "sphere") {
    ...
    contactors->emplace_back(std::make_shared<siconos::collision::SiconosContactor>(sphere));
  } else if (moving == "box") {
    ...
    contactors->emplace_back(std::make_shared<siconos::collision::SiconosContactor>(ch));
  }
  ...
  while (simulation->hasNextEvent()) {
    ...
    auto interactions = collisionMan->statistics().new_interactions_created +
...
void ContactTest::t2() {
  try {
    ...
    printf("sphere | %#6.3f | %#6.3f | %#6.3f\n", results[0][0].final_position,
           results[0][1].final_position, results[0][2].final_position);
    printf("box    | %#6.3f | %#6.3f | %#6.3f\n", results[1][0].final_position,
           results[1][1].final_position, results[1][2].final_position);
    printf("\nFinal resting position standard deviation:\n\n");
    printf("       | sphere   | box      | plane\n");
    printf("-------+----------+----------+---------\n");
    printf("sphere | %#.2e | %#.2e | %#.2e\n", results[0][0].final_position_std,
           results[0][1].final_position_std, results[0][2].final_position_std);
    printf("box    | %#.2e | %#.2e | %#.2e\n", results[1][0].final_position_std,
           results[1][1].final_position_std, results[1][2].final_position_std);
  } catch (...) {
    siconos::exception::process();
    CPPUNIT_ASSERT(0);
  }
  ...
  try {
    ...
    for (i = 0; i < 3; i++) {
      fprintf(stderr, "   pos %.2f: %f  (std=%f)\n", params[i].position,
              results[i].final_position, results[i].final_position_std);
    }
  } catch (...) {
    siconos::exception::process();
    CPPUNIT_ASSERT(0);
