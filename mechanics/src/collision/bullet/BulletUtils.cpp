...
void siconos::collision::bullet::display_info_collision_object(
    const btCollisionObject* collisionObject) {
  printf("---- collision_object \n");

  const auto* collisionShape = collisionObject->getCollisionShape();
  ...
  if (collisionShape->getShapeType() == 25) {
    ...
    auto* gimpact_mesh_shape = (btGImpactMeshShape*)collisionShape;
    auto* striding_mesh = gimpact_mesh_shape->getMeshInterface();
    auto* triangle_mesh = (btTriangleIndexVertexArray*)striding_mesh;
    auto& mesh_array = triangle_mesh->getIndexedMeshArray();
    for (int i = 0; i < triangle_mesh->getNumSubParts(); i++) {
      printf(" mesh_array[%i] : number of triangles = %i\n", i, mesh_array[i].m_numTriangles);
      auto* triangleIndexBase = (int*)mesh_array[i].m_triangleIndexBase;
      int k = 0;
      ...
      auto* vertexBase = (btScalar*)mesh_array[i].m_vertexBase;
      k = 0;
  ...
  if (collisionShape->getShapeType() == CONVEX_2D_SHAPE_PROXYTYPE) {
    ...
    auto* btConvex2d = (btConvex2dShape*)collisionShape;
    auto* btconvexchild = (btConvexShape*)(btConvex2d->getChildShape());
    ...
    if (btconvexchild->getShapeType() == CONVEX_HULL_SHAPE_PROXYTYPE) {
      auto* btch = (btConvexHullShape*)btconvexchild;
...
void siconos::collision::bullet::display_info_manifold(const btPersistentManifold& manifold) {
  printf("-------- manifold : %p\n", &manifold);
...
void siconos::collision::bullet::display_info_contact_point(const btManifoldPoint& cp) {
  printf("   --------- contact point: %p \n", &cp);
  printf("   m_partId0: %i  m_index0 : %i \n", cp.m_partId0, cp.m_index0);

  auto lpA = cp.m_localPointA;
  printf("   lpA x , y, x : %e\t, %e\t, %e\t \n", lpA.x(), lpA.y(), lpA.z());
  auto lpB = cp.m_localPointB;
  printf("   lpB x , y, x : %e\t, %e\t, %e\t \n", lpB.x(), lpB.y(), lpB.z());

  auto pA = cp.m_positionWorldOnA;
  printf("   pA x , y, x : %e\t, %e\t, %e\t \n", pA.x(), pA.y(), pA.z());
  auto pB = cp.m_positionWorldOnB;
  printf("   pB x , y, x : %e\t, %e\t, %e\t \n", pB.x(), pB.y(), pB.z());

  auto normalOnB = cp.m_normalWorldOnB;
...
void siconos::collision::bullet::display_info_btConvexHullShape(
    const btConvexHullShape& btch) {
  ...
  printf("   number of vertices: %i \t, edge: %i\t, planes: %i\n", numVertices, numEdges,
         numPlanes);
  const auto* points = btch.getPoints();
