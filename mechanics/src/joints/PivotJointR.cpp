...
siconos::joints::PivotJointR::PivotJointR()
    : KneeJointR{}, _A{std::make_shared<siconos::algebra::SiconosVector>(3)} {
  _axes.resize(1);
}

siconos::joints::PivotJointR::PivotJointR(std::shared_ptr<siconos::algebra::SiconosVector> P,
    ...
    : KneeJointR{P, absoluteRef, d1, d2},
      _A{std::make_shared<siconos::algebra::SiconosVector>(3)} {
  _axes.resize(1);
...
void siconos::joints::PivotJointR::setBasePositions(
  ...
  *_A = *_axes[0];
  ...
  if (q2)
    rot2to1((*q1)(3), (*q1)(4), (*q1)(5), (*q1)(6), (*q2)(3), (*q2)(4), (*q2)(5), (*q2)(6),
            &rot2to1w, &rot2to1x, &rot2to1y, &rot2to1z);
  else
    rot2to1((*q1)(3), (*q1)(4), (*q1)(5), (*q1)(6), 1, 0, 0, 0, &rot2to1w, &rot2to1x,
            &rot2to1y, &rot2to1z);
...
void siconos::joints::PivotJointR::buildA1A2() {
  ...
  double Ay = (*_A)(1);
  double Az = (*_A)(2);
  if (orthoBaseFromVector(&Ax, &Ay, &Az, &_A1x, &_A1y, &_A1z, &_A2x, &_A2y, &_A2z))
    THROW_EXCEPTION(
  ...
  // std::cout << "JointPivot: _A1x _A1y _A1z :" << _A1x << " " << _A1y << " " << _A1z <<
  // std::endl; std::cout << "JointPivot: _A2x _A2y _A2z :" << _A2x << " " << _A2y << " " <<
  // _A2z << std::endl;
}

void siconos::joints::PivotJointR::Jd1d2(double X1, double Y1, double Z1, double q10,
...
void siconos::joints::PivotJointR::rot2to1(double q10, double q11, double q12, double q13,
...
double siconos::joints::PivotJointR::AscalA1(double rot2to1x, double rot2to1y,
                                             double rot2to1z) {
  /*
...
double siconos::joints::PivotJointR::AscalA2(double rot2to1x, double rot2to1y,
                                             double rot2to1z) {
  /*
...
double siconos::joints::PivotJointR::AscalA(double rot2to1x, double rot2to1y,
                                            double rot2to1z) {
  /*
...
void siconos::joints::PivotJointR::computeh(double time,
  ...
  rot2to1(q10, q11, q12, q13, q20, q21, q22, q23, nullptr, &rot2to1x, &rot2to1y, &rot2to1z);
...
void siconos::joints::PivotJointR::computehDoF(double time,
  ...
  double rot2to1w, rot2to1x, rot2to1y, rot2to1z;
  rot2to1(q10, q11, q12, q13, q20, q21, q22, q23, &rot2to1w, &rot2to1x, &rot2to1y, &rot2to1z);
...
void siconos::joints::PivotJointR::computeJachqDoF(
  ...
  double x19 = q10 * x18;
  double x20 = _A->getValue(0) * (q10 * x4 + q11 * x18 - q12 * x14 + q13 * x9) +
               _A->getValue(1) * (q10 * x9 + q11 * x14 + q12 * x18 - q13 * x4) +
               _A->getValue(2) * (q10 * x14 - q11 * x9 + q12 * x4 + q13 * x18);
  ...
  jachq.setValue(
      0, 3,
      x18 * x23 + x22 * (_A->getValue(0) * x4 + _A->getValue(1) * x9 + _A->getValue(2) * x14));
  jachq.setValue(
      0, 4,
      x22 * (_A->getValue(0) * x18 + _A->getValue(1) * x14 + _A->getValue(2) * x24) +
          x23 * x25);
  jachq.setValue(0, 5,
                 x22 * (_A->getValue(0) * x26 + _A->getValue(1) * x18 + _A->getValue(2) * x4) +
                     x23 * x24);
  jachq.setValue(0, 6,
  ...
  jachq.setValue(
      0, 10,
      x22 * (_A->getValue(0) * (x30 + x31 + x33) + _A->getValue(1) * (x34 + x35 + x40) +
             _A->getValue(2) * (x44 + x45 + x47)) +
          x23 * (x48 + x49 + x52));
  jachq.setValue(
      0, 11,
      x22 * (_A->getValue(0) * (-x49 + x52 + x53) + _A->getValue(1) * (x44 + x46 + x54) +
             _A->getValue(2) * (-x34 + x40 + x55)) +
          x23 * (x30 + x32 + x56));
  jachq.setValue(
      0, 12,
      x22 * (_A->getValue(0) * (-x41 + x43 + x59) + _A->getValue(1) * (-x50 + x51 + x57) +
             _A->getValue(2) * (x27 + x28 + x58)) +
          x23 * (x37 + x38 + x60));
  jachq.setValue(
      0, 13,
      x22 * (_A->getValue(0) * (x36 + x39 + x60) + _A->getValue(1) * (-x27 + x29 + x58) +
             _A->getValue(2) * (x50 - x51 + x57)) +
          x23 * (x41 + x42 + x59));
}

/** Return the normal of the angular DoF axis of rotation.
 * \param axis must be 0 */
void siconos::joints::PivotJointR::_normalDoF(siconos::algebra::SiconosVector& ans,
                                              const siconos::algebra::BlockVector& q0,
