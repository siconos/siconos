...
#include "SiconosConst.hpp"  // for DEFAULT_TOL_ED
 ...
 */
EventDriven::EventDriven(SP::NonSmoothDynamicalSystem nsds, SP::TimeDiscretisation td)
    : Simulation(nsds, td), _istate(1), _isNewtonConverge(false) {
  _numberOfOneStepNSproblems = 2;
  (*_allNSProblems).resize(_numberOfOneStepNSproblems);
  _newtonTolerance = siconos::internal::DEFAULT_TOL_ED;
...
EventDriven::EventDriven(SP::NonSmoothDynamicalSystem nsds, SP::TimeDiscretisation td, int nb)
    : Simulation(nsds, td), _istate(1), _isNewtonConverge(false) {
...
double EventDriven::_TOL_ED = siconos::internal::DEFAULT_TOL_ED;
...
void EventDriven::updateIndexSet(unsigned int i) {
  ...
  assert(i > 0 && "EventDriven::updateIndexSet(i=0), indexSets[0] cannot be updated.");
...
void EventDriven::updateIndexSetsWithDoubleCondition() {
  ...
  for (vnext = ui; ui != uiend; ui = vnext) {
    ...
    if (fabs(F) < _TOL_ED)
      ...
      THROW_EXCEPTION(
          "EventDriven::updateIndexSetsWithDoubleCondition(), output[2] and lambda[2] for "
          "Interaction of indexSet[2] must be nonnegative.");
    } else if (((fabs(gamma) > _TOL_ED) && (fabs(F) > _TOL_ED))) {
      THROW_EXCEPTION(
          "EventDriven::updateIndexSetsWithDoubleCondition(), something is wrong for the LCP "
          "resolution.");
    } else {
    };
...
void EventDriven::initializeOneStepNSProblem() {
  ...
  if (!_allNSProblems->empty())  // ie if at least a non smooth problem has been built.
  {
    ...
    if (osiType == OSI::LSODAROSI ||
        osiType == OSI::HEM5OSI)  // EventDriven associated with LsodarOSI OSI
    {
    } else if (osiType == OSI::NEWMARKALPHAOSI)  // EventDriven associated with NewMarkAlpha
    {
      if (_allNSProblems->size() != 3)
        THROW_EXCEPTION(
            " EventDriven::initialize, \n an EventDriven simulation associated with "
            "NewMarkAlphaOSI must have three non smooth problems.\n Here, there are " +
    ...
    if (!((*_allNSProblems)[SICONOS_OSNSP_ED_IMPACT])) /* ie if the impact problem does not
                                                        *  exist */
      THROW_EXCEPTION(
          "EventDriven::initialize, an EventDriven simulation must have an 'impact' non "
          "smooth problem.");

    if (!((*_allNSProblems)[SICONOS_OSNSP_ED_SMOOTH_ACC])) /* ie if the acceleration-level
                                                            * problem does not exist */
      THROW_EXCEPTION(
          "EventDriven::initialize, an EventDriven simulation must have an 'acceleration' non "
          "smooth problem.");
    ...
    //     _eventsManager->scheduleNonSmoothEvent(*this, _eventsManager->startingTime(),
    //     false);
    //   };
    // }
  }
...
void EventDriven::computef(OneStepIntegrator& osi, integer* sizeOfX, doublereal* time,
                           doublereal* x, doublereal* xdot) {
  DEBUG_BEGIN(
      "EventDriven::computef(OneStepIntegrator& osi, integer * sizeOfX, doublereal * time, "
      "doublereal * x, doublereal * xdot)\n");
...
void EventDriven::computeJacobianfx(OneStepIntegrator& osi, integer* sizeOfX, doublereal* time,
                                    doublereal* x, doublereal* jacob) {
  assert(osi.getType() == OSI::LSODAROSI);
  ...
  for (std::tie(dsi, dsend) = osiDSGraph->vertices(); dsi != dsend; ++dsi) {
    ...
    if (dsType == Type::LagrangianDS || dsType == Type::LagrangianLinearTIDS) {
      ...
      for (unsigned int j = 0; j < lds.n(); ++j) {
        for (unsigned int k = 0; k < lds.dimension(); ++k) jacob[i++] = jacotmp(k, j);
      }
    } else if (dsType == Type::FirstOrderNonLinearDS || dsType == Type::FirstOrderLinearDS ||
               dsType == Type::FirstOrderLinearTIDS) {
      SimpleMatrix& jacotmp =
          static_cast<SimpleMatrix&>(*(ds.jacobianRhsx()));  // Pointer link !
      pos += jacotmp.copyData(&jacob[pos]);
    } else {
      THROW_EXCEPTION(
          "EventDriven::computeJacobianfx, type of DynamicalSystem not yet supported.");
    }
  }
}

unsigned int EventDriven::computeSizeOfg() { return (_indexSet0->size()); }

void EventDriven::computeg(SP::OneStepIntegrator osi, integer* sizeOfX, doublereal* time,
                           doublereal* x, integer* ng, doublereal* gOut) {
  assert(_nsds);
  assert(_nsds->topology());
  InteractionsGraph::VIterator ui, uiend;
  SP::Topology topo = _nsds->topology();
...
void EventDriven::updateImpactState() {
  ...
  for (itOSI = _allOSI->begin(); itOSI != _allOSI->end(); ++itOSI) (*itOSI)->updateState(1);
}
...
void EventDriven::updateSmoothState() {
  ...
  _nsds->updateInput(
      nextTime(),
      2);  // Note FP : Probably already up to date? (previous call of updateInput in simu)
  OSIIterator itOSI;
  // Compute acceleration
  for (itOSI = _allOSI->begin(); itOSI != _allOSI->end(); ++itOSI) (*itOSI)->updateState(2);
}
  ...
  if (osiType == OSI::NEWMARKALPHAOSI) {
    ...
    if (_istate != 2)  // some events occur
    {
      ...
      if (std::abs(_minConstraint) <
          _TOL_ED)  // events occur at the end of the integration step
      {
    ...
    for (it = _allOSI->begin(); it != _allOSI->end(); ++it) {
      ...
      if (_istate ==
          3)  // ie if _tout is not equal to _tend: one or more roots have been found.
      {
      ...
      //     SA::integer iwork = lsodar->getIwork();
      //     SA::doublereal Rwork = lsodar->getRwork();
      //     statOut << "Number of steps: " << iwork[10] << ", number of f evaluations: " <<
      //     iwork[11] << ", number of jacobianF eval.: " << iwork[12] << "." <<endl;
      //   }
    }
    ...
    updateOutput(1);
    updateOutput(2);
    // update lambda[2], input[2] and indexSet[2] with double consitions for the case there is
    // no new event added during time integration, otherwise, this
    ...
    THROW_EXCEPTION(
        "In EventDriven::advanceToEvent, this type of OneStepIntegrator does not exist for "
        "Event-Driven scheme!!!");
  }
...
double EventDriven::computeResiduConstraints() {
  ...
  for (OSIIterator itosi = _allOSI->begin(); itosi != _allOSI->end(); ++itosi) {
    if ((*itosi)->getType() == OSI::NEWMARKALPHAOSI) {
      ...
      THROW_EXCEPTION(
          "In EventDriven::predictionNewtonIteration, the current OSI must be NewMarkAlpha "
          "scheme!!!");
    }
  }
...
void EventDriven::prepareNewtonIteration() {
  ...
  for (OSIIterator itosi = _allOSI->begin(); itosi != _allOSI->end(); ++itosi) {
    ...
    if (osiType != OSI::NEWMARKALPHAOSI) {
      THROW_EXCEPTION(
          "In EventDriven::prepareNewtonIteration, the current OSI is not NewMarkAlpha "
          "scheme!!!");
    }
...
void EventDriven::predictionNewtonIteration() {
  ...
  for (std::tie(ui, uiend) = _indexSet0->vertices(); ui != uiend; ++ui) {
    Interaction& inter = *_indexSet0->bundle(*ui);
    inter.computeOutput(t, 0);  // compute y[0] for the interaction at the end time with the
                                // state predicted for Dynamical Systems
...
void EventDriven::correctionNewtonIteration() {
  ...
  for (OSIIterator itosi = _allOSI->begin(); itosi != _allOSI->end(); ++itosi) {
    if ((*itosi)->getType() == OSI::NEWMARKALPHAOSI) {
      ...
      THROW_EXCEPTION(
          "In EventDriven::correctionNewtonIteration, the current OSI must be NewMarkAlpha "
          "scheme!!!");
    }
  }
}
...
void EventDriven::newtonSolve(double criterion, unsigned int maxStep) {
  ...
  while (1 != 0) {
    ...
    if (_newtonNbIterations > maxStep) {
      cout << "Warning!!!In EventDriven::newtonSolve: Number of iterations is greater than "
              "the maximum value "
           << maxStep << endl;
    }
...
double EventDriven::detectEvents(bool updateIstate) {
  ...
  double _minResiduOutput =
      0.0;  // maximum of g_i with i running over all activated or deactivated contacts
  ...
  for (std::tie(ui, uiend) = _indexSet0->vertices(); ui != uiend; ++ui) {
    ...
    if (nsLawSize != 1) {
      THROW_EXCEPTION(
          "In EventDriven::detectEvents, the interaction size > 1 has not been implemented "
          "yet!!!");
    }
    y = inter->y(0);                     // output y at this Interaction
    ydot = inter->y(1);                  // output of level 1 at this Interaction
    ...
    //
    DEBUG_EXPR_WE(cout.precision(15); cout << "Contact number: " << inter->number() << endl;
...
void EventDriven::LocalizeFirstEvent() {
  // We localize the first event occuring during the integration step when the flag _istate = 3
  // or 4 Compute the coefficients of the dense output polynomial for all DSs
  for (OSIIterator itosi = _allOSI->begin(); itosi != _allOSI->end(); ++itosi) {
    if ((*itosi)->getType() == OSI::NEWMARKALPHAOSI) {
      SP::NewMarkAlphaOSI osi_NewMark = std::static_pointer_cast<NewMarkAlphaOSI>(*itosi);
      ...
      THROW_EXCEPTION(
          "In EventDriven::LocalizeFirstEvent, the current OSI must be NewMarkAlpha "
          "scheme!!!");
    }
  }
  //
  ...
  while (!found) {
    ...
    // If _istate = 3 or 5, i.e. some contacts are closed, we need to compute y[0] for all
    // interactions
    if ((_istate == 3) || (_istate == 5))  // some contacts are closed
    {
      _nsds->updateOutput(t_i, 0);
    }
    // If _istate = 4 or 5, i.e. some contacts are detached, we need to solve LCP at the
    // acceleration level to compute contact forces
    ...
    if (_minConstraint < -_TOL_ED) {
      t_b = t_i;
    } else  // if no event is detected in [t_a, t_i], then we have to detect events in the
            // interval [t_i, t_b]
    {
      t_a = t_i;
    }
    ...
    if (_numIter > _localizeEventMaxIter) {
      THROW_EXCEPTION(
          "In EventDriven::LocalizeFirstEvent, the numbner of iterations performed is too "
          "large!!!");
    }
  }
}
