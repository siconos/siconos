 ...
 * work of J.J. Moreau for the sweeping process
 *
 * J.J. Moreau. Evolution problem associated with a moving convex set in a
 * Hilbert space. Journal of Differential Equations, 26, pp 347--374, 1977.
 *
 * Variants are now used to integrate LCS, Relay systems, Higher order sweeping
 * process see for instance
 *
 *
 * Consistency of a time-stepping method for a class of piecewise linear
 * networks
 *
 ...
 *
 * Convergence of time-stepping schemes for passive and extended linear
 * complementarity systems L. Han, A. Tiwari, M.K. Camlibel, and J.-S. Pang SIAM
 * Journal on Numerical Analysis 2009, 47(5):3768-3796
 *
 * On preserving dissipativity properties of linear complementarity dynamical
 * systems with the &theta-method
 *
 * Greenhalgh Scott, Acary Vincent, Brogliato Bernard Numer. Math., , 2013.
 *
 * Main time--integration schemes are based on the following \f$\theta-\gamma\f$
 * scheme
 *
 * \rststar
 *
 ...
 *     \begin{cases}
 *      \label{eq:toto1}
 *      M x_{k+1} = M x_{k} +h\theta f(x_{k+1},t_{k+1})+h(1-\theta) f(x_k,t_k) +
 * h \gamma r(t_{k+1})
 *      + h(1-\gamma)r(t_k)  \\[2mm]
 ...
 *
 *     \begin{cases}
 *     M x_{k+1} = M x_{k} +h f(x_{k+\theta},t_{k+1}) + h r(t_{k+\gamma})
 * \\[2mm] y_{k+\gamma} =  h(t_{k+\gamma},x_{k+\gamma},\lambda _{k+\gamma})
 * \\[2mm] r_{k+\gamma} = g(x_{k+\gamma},\lambda_{k+\gamma},t_{k+\gamma})\\[2mm]
 ...
 * list of first order dynamical systems. A EulerMoreauOSI instance is defined
 * by the value of theta  and possibly gamma and the list of concerned dynamical
 * systems.
 *
...
public:
   ...
   */
  inline void setUseGamma(bool b) { _useGamma = b; };
   ...
   * invocation) \param W the matrix to compute
   */
   ...
   */
  void updateOutput(double time, unsigned int level) override;
  ...
  double computeResiduOutput(double time,
                             SP::InteractionsGraph indexSet) override;
