...
/**
   ...
   - Sparse matrix (_storageType = NM_SPARSE): at the time of writting, only csc
   (compressed-sparse column). Could also be triplet (coo or coordinate) or csr
...
class OSNSMatrix {
 ...
 public:
   ...
   */
  inline SP::NumericsMatrix numericsMatrix() { return _numericsMatrix; };

  /** get the matrix used for default storage
   *
   *  \return SP::NumericsMatrix
   */
  inline SP::SiconosMatrix defaultMatrix() { return _M1; };
   ...
   *  \param update if true update the size of the Matrix (default true)
   */
  virtual void fillH(DynamicalSystemsGraph& DSG, InteractionsGraph& indexSet,
                     bool update = true);
   ...
   *  \param update if true update the size of the Matrix (default true)
   */
  virtual void fillHtrans(DynamicalSystemsGraph& DSG, InteractionsGraph& indexSet,
                          bool update = true);
