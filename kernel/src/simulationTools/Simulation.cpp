    ...
    unsigned int level, unsigned int coor)
{
  ...
  DEBUG_BEGIN(
      "siconos::simulation::Simulation::output(unsigned int level, unsigned int coor)\n");
    ...
    unsigned int level, unsigned int coor)
{
  ...
  DEBUG_BEGIN(
      "siconos::simulation::Simulation::input(unsigned int level, unsigned int coor)\n");
  DEBUG_PRINTF("with level = %i and coor = %i \n", level, coor);
  ...
  DEBUG_END("siconos::simulation::Simulation::input(unsigned int level, unsigned int coor)\n");
  return lambda;
}
...
void siconos::simulation::Simulation::updateInput(unsigned int)
{
  DEBUG_BEGIN("siconos::simulation::Simulation::updateInput()\n");
  // 1 - compute input (lambda -> r)
  if (!_allNSProblems->empty()) {
    for (auto osi : *_allOSI) {
      osi->updateInput(nextTime());
    }
    //_nsds->updateInput(nextTime(),levelInput);
  }
  DEBUG_END("siconos::simulation::Simulation::updateInput()\n");
}
...
// void
// siconos::simulation::Simulation::prepareIntegratorForDS(std::shared_ptr<siconos::integrators::OneStepIntegrator>
// osi,
//                                         std::shared_ptr<siconos::modeling::DynamicalSystem>
//                                         ds, SP::Model m, double time)
// {
//   assert(m && m->nonSmoothDynamicalSystem() &&
//   "siconos::simulation::Simulation::prepareIntegratorForDS requires a Model with an NSDS.");
...
//    *
//    * 2. Add the OSI to simulation->_allOSI
//    (siconos::simulation::Simulation::insertIntegrator)
//    *
...
//    *    be called later during siconos::simulation::Simulation::initialize().
//   */
...
//   // (Typically, during siconos::simulation::Simulation::initialize())
