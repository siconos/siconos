...
void LinearOSNS::initOSNSMatrix() {
        ...
        case NM_SPARSE_BLOCK: {
          ...
          if (indexSetLevel() != siconos::internal::LEVELMAX &&
              simulation()->nonSmoothDynamicalSystem()->topology()->indexSetsSize() >
                  indexSetLevel()) {
            _M.reset(new OSNSMatrix(simulation()->indexSet(indexSetLevel())->size(),
  ...
  if (_assemblyType == GLOBAL) {
    ...
    if (!_W) {
      ...
      //   _W.reset(new
      //   OSNSMatrix(simulation()->nonSmoothDynamicalSystem()->dynamicalSystems()->size(),
      //   1));
    ...
    if (!_H) {
      switch (_numericsMatrixStorageType) {
        ...
        case NM_SPARSE: {
          _H.reset(
              new OSNSMatrix(0, simulation()->indexSet(_indexSetLevel)->size(), NM_SPARSE));
          break;
        }
        case NM_SPARSE_BLOCK: {
          _H.reset(new OSNSMatrix(
              simulation()->nonSmoothDynamicalSystem()->dynamicalSystems()->size(),
  ...
  if (_assemblyType == REDUCED_DIRECT) {
    ...
    if (!_W_inverse) {
      ...
      switch (_numericsMatrixStorageType) {
        ...
        case NM_SPARSE_BLOCK: {
          _W_inverse.reset(new OSNSMatrix(
              simulation()->nonSmoothDynamicalSystem()->dynamicalSystems()->size(),
              NM_SPARSE_BLOCK));
          break;
        }
...
void LinearOSNS::computeDiagonalInteractionBlock(const InteractionsGraph::VDescriptor& vd) {
  DEBUG_BEGIN(
      "LinearOSNS::computeDiagonalInteractionBlock(const InteractionsGraph::VDescriptor& "
      "vd)\n");
  ...
  for (SP::DynamicalSystem ds = ds1; !endl; ds = ds2) {
    ...
    if (relationType == FirstOrder) {
      ...
      if (osiType == OSI::EULERMOREAUOSI) {
        if ((static_cast<EulerMoreauOSI&>(osi)).useGamma() ||
            (static_cast<EulerMoreauOSI&>(osi)).useGammaForRelation()) {
        ...
        if (relationSubType == CompliantLinearTIR) {
          if (osiType == OSI::MOREAUJEANOSI) {
            ...
            *currentInteractionBlock +=
                *std::static_pointer_cast<LagrangianCompliantLinearTIR>(inter->relation())
                     ->D() /
                simulation()->timeStep();
          }
        }
      ...
      THROW_EXCEPTION(
          "LinearOSNS::computeDiagonalInteractionBlock not yet implemented for relation of "
          "type " +
  ...
  DEBUG_END(
      "LinearOSNS::computeDiagonalInteractionBlock(const InteractionsGraph::VDescriptor& vd) "
      "ends \n");
}

void LinearOSNS::computeInteractionBlock(const InteractionsGraph::EDescriptor& ed) {
  DEBUG_BEGIN(
      "LinearOSNS::computeInteractionBlock(const InteractionsGraph::EDescriptor& ed)\n");
  ...
  if (relationType1 == FirstOrder && relationType2 == FirstOrder) {
    ...
    *leftInteractionBlock *= h;
    ...
    // left = C, right = inv(W).B
             ...
             relationType1 == NewtonEuler || relationType2 == NewtonEuler) {
    // Applying boundary conditions
    SP::BoundaryCondition bc;
    SP::SecondOrderDS d = std::static_pointer_cast<SecondOrderDS>(ds);
    if (d->boundaryConditions()) bc = d->boundaryConditions();
    ...
    THROW_EXCEPTION(
        "LinearOSNS::computeInteractionBlock not yet implemented for relation of type " +
        std::to_string(relationType1));
  DEBUG_END("LinearOSNS::computeInteractionBlock(const InteractionsGraph::EDescriptor& ed)\n");
}

void LinearOSNS::computeqBlock(InteractionsGraph::VDescriptor& vertex_inter,
                               unsigned int pos) {
...
void LinearOSNS::computeM() {
  ...
  Interaction& inter = *indexSet.bundle(*vs);
  NonSmoothLaw& nslaw = *inter.nonSmoothLaw();
  if (Type::value(nslaw) == Type::FremondImpactFrictionNSL) {
    SP::DynamicalSystem ds1 = indexSet.properties(*vs).source;
