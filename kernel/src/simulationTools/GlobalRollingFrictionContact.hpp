...
/**
   ...
   contact.
    ...
    - \f$ tildeGlobalVelocities \in R^{m} \f$ is the modified local velocity (\f$ e U_{N,k}
   \f$)
    - \f$ M \in R^{n \times n } \f$  and \f$ q \in R^{n} \f$
    - \f$ H \in R^{n \times m } \f$
   ...
   the simulation and \n the interactionBlock-matrices saved in the field interactionBlocks.\n
    ...
    - solving of the GlobalRollingFrictionContact problem: function compute(), used to call
   solvers from Numerics through \n the frictionContact2D_driver() or
   frictionContact3D_driver() interface of Numerics.
...
class GlobalRollingFrictionContact : public GlobalFrictionContact {
 ...
 public:
   ...
   */
  inline int getGlobalRollingFrictionContactDim() const { return _contactProblemDim; }
   ...
   */
  inline std::shared_ptr<std::vector<double>> mur() const { return _mu_r; }
   ...
   */
  inline double getMur(unsigned int i) const { return (*_mu_r)[i]; }
