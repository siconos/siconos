  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    if((dsType == Type::LagrangianDS) || (dsType == Type::LagrangianLinearTIDS))
    {
      ...
      if(d->inverseMass())
      {
        ...
        d->inverseMass()->PLUSolve(accFree);
      }
    ...
    else if(dsType == Type::NewtonEulerDS)
    {
      ...
      {
        d->update_inverse_mass();
        d->inverseMass()->PLUSolve(accFree); // contains left (right limit) acceleration without contact force
      }
      *work_tdg = accFree; // store the value in WorkFreeFree
  ...
  else
  {
    ...
    for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
    {
      ...
      if((dsType == Type::LagrangianDS) || (dsType == Type::LagrangianLinearTIDS))
      {
        ...
        {
          d->update_inverse_mass();
          d->inverseMass()->PLUSolve(accFree);// contains right (left limit) acceleration without contact force
        }
...
void D1MinusLinearOSI::computeFreeOutputHalfExplicitAccelerationLevel(InteractionsGraph::VDescriptor& vertex_inter, OneStepNSProblem* osnsp)
{
  ...
  if(relationType == Lagrangian)
  {
    ...
    if(((*allOSNS)[SICONOS_OSNSP_TS_VELOCITY]).get() == osnsp)
    {
      ...
      //   subprod(*ID, *(std::static_pointer_cast<LagrangianRheonomousR>(inter->relation())->hDot()), osnsp_rhs, xcoord, false);
      // }
    ...
    if(((*allOSNS)[SICONOS_OSNSP_TS_VELOCITY + 1]).get() == osnsp)
    {
      if(relationSubType == ScleronomousR)  // acceleration term involving Hessian matrix of Relation with respect to degree is added
      {
        ...
        subprod(*ID, *(std::static_pointer_cast<LagrangianScleronomousR>(inter->relation())->dotjacqhXqdot()), osnsp_rhs, xcoord, false);
      }
      DEBUG_EXPR(osnsp_rhs.display(););
  ...
  else if(relationType == NewtonEuler)
  {
    ...
    if(((*allOSNS)[SICONOS_OSNSP_TS_VELOCITY + 1]).get() == osnsp)
    {
      ...
      subprod(*ID, *(std::static_pointer_cast<NewtonEulerR>(inter->relation())->secondOrderTimeDerivativeTerms()), osnsp_rhs, xcoord, false);
      DEBUG_EXPR(osnsp_rhs.display(););
