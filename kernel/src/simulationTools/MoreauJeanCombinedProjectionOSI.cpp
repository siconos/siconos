...
siconos::integrators::MoreauJeanCombinedProjectionOSI::MoreauJeanCombinedProjectionOSI(double theta)
    : MoreauJeanOSI(theta)
{
  _levelMinForInput = 0;
...
{
  DEBUG_BEGIN(
      "siconos::integrators::MoreauJeanCombinedProjectionOSI::initializeWorkVectorsForDS( double t, "
  ...
  if (auto d = std::dynamic_pointer_cast<siconos::modeling::LagrangianDS>(ds)) {
    ...
    THROW_EXCEPTION(
        "siconos::integrators::MoreauJeanCombinedProjectionOSI::initialize() - DS not of the right type");
  }
...
void siconos::integrators::MoreauJeanCombinedProjectionOSI::initializeWorkVectorsForInteraction(
    siconos::modeling::Interaction& inter, siconos::graphs::InteractionProperties& interProp,
    siconos::graphs::DynamicalSystemsGraph& DSG)
{
  DEBUG_BEGIN(
      "siconos::integrators::MoreauJeanCombinedProjectionOSI::initializeWorkVectorsForInteraction(siconos::modeling:"
    ...
    std::shared_ptr<siconos::modeling::Interaction> inter, unsigned int i)
{
  ...
  if (i == 1)  // index set for resolution at the velocity
  {
    ...
    DEBUG_PRINTF("siconos::integrators::MoreauJeanCombinedProjectionOSI::addInteractionInIndexSet yref=%e \n", y);
    DEBUG_EXPR(if (y <= 0) printf("siconos::integrators::MoreauJeanCombinedProjectionOSI::addInteractionInIndexSet "
  ...
  else if (i == 2)  //  special index for the projection
  {
    DEBUG_EXPR(
        double lambda = 0;
        auto = (inter->lambda(1))
                   ->getValue(0);  // lambda(1) is the contact impulse for MoreauJeanOSI scheme
        printf("siconos::integrators::MoreauJeanCombinedProjectionOSI::addInteractionInIndexSet lambdaref=%e \n",
               lambda);
        if (lambda > 0) printf("siconos::integrators::MoreauJeanCombinedProjectionOSI::addInteractionInIndexSet "
                               "ACTIVATE in indexSet level = %i.\n",
                               i);)
    //    return (lambda > 0);
    return true;
