...
void siconos::integrators::MoreauJeanOSI::_NSLEffectOnFreeOutput::visit(
  ...
  // *(*_interProp.workVectors)[tools::enum_to_index(wk_inter::osnsp_rhs_position)];

  // // Get the cohesion force from the internal variables
  // double* r_cohesion = nslaw.r_cohesion(_inter);
  // if (r_cohesion) {
  //   // Copy cohesion force to osnsp_rhs_position
  //   for (unsigned int i = 0; i < _inter.nonSmoothLaw()->size(); ++i) {
  //     osnsp_rhs_position(i) = _h * r_cohesion[i];
  //   }
  // }
}
...
void siconos::integrators::MoreauJeanOSI::initializeWorkVectorsForInteraction(
  ...
  if (cohesive_nslaw) {
    if (!inter_work[tools::enum_to_index(wk_inter::osnsp_rhs_position)]) {
      inter_work[tools::enum_to_index(wk_inter::osnsp_rhs_position)] =
          std::make_shared<siconos::algebra::SiconosVector>(inter.dimension());
      inter_work[tools::enum_to_index(wk_inter::osnsp_rhs_position)]->setZero();
    }
...
void siconos::integrators::MoreauJeanOSI::initialize_nonsmooth_problems() {
  ...
  auto allOSNS = sim->oneStepNSProblems();
  ...
  //  ((*allOSNS)[SICONOS_OSNSP_TS_VELOCITY])->initialize(simulation());
}
...
void siconos::integrators::MoreauJeanOSI::initializeIterationMatrix(
  ...
  // General Lagrangian DS
    ...
    siconos::integrators::moreau_jean::computeIterationMatrix_Lagrangian_T(
        time, time_step, _theta, *lds, *iterationMat,
        _dynamicalSystemsGraph->properties(dsv).LUW);
    ...
    siconos::integrators::moreau_jean::computeIterationMatrix_Lagrangian_T(
        time, time_step, _theta, *lsds, *iterationMat,
        _dynamicalSystemsGraph->properties(dsv).LUW);
    ...
    siconos::integrators::moreau_jean::computeIterationMatrix_NewtonEuler(
        time, time_step, _theta, *neds, *iterationMat,
        _dynamicalSystemsGraph->properties(dsv).LUW);
...
void siconos::integrators::MoreauJeanOSI::computeFreeState() {
  ...
  auto sim = simulation();
  double nextTime = sim->nextTime();  // End of the time step
...
void siconos::integrators::MoreauJeanOSI::prepareNewtonIteration(double time) {
  ...
  siconos::graphs::DynamicalSystemsGraph::VIterator dsi, dsend;
  auto sim = simulation();
  auto time_step = sim->timeStep();
  for (std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi) {
    if (!checkOSI(dsi)) continue;
    auto ds = _dynamicalSystemsGraph->bundle(*dsi);
    if (auto lds = std::dynamic_pointer_cast<siconos::modeling::LagrangianDS>(ds)) {
      moreau_jean::computeIterationMatrix_Lagrangian_T(
          time, time_step, _theta, *lds,
...
void siconos::integrators::MoreauJeanOSI::computeFreeOutputPosition(
    siconos::graphs::InteractionsGraph::VDescriptor& vertex_inter,
    siconos::nonsmooth_formulations::OneStepNSProblem* osnsp) {
  /** \warning: ensures that it can also work with two different osi for two
   * different ds ?
  ...
  auto& osnsp_rhs_position =
      *(*indexSet.properties(vertex_inter)
             .workVectors)[tools::enum_to_index(wk_inter::osnsp_rhs_position)];
  ...
  if (relationType == siconos::modeling::RelationType::Lagrangian) {
    auto relation =
        std::dynamic_pointer_cast<siconos::modeling::LagrangianR>(inter.relation());
    auto H = relation->jacobianhOver_q();
...
void siconos::integrators::MoreauJeanOSI::updateState(const unsigned int) {
  ...
  for (std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi) {
    ...
    moreau_jean::updateVelocity(time_step, _theta, ds, v_iter);
    moreau_jean::updatePosition(time_step, _theta, ds);
  }
}
...
void siconos::integrators::MoreauJeanOSI::computeIteration() {
  ...
  for (std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi) {
    ...
    if (dsType == siconos::modeling::Type::LagrangianDS ||
      ...
      auto baux = useRCC && sim->relativeConvergenceCriterionHeld();

      if (baux) local_buffer = d.q_read();

      // moreau_jean::updatePosition(sim->timeStep(), _theta, ds);
...
bool siconos::integrators::MoreauJeanOSI::addInteractionInIndexSet(
  ...
  auto relationSubType = inter->relation()->getSubType();
  if (relationSubType ==
      modeling::RelationSubType::StressLinearTIR) {  // Special case of stress interaction
    return true;
  } else  // other interaction (Contact...)
  {
...
siconos::algebra::SiconosDenseMatrix siconos::integrators::MoreauJeanOSI::computeWorkForces() {
  ...
  auto number_of_ds = sim->nonSmoothDynamicalSystem()->getNumberOfDS();
...
void siconos::integrators::MoreauJeanOSI::updateInteractionInternalState() {
  ...
  siconos::graphs::InteractionsGraph& indexSet0 =
      *simulation()->indexSet(0); /* we work all the nslaw for indexSet0 */
  ...
  if (_hasInputInIndexSet0) {
    ...
    for (auto [ui, uiend] = indexSet0.vertices(); ui != uiend; ++ui) {
      ...
      if (cohesive_nslaw) {
        ...
        auto lambda_1_save =
            std::make_shared<siconos::algebra::SiconosVector>(*inter.lambda(level));
