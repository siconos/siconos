...
const SimpleMatrix MoreauJeanOSI::getW(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  assert(ds &&
...
std::shared_ptr<siconos::algebra::SimpleMatrix> MoreauJeanOSI::W(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
const SimpleMatrix MoreauJeanOSI::getWBoundaryConditions(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  assert(ds &&
...
SP::SiconosMatrix MoreauJeanOSI::WBoundaryConditions(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
void MoreauJeanOSI::initializeWorkVectorsForDS(double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  DEBUG_BEGIN("MoreauJeanOSI::initializeWorkVectorsForDS(Model&, double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)\n");
  ...
  DEBUG_END("MoreauJeanOSI::initializeWorkVectorsForDS(Model&, double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)\n");
...
void MoreauJeanOSI::_initializeIterationMatrixWBoundaryConditions(SecondOrderDS& ds, const DynamicalSystemsGraph::VDescriptor& dsv)
{
  ...
  DEBUG_BEGIN("MoreauJeanOSI::initializeIterationMatrixWBoundaryConditions(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)\n");
  ...
  DEBUG_END("MoreauJeanOSI::initializeIterationMatrixWBoundaryConditions(std::shared_ptr<siconos::modeling::DynamicalSystem> ds) \n");
}
    ...
    SiconosMatrix& iteration_matrix)
{
  ...
  if(dsType == Type::LagrangianLinearTIDS || dsType == Type::LagrangianDS ||  dsType == Type::NewtonEulerDS || dsType == Type::LagrangianLinearDiagonalDS)
  {
    ...
    std::shared_ptr<siconos::algebra::SiconosVector> columntmp(new SiconosVector(ds.dimension(), WBoundaryConditions.num()));
...
void MoreauJeanOSI::computeW(double t, SecondOrderDS& ds, SiconosMatrix& W)
{
  ...
  else if(dsType == Type::NewtonEulerDS)
  {
    ...
    if(d.jacobianqForces())
    {
      ...
      std::shared_ptr<siconos::algebra::SimpleMatrix>  buffer(new SimpleMatrix(*(d.mass())));
...
std::shared_ptr<siconos::algebra::SimpleMatrix> MoreauJeanOSI::Winverse(SP::SecondOrderDS ds, bool keepW)
{
  ...
  std::shared_ptr<siconos::algebra::SimpleMatrix> W = _dynamicalSystemsGraph->properties(dsv).W;
  std::shared_ptr<siconos::algebra::SimpleMatrix> Winverse = _dynamicalSystemsGraph->properties(dsv).Winverse;
  if (!Winverse)
  {
    unsigned int sizeW = ds->dimension();
    ...
    const SiconosVector & v)
{
  ...
  if(d.boundaryConditions())
  {
    ...
    std::shared_ptr<siconos::algebra::SiconosVector> columntmp(new SiconosVector(d.dimension()));
...
double MoreauJeanOSI::computeResidu()
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    if(dsType == Type::LagrangianDS)
    {
      ...
      {
        d.computeMass(d.q());
        siconos::algebra::prod(*(d.mass()), residuFree, residuFree); // residuFree = M(v - vold)
      }
      ...
      if(d.forces())
      {
        ...
        //std::shared_ptr<siconos::algebra::SiconosVector> qbasedonv(new SiconosVector(*qold));
    ...
    else if(dsType == Type::LagrangianLinearTIDS)
    {
      ...
      {
        coeff = h * h * _theta;
        siconos::algebra::prod(coeff, *d.K(), vold, residuFree, false); // vfree += h^2*_theta*K*vi
      ...
      //       std::shared_ptr<siconos::algebra::SiconosVector> realresiduFree (new SiconosVector(residuFree));
      //       realresiduFree->zero();
      //       siconos::algebra::prod(*M, (*v-*vold), *realresiduFree); // residuFree = M(v - vold)
      //       std::shared_ptr<siconos::algebra::SiconosVector> qkplustheta (new SiconosVector(*qold));
      //       qkplustheta->zero();
    ...
    else if(dsType == Type::NewtonEulerDS)
    {
      ...
      siconos::algebra::prod(massMatrix, (v - vold), residuFree, true); // residuFree = M(v - vold)
      DEBUG_EXPR(residuFree.display(););
      ...
      if(d.forces())   // if fL exists
      {
        ...
        //std::shared_ptr<siconos::algebra::SiconosVector> qold = d.qMemory()->getSiconosVector(0);
        //std::shared_ptr<siconos::algebra::SiconosVector> vold = d.twistMemory()->getSiconosVector(0);
        // d.computeForces(told,qold,vold);
...
void MoreauJeanOSI::computeFreeOutput(InteractionsGraph::VDescriptor& vertex_inter, OneStepNSProblem* osnsp)
{
  ...
  std::shared_ptr<siconos::algebra::BlockVector> Xfree = inter_work_block[MoreauJeanOSI::xfree];
  assert(Xfree);
     ...
     (relationSubType != ScleronomousR))
  {
    ...
    if(relationSubType == RheonomousR)
    {
      ...
      if(((*allOSNS)[SICONOS_OSNSP_TS_VELOCITY]).get() == osnsp)
      {
        ...
        siconos::algebra::subprod(*ID, *(std::static_pointer_cast<LagrangianRheonomousR>(inter.relation())->hDot()), osnsp_rhs, _selected_coordinates, false); // y += hDot
      }
      else
    ...
    if(relationSubType == CompliantLinearTIR)
    {
      if(((*allOSNS)[SICONOS_OSNSP_TS_VELOCITY]).get() == osnsp)
      {
        ...
        // + C q_k
        siconos::algebra::subprod(C, *DSlink[LagrangianR::q0], osnsp_rhs, _selected_coordinates, false);
        // + h(1-_theta)v_k

        *DSlink[LagrangianR::q1] *= (1-_theta)* h ;
        siconos::algebra::subprod(C, *DSlink[LagrangianR::q1], osnsp_rhs, _selected_coordinates, false);
...
void MoreauJeanOSI::integrate(double& tinit, double& tend, double& tout, int& notUsed)
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    if(dsType == Type::LagrangianLinearTIDS)
    {
      ...
      SP::SiconosMatrix C = d->C();
      if(C)
        siconos::algebra::prod(-h, *C, vold, v, false); // v += -h*C*vi
...
{
  DEBUG_BEGIN("MoreauJeanOSI::updatePosition(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)\n");
  ...
  else if(dsType == Type::NewtonEulerDS)
  {
    ...
    std::shared_ptr<siconos::algebra::SiconosVector> velocityIncrement(new SiconosVector(v.size()));
    double coeff = h * _theta;
  ...
  DEBUG_END("MoreauJeanOSI::updatePosition(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)\n");
...
void MoreauJeanOSI::updateState(const unsigned int)
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    if(dsType == Type::LagrangianDS || dsType == Type::LagrangianLinearTIDS || dsType == Type::LagrangianLinearDiagonalDS)
    {
      ...
      {
        int bc = 0;
        std::shared_ptr<siconos::algebra::SiconosVector> columntmp(new SiconosVector(ds.dimension()));
...
bool MoreauJeanOSI::addInteractionInIndexSet(std::shared_ptr<siconos::modeling::Interaction> inter, unsigned int i)
{
  DEBUG_PRINT("addInteractionInIndexSet(std::shared_ptr<siconos::modeling::Interaction> inter, unsigned int i)\n");
...
bool MoreauJeanOSI::removeInteractionFromIndexSet(std::shared_ptr<siconos::modeling::Interaction> inter, unsigned int i)
{
