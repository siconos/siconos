...
void Simulation::associate(SP::OneStepIntegrator osi, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  _allOSI->insert(osi);
...
std::shared_ptr<InteractionsGraph> Simulation::indexSet(unsigned int i)
{
...
void Simulation::initializeOSIAssociations()
{
  ...
  std::map< SP::OneStepIntegrator, std::list<std::shared_ptr<siconos::modeling::DynamicalSystem>> >::iterator  it;
...
void Simulation::initializeNSDSChangelog()
{
   ...
   * each step. */
  std::shared_ptr<DynamicalSystemsGraph> DSG = _nsds->topology()->dSG(0);
...
void Simulation::initializeInteraction(double time, std::shared_ptr<siconos::modeling::Interaction> inter)
{
  DEBUG_BEGIN("Simulation::initializeInteraction(double time, std::shared_ptr<siconos::modeling::Interaction> inter)\n");
  // Get the interaction properties from the topology for initialization.
  std::shared_ptr<InteractionsGraph> indexSet0 = _nsds->topology()->indexSet0();
  ...
  DEBUG_END("Simulation::initializeInteraction(double time, std::shared_ptr<siconos::modeling::Interaction> inter)\n");
}
...
std::shared_ptr<siconos::algebra::SiconosVector> Simulation::y(unsigned int level, unsigned int coor)
{
  ...
  std::shared_ptr<siconos::algebra::SiconosVector> y(new SiconosVector(_nsds->topology()->indexSet0()->size()));
  int i=0;
...
std::shared_ptr<siconos::algebra::SiconosVector> Simulation::lambda(unsigned int level, unsigned int coor)
{
  ...
  std::shared_ptr<siconos::algebra::SiconosVector> lambda(new SiconosVector(_nsds->topology()->indexSet0()->size()));
  int i=0;
  for(std::tie(ui, uiend) = indexSet0->vertices(); ui != uiend; ++ui)
  {
    inter = indexSet0->bundle(*ui);
...
void Simulation::link(std::shared_ptr<siconos::modeling::Interaction> inter,
                      std::shared_ptr<siconos::modeling::DynamicalSystem> ds1,
                      std::shared_ptr<siconos::modeling::DynamicalSystem> ds2)
{
...
void Simulation::unlink(std::shared_ptr<siconos::modeling::Interaction> inter)
{
