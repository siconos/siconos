...
//    _mu.reserve(indexSet.size())
    ...
    typedef std::unordered_map<std::shared_ptr<siconos::modeling::DynamicalSystem>, SiconosMatrix*> dsMatMap;
    typedef std::unordered_map<std::shared_ptr<siconos::modeling::DynamicalSystem>, size_t> dsPosMap;
#else
    typedef std::map<std::shared_ptr<siconos::modeling::DynamicalSystem>, SiconosMatrix*> dsMatMap;
    typedef std::map<std::shared_ptr<siconos::modeling::DynamicalSystem>, size_t> dsPosMap;
#endif
    dsMatMap dsMat;
    dsPosMap absPosDS;
...
void GlobalFrictionContact::postCompute()
{
  ...
  std::shared_ptr<siconos::algebra::SiconosVector>  y, lambda;
  ...
  for(std::tie(dsi, dsend) = DSG0.vertices(); dsi != dsend; ++dsi)
  {
    ...
    if(dsType == Type::LagrangianDS || dsType == Type::LagrangianLinearTIDS || dsType == Type::LagrangianLinearDiagonalDS)
    {
      ...
      sizeDS = d.dimension();
      std::shared_ptr<siconos::algebra::SiconosVector> velocity = d.velocity();
    ...
    else if(dsType == Type::NewtonEulerDS)
    {
      ...
      std::shared_ptr<siconos::algebra::SiconosVector> twist = neds.twist();
      DEBUG_PRINTF("ds.number() : %i \n",ds.number());
