...
BlockCSRMatrix::BlockCSRMatrix(InteractionsGraph& indexSet):
  ...
  colPos(new IndexInt(_nr))
{
  DEBUG_BEGIN("BlockCSRMatrix::BlockCSRMatrix(std::shared_ptr<InteractionsGraph> indexSet)\n");
  fill(indexSet);
  DEBUG_END("BlockCSRMatrix::BlockCSRMatrix(std::shared_ptr<InteractionsGraph> indexSet)\n");
}
...
void BlockCSRMatrix::fillH(InteractionsGraph& indexSet)
{
      ...
      vi != viend; ++vi)
  {
    ...
    std::shared_ptr<siconos::modeling::DynamicalSystem> first = std::shared_ptr<siconos::modeling::DynamicalSystem>();
