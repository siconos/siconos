  ...
  if (_assemblyType == REDUCED_BLOCK or  _assemblyType == REDUCED_DIRECT)
  {
    if(! _M)
    {
      ...
      case NM_SPARSE_BLOCK:
      {
        ...
        if(indexSetLevel() != simulation::internal::LEVELMAX && simulation()->nonSmoothDynamicalSystem()->topology()->indexSetsSize() > indexSetLevel())
        {
...
void LinearOSNS::computeInteractionBlock(const InteractionsGraph::EDescriptor& ed)
{
          ...
          relationType2 == NewtonEuler)
  {
    ...
    if(bc)
    {
      ...
      {
        // (nslawSize,sizeDS));
        std::shared_ptr<siconos::algebra::SiconosVector> coltmp(new SiconosVector(nslawSize1));
        coltmp->zero();
        leftInteractionBlock->setCol(*itindex, *coltmp);
        }
    }
    ...
    if(osiType == OSI::MOREAUJEANBILBAOOSI || dsType == Type::LagrangianLinearDiagonalDS)
    {
      ...
      siconos::algebra::axpy_prod(*leftInteractionBlock, inv_iteration_matrix, *rightInteractionBlock, true);
      ...
      siconos::algebra::prod(*rightInteractionBlock, *leftInteractionBlock, *currentInteractionBlock, false);
    }
    else
    {
      // inter1 != inter2
      ...
      std::shared_ptr<siconos::algebra::SimpleMatrix> centralInteractionBlock = getOSIMatrix(osi, ds);
      centralInteractionBlock->Solve(*rightInteractionBlock);
      //*currentInteractionBlock +=  *leftInteractionBlock ** work;
      siconos::algebra::prod(*leftInteractionBlock, *rightInteractionBlock, *currentInteractionBlock, false);
    }
...
{
  DEBUG_BEGIN("LinearOSNS::computeqBlock(std::shared_ptr<siconos::modeling::Interaction> inter, unsigned int pos)\n");
  std::shared_ptr<InteractionsGraph> indexSet = simulation()->indexSet(indexSetLevel());
  ...
  DEBUG_END("LinearOSNS::computeqBlock(std::shared_ptr<siconos::modeling::Interaction> inter, unsigned int pos)\n");
}
