...
void OneStepNSProblem::displayBlocks(std::shared_ptr<InteractionsGraph> indexSet)
{

  std::cout <<  "OneStepNSProblem::displayBlocks(std::shared_ptr<InteractionsGraph> indexSet) " << std::endl;
  InteractionsGraph::VIterator vi, viend;
  for(std::tie(vi, viend) = indexSet->vertices();
      vi != viend; ++vi)
  {
...
std::shared_ptr<siconos::algebra::SimpleMatrix> OneStepNSProblem::getOSIMatrix(OneStepIntegrator& Osi, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  ...
  else if(osiType == OSI::D1MINUSLINEAROSI)
  {
    ...
    if(dsType == Type::LagrangianDS || dsType == Type::LagrangianLinearTIDS)
    {
      // std::shared_ptr<siconos::algebra::SimpleMatrix> Mold;
