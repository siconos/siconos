...
void EventDriven::updateIndexSet(unsigned int i)
{
  ...
  {
    std::shared_ptr<siconos::modeling::Interaction> inter = _indexSet0->bundle(*uip);
    if(i == 1)  // IndexSet[1]
    {
...
void EventDriven::initOSIRhs()
{
  ...
  for(OSIIterator itosi = _allOSI->begin();  itosi != _allOSI->end(); ++itosi)
  {
    ...
    std::shared_ptr<DynamicalSystemsGraph> osiDSGraph = (*itosi)->dynamicalSystemsGraph();
    for(std::tie(dsi, dsend) = osiDSGraph->vertices(); dsi != dsend; ++dsi)
    {
      if(!(*itosi)->checkOSI(dsi)) continue;

      std::shared_ptr<siconos::modeling::DynamicalSystem> ds = osiDSGraph->bundle(*dsi);
...
void EventDriven::computef(OneStepIntegrator& osi, integer * sizeOfX, doublereal * time, doublereal * x, doublereal * xdot)
{
  ...
  DynamicalSystemsGraph::VIterator dsi, dsend;
  std::shared_ptr<DynamicalSystemsGraph> osiDSGraph = lsodar.dynamicalSystemsGraph();
  for(std::tie(dsi, dsend) = osiDSGraph->vertices(); dsi != dsend; ++dsi)
  {
                           ...
                           doublereal * gOut)
{
  ...
  for(std::tie(ui, uiend) = _indexSet0->vertices(); ui != uiend; ++ui)
  {
    std::shared_ptr<siconos::modeling::Interaction> inter = _indexSet0->bundle(*ui);
