...
namespace SA {
using doublereal = boost::shared_array<doublereal>;
using integer = boost::shared_array<doublereal>;
}  // namespace SA
...
class OneStepIntegrator : public std::enable_shared_from_this<OneStepIntegrator> {
 protected:
   ...
   */
  std::shared_ptr<DynamicalSystemsGraph> _dynamicalSystemsGraph;
  ...
  */
   ...
   */
  SP::VectorOfVectors _initializeDSWorkVectors(std::shared_ptr<siconos::modeling::DynamicalSystem> ds);
 ...
 public:
   ...
   */
  inline OSI::TYPES getType() const { return _integratorType; }
  ...
  */
  inline std::shared_ptr<DynamicalSystemsGraph> dynamicalSystemsGraph() const
  {
  ...
  */
  inline void setDynamicalSystemsGraph(std::shared_ptr<DynamicalSystemsGraph> dsg)
  {
   ...
   */
  inline SP::Simulation simulation() const { return _simulation; }

  /** set the Simulation of the OneStepIntegrator
   *
   *  \param newS a pointer to Simulation
   */
  inline void setSimulationPtr(SP::Simulation newS) { _simulation = newS; }
  ...
  */
   ...
   *  \param ds the dynamical system
   */
  virtual void initializeWorkVectorsForDS(double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds) = 0;
  ...
  */
  void update_interaction_output(Interaction& inter, double time,
                                 InteractionProperties& interaction_properties);
  ...
  virtual double computeResidu()
  {
    // default : error
    THROW_EXCEPTION(
        "OneStepIntegrator::computeResidu not implemented for integrator of type " +
        std::to_string(_integratorType));
    return 0.0;
  }
  ...
  */
   ...
   */
  virtual double computeResiduOutput(double time, std::shared_ptr<InteractionsGraph> indexSet);
   ...
   *  \param indexSet the index set of the interaction that are concerned
   */
  virtual double computeResiduInput(double time, std::shared_ptr<InteractionsGraph> indexSet);
   ...
   *  \return bool
   */
  virtual bool addInteractionInIndexSet(std::shared_ptr<siconos::modeling::Interaction> inter, unsigned int i)
  {
    THROW_EXCEPTION(
   ...
   *  \return bool
   */
  virtual bool removeInteractionFromIndexSet(std::shared_ptr<siconos::modeling::Interaction> inter, unsigned int i)
  {
    THROW_EXCEPTION(
   ...
   */
  inline SP::ExtraAdditionalTerms extraAdditionalTerms() { return _extraAdditionalTerms; }
  ...
  /**
      ...
      \param dsgv the descriptor of the node in the graph corresponding to the dynamical system
     of interest.
   */
