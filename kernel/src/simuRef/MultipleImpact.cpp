...
void MultipleImpact::Compute_velocityContact()
{
  ...
  (*_velocityContact) = (*_velocityContact) + siconos::algebra::prod(*(_M->defaultMatrix()), *_deltaImpulseContact); // compute the relative velocity at the end of the step
  //
  /*
...
void MultipleImpact::display() const
{
  ...
  {
    std::shared_ptr<siconos::modeling::Interaction> inter = IndexSet0->bundle(*vi);
