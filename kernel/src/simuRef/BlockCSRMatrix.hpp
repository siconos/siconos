...
/**
   ...
   map<std::shared_ptr<siconos::modeling::Interaction> u1, <std::shared_ptr<siconos::modeling::Interaction> u2, SP::SiconosMatrix block> >,
   block being the link between u1 and u2. Only Interaction present in the index
   set are picked out in the map.
...
public:
   ...
   */
  inline SP::CompressedRowMat getMSparse() { return _blockCSR; };
