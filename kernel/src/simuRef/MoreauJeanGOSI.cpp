...
void MoreauJeanGOSI::initializeWorkVectorsForDS(double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
double MoreauJeanGOSI::computeResidu()
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    if(dsType == Type::LagrangianDS)
    {
      ...
      std::shared_ptr<siconos::algebra::SiconosVector> v = d->velocity(); // v = v_k,i+1
      //residu.zero();
    ...
    else if(dsType == Type::LagrangianLinearTIDS)
    {
      ...
      if(C)
        siconos::algebra::prod(h, *C, vold, free_rhs, false); // free_rhs += h*C*vi

      SP::SiconosMatrix K = d->K();
      if(K)
      {
        coeff = -h * h * _theta;
        siconos::algebra::prod(coeff, *K, vold, residu, false); // free_rhs += -h^2*_theta*K*vi
    ...
    else if(dsType == Type::NewtonEulerDS)
    {
      ...
      std::shared_ptr<siconos::algebra::SiconosVector> q = d->q();
      std::shared_ptr<siconos::algebra::SiconosVector> v = d->twist(); // v = v_k,i+1
      ...
      // siconos::algebra::prod(*massMatrix, (*v - vold), residu, true); // residu = M(v - vold)
      // DEBUG_EXPR(residu.display(););
      free_rhs.zero();
      SiconosMatrix& W = *_dynamicalSystemsGraph->properties(*dsi).W;
      siconos::algebra::prod(W, vold, free_rhs);
...
void MoreauJeanGOSI::NonSmoothLawContributionToOutput(std::shared_ptr<siconos::modeling::Interaction> inter, OneStepNSProblem& osnsp)
{
