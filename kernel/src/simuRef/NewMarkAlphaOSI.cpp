...
const SimpleMatrix NewMarkAlphaOSI::getW(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
std::shared_ptr<siconos::algebra::SimpleMatrix> NewMarkAlphaOSI::W(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
void NewMarkAlphaOSI::initializeIterationMatrixW(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
void NewMarkAlphaOSI::computeW(std::shared_ptr<siconos::modeling::DynamicalSystem> ds, SiconosMatrix& W)
{
...
double NewMarkAlphaOSI::computeResidu()
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    if((dsType == Type::LagrangianDS) || (dsType == Type::LagrangianLinearTIDS))
    {
      ...
      SiconosVector& residuFree = *workVectors[NewMarkAlphaOSI::RESIDU_FREE];
      ...
      std::shared_ptr<siconos::algebra::SiconosVector> a = d->acceleration();
      std::shared_ptr<siconos::algebra::SiconosVector> F;
      // get the reaction force p
      std::shared_ptr<siconos::algebra::SiconosVector> p = d->p(2);
      ...
      if(dsType == Type::LagrangianLinearTIDS)
      {
        ...
        SP::LagrangianLinearTIDS dtids = std::static_pointer_cast<LagrangianLinearTIDS>(ds);
        ...
        if(K)
          siconos::algebra::prod(*K, *q, residuFree, false); // f = M*a + C*v + K*q
        if(C)
          siconos::algebra::prod(*C, *v, residuFree, false); // freeR = M*a + C*v
      }
...
void NewMarkAlphaOSI::computeFreeState()
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    SiconosVector& residuFree = *workVectors[NewMarkAlphaOSI::RESIDU_FREE];
    SiconosVector& qfree = *workVectors[NewMarkAlphaOSI::FREE];
...
void NewMarkAlphaOSI::computeFreeOutput(InteractionsGraph::VDescriptor& vertex_inter, OneStepNSProblem* osnsp)
{
  ...
  std::shared_ptr<InteractionsGraph> indexSet = osnsp->simulation()->indexSet(osnsp->indexSetLevel());
  ...
  if(relationType == Lagrangian)
  {
    ...
    if(relationSubType == ScleronomousR)
    {
      ...
      if(((*allOSNS)[SICONOS_OSNSP_ED_SMOOTH_ACC]).get() == osnsp)  // LCP at acceleration level
      {
        ...
        xcoord[6] = 0;
        xcoord[7] = sizeY;
        SP::LagrangianScleronomousR _SclerR = std::static_pointer_cast<LagrangianScleronomousR>(inter->relation());
        _SclerR->computedotjacqhXqdot(t, *inter);
        siconos::algebra::subprod(*ID, *(_SclerR->dotjacqhXqdot()), osnsp_rhs, xcoord, false); // y += NonLinearPart
      }
...
void NewMarkAlphaOSI::initializeWorkVectorsForDS(double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  DEBUG_BEGIN("NewMarkAlphaOSI::initializeWorkVectorsForDS( double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)\n")
  ...
  if((dsType == Type::LagrangianDS) || (dsType == Type::LagrangianLinearTIDS))
  {
    ...
    ds_work_vectors.resize(NewMarkAlphaOSI::WORK_LENGTH);
    ds_work_vectors[NewMarkAlphaOSI::RESIDU_FREE].reset(new SiconosVector(d->dimension()));
    //ds_work_vectors[NewMarkAlphaOSI::FREE].reset(new SiconosVector(d->dimension()));
    ds_work_vectors[NewMarkAlphaOSI::FREE].reset(new SiconosVector(*(d->acceleration())));
    ds_work_vectors[NewMarkAlphaOSI::ACCE_LIKE].reset(new SiconosVector(*(d->acceleration()))); // set a0 = ddotq0
    ds_work_vectors[NewMarkAlphaOSI::ACCE_MEMORY].reset(new SiconosVector(*(d->acceleration()))); // set a0 = ddotq0

    // Allocate the memory to stock coefficients of the polynomial for the dense output
    workMatrices.resize(NewMarkAlphaOSI::MAT_WORK_LENGTH);
    workMatrices[NewMarkAlphaOSI::DENSE_OUTPUT_COEFFICIENTS].reset(new SimpleMatrix(ds->dimension(), (getOrderDenseOutput() + 1)));
    ...
    //          d->allocateWorkMatrix(NewMarkAlphaOSI::DENSE_OUTPUT_COEFFICIENTS, ds->dimension(), (osi_NewMark->getOrderDenseOutput() + 1));
  ...
  DEBUG_END("NewMarkAlphaOSI::initializeWorkVectorsForDS( double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)\n")
    ...
    DynamicalSystemsGraph & DSG)
{
  ...
  {
    interProp.workMatrices = std::make_shared<std::vector<std::shared_ptr<siconos::algebra::SimpleMatrix>>>();
  ...
  inter_work[NewMarkAlphaOSI::OSNSP_RHS].reset(new SiconosVector(inter.dimension()));
...
void NewMarkAlphaOSI::prediction()
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    if((dsType == Type::LagrangianDS) || (dsType == Type::LagrangianLinearTIDS))
    {
      ...
      SiconosVector& acce_like = *ds_work_vectors[NewMarkAlphaOSI::ACCE_LIKE];
...
void NewMarkAlphaOSI::correction()
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    std::shared_ptr<siconos::algebra::SimpleMatrix> W = _dynamicalSystemsGraph->properties(*dsi).W; // Its W matrix of iteration.
    ; // Iteration matrix W_{n+1,k} computed at kth iteration
    SiconosVector& residuFree = *ds_work_vectors[NewMarkAlphaOSI::RESIDU_FREE];
    SiconosVector& acce_like = *ds_work_vectors[NewMarkAlphaOSI::ACCE_LIKE];
    ...
    {
      SP::LagrangianDS d = std::static_pointer_cast<LagrangianDS>(ds);
      std::shared_ptr<siconos::algebra::SiconosVector> _p = d->p(2); // resultant force p_{n+1,k+1} of DS at (k+1)th iteration
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void NewMarkAlphaOSI::computeCoefsDenseOutput(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  double h = _simulation->nextTime() - _simulation->startingTime();
  Type::Siconos dsType = Type::value(*ds);    // Type of the current DS
  std::shared_ptr<siconos::algebra::SiconosVector> _vec(new SiconosVector(ds->dimension()));
  auto& workMatrices = *_dynamicalSystemsGraph->properties(_dynamicalSystemsGraph->descriptor(ds)).workMatrices;
  auto& ds_work_vectors = *_dynamicalSystemsGraph->properties(_dynamicalSystemsGraph->descriptor(ds)).workVectors;
  if((dsType == Type::LagrangianDS) || (dsType == Type::LagrangianLinearTIDS))
  {
    SP::LagrangianDS d = std::static_pointer_cast<LagrangianDS>(ds);
...
void NewMarkAlphaOSI::DenseOutputallDSs(double t)
{
  ...
  std::shared_ptr<siconos::algebra::SiconosVector> _vec1(new SiconosVector(_orderDenseOutput + 1));
  ...
  //
  std::shared_ptr<siconos::algebra::SiconosVector> _vec2(new SiconosVector(_orderDenseOutput + 1));
  ...
  //
  std::shared_ptr<siconos::algebra::SiconosVector> _vec3(new SiconosVector(_orderDenseOutput + 1));
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    {
      SP::LagrangianDS d = std::static_pointer_cast<LagrangianDS>(ds);
      SiconosMatrix &coeffsDense = *workMatrices[NewMarkAlphaOSI::DENSE_OUTPUT_COEFFICIENTS];
      siconos::algebra::prod(coeffsDense, *_vec1, *(d->q()), true); // q = Matrix_coeffs*_vec1
      siconos::algebra::prod(coeffsDense, *_vec2, *(d->velocity()), true); // dotq = Matrix_coeffs*_vec2
      siconos::algebra::prod(coeffsDense, *_vec3, *(d->acceleration()), true); // ddotq = Matrix_coeffs*_vec3
    }
    else
    {
