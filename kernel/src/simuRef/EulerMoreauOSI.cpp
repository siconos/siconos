...
const SimpleMatrix EulerMoreauOSI::getW(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  assert(ds &&
...
std::shared_ptr<siconos::algebra::SimpleMatrix> EulerMoreauOSI::W(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
const SimpleMatrix EulerMoreauOSI::getWBoundaryConditions(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  assert(ds &&
...
SP::SiconosMatrix EulerMoreauOSI::WBoundaryConditions(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
void EulerMoreauOSI::initializeWorkVectorsForDS(double t, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
...
void EulerMoreauOSI::initializeIterationMatrixW(double time, std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  // This function:
...
void EulerMoreauOSI::initializeIterationMatrixWBoundaryConditions(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
  // This function:
...
void EulerMoreauOSI::computeWBoundaryConditions(std::shared_ptr<siconos::modeling::DynamicalSystem> ds)
{
                                 ...
                                 std::vector<std::shared_ptr<siconos::algebra::SimpleMatrix>>& workM, double h) const
{
  ...
  if((relationType == FirstOrder) && (workM[EulerMoreauOSI::MAT_KHAT]))
  {
    ...
    siconos::algebra::prod(*K, m, *workM[EulerMoreauOSI::MAT_KHAT], true);
...
void EulerMoreauOSI::computeFreeState()
{
  ...
  for(std::tie(dsi, dsend) = _dynamicalSystemsGraph->vertices(); dsi != dsend; ++dsi)
  {
    ...
    if(dsType == Type::FirstOrderNonLinearDS || dsType == Type::FirstOrderLinearDS || dsType == Type::FirstOrderLinearTIDS)
    {
      ...
      //    std::shared_ptr<siconos::algebra::SiconosVector> xold = d->xMemory()->getSiconosVector(0); // xi
...
void EulerMoreauOSI::prepareNewtonIteration(double time)
{
  ...
  if(!_explicitJacobiansOfRelation)
  {
    ...
    std::shared_ptr<InteractionsGraph> indexSet0 = _simulation->nonSmoothDynamicalSystem()->topology()->indexSet0();
    ...
    for(std::tie(ui, uiend) = indexSet0->vertices(); ui != uiend; ++ui)
    {
      ...
      if(relationType == FirstOrder)
      {
        ...
        {
          if(relation.B())
            siconos::algebra::prod(*relation.B(), *inter.lambda(0), *inter_work[EulerMoreauOSI::VEC_X], true);
          else
            siconos::algebra::prod(*relationMat[FirstOrderR::mat_B], *inter.lambda(0), *inter_work[EulerMoreauOSI::VEC_X], true);
...
void EulerMoreauOSI::updateOutput(double time, unsigned int level)
{
  ...
  for(std::tie(ui, uiend) = indexSet0->vertices(); ui != uiend; ++ui)
  {
    ...
    if(relationSubType == Type2R)
    {
      ...
      if(r.D())
        siconos::algebra::prod(*r.D(), *inter_work[EulerMoreauOSI::LAMBDAOLD], y, true);
      else
        siconos::algebra::prod(*relationMat[FirstOrderR::mat_D], *inter_work[EulerMoreauOSI::LAMBDAOLD], y, true);
...
void EulerMoreauOSI::updateInput(double time, unsigned int level)
{
  ...
  for(std::tie(ui, uiend) = indexSet0->vertices(); ui != uiend; ++ui)
  {
    ...
    if(relationSubType == Type2R)
    {
      ...
      if(r.B())
        siconos::algebra::prod(*r.B(), lambda, *inter_work_block[EulerMoreauOSI::G_ALPHA], false);
      else
        siconos::algebra::prod(*relationMat[FirstOrderR::mat_B], lambda, *inter_work_block[EulerMoreauOSI::G_ALPHA], false);
    ...
    else if(relationSubType == NonLinearR)
    {
      ...
      // Khat = h * K * W^-1 * B
      siconos::algebra::prod(*inter_work_mat[EulerMoreauOSI::MAT_KHAT], *inter.lambda(level), g_alpha, false);
...
double EulerMoreauOSI::computeResiduOutput(double time, std::shared_ptr<InteractionsGraph> indexSet)
{
...
double EulerMoreauOSI::computeResiduInput(double time, std::shared_ptr<InteractionsGraph> indexSet)
{
  double residu =0.0;
  InteractionsGraph::VIterator ui, uiend;
  for(std::tie(ui, uiend) = indexSet->vertices(); ui != uiend; ++ui)
