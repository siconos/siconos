...
{
  DEBUG_BEGIN("void OSNSMatrix::fillM(std::shared_ptr<InteractionsGraph> indexSet, bool update)\n");
  ...
  if(update)
    convert();
  DEBUG_END("void OSNSMatrix::fillM(std::shared_ptr<InteractionsGraph> indexSet, bool update)\n");
}
...
{
  DEBUG_BEGIN("void OSNSMatrix::fillW(std::shared_ptr<DynamicalSystemsGraph> DSG, bool update)\n");
  ...
  DEBUG_END("void OSNSMatrix::fillW(std::shared_ptr<DynamicalSystemsGraph> DSG, bool update)\n");
}
...
{
  DEBUG_BEGIN("void OSNSMatrix::fillWinverse(std::shared_ptr<DynamicalSystemsGraph> DSG, bool update)\n");
...
{
  DEBUG_BEGIN("void OSNSMatrix::fillH(std::shared_ptr<DynamicalSystemsGraph> DSG, InteractionsGraph& indexSet, bool update)\n");
  ...
  DEBUG_END("void OSNSMatrix::fillH(std::shared_ptr<DynamicalSystemsGraph> DSG, InteractionsGraph& indexSet, bool update)\n");
}

// Fill the matrix Htrans
void OSNSMatrix::fillHtrans(DynamicalSystemsGraph & DSG, InteractionsGraph& indexSet, bool update)
{
  DEBUG_BEGIN("void OSNSMatrix::fillHtrans(std::shared_ptr<DynamicalSystemsGraph> DSG, InteractionsGraph& indexSet, bool update)\n");
  if(update)
  {
  ...
  case NM_SPARSE:
  {
    if(update)
    {
      ...
      for(std::tie(ui, uiend) = indexSet.vertices(); ui != uiend; ++ui)
      {
        ...
        for(std::shared_ptr<siconos::modeling::DynamicalSystem> ds = ds1; !endl; ds = ds2, posBlock = pos_ds2)
        {
          endl = (ds == ds2);
          ...
          if (sods)
          {
            ...
            if(sods->boundaryConditions())
            {
              ...
              //   }
              //     // (nslawSize,sizeDS));
              //   //std::shared_ptr<siconos::algebra::SiconosVector> coltmp(new SiconosVector(nslawSize));
              //   //coltmp->zero();
  ...
  DEBUG_END("void OSNSMatrix::fillHtrans(std::shared_ptr<DynamicalSystemsGraph> DSG, InteractionsGraph& indexSet, bool update)\n");
}
