...
void NewtonEulerR::initialize(Interaction& inter)
{
  ...
  if(! _jachqT)
    _jachqT = std::make_shared<siconos::algebra::SimpleMatrix>(ySize, xSize));
  ...
  if(!_contactForce)
  {
    _contactForce = std::make_shared<siconos::algebra::SiconosVector>(DSlink[NewtonEulerR::p1]->size()));
    _contactForce->zero();
  }
...
void NewtonEulerR::setJachq(std::shared_ptr<siconos::algebra::SimpleMatrix> newJachq)
{
  _jachq = newJachq;
}

void NewtonEulerR::setJachqPtr(std::shared_ptr<siconos::algebra::SimpleMatrix> newPtr)
{
  _jachq = newPtr ;
}
...
void NewtonEulerR::computeInput(double time, Interaction& inter, unsigned int level)
{
  ...
  if(level == 1)  /* \warning : we assume that ContactForce is given by lambda[level] */
  {
    ...
    DEBUG_EXPR(_jachqT->display(););
    DEBUG_EXPR(DSlink[NewtonEulerR::p0 + level]->display(););
    // DEBUG_EXPR(std::shared_ptr<siconos::algebra::SiconosVector> buffer = std::make_shared<siconos::algebra::SiconosVector>(DSlink[NewtonEulerR::p0 + level]->size()));
...
void NewtonEulerR::computeJachqT(Interaction& inter, SP::BlockVector q0)
{
  ...
  unsigned int k = 0;
  unsigned int ySize = inter.dimension();
  std::shared_ptr<siconos::algebra::SimpleMatrix> auxBloc = std::make_shared<siconos::algebra::SimpleMatrix>(ySize, 7));
  std::shared_ptr<siconos::algebra::SimpleMatrix> auxBloc2 = std::make_shared<siconos::algebra::SimpleMatrix>(ySize, 6));
  Index dimIndex(2);
  Index startIndex(4);

  for(unsigned int i =0 ; i < q0->numberOfBlocks()  ; i++)
  {
    std::shared_ptr<siconos::algebra::SiconosVector> q = (q0->getAllVect())[i];
    startIndex[0] = 0;
    startIndex[1] = 7 * k / 6;
    startIndex[2] = 0;
    startIndex[3] = 0;
...
void  NewtonEulerR::computeSecondOrderTimeDerivativeTerms(double time, Interaction& inter, VectorOfBlockVectors& DSlink, SP::DynamicalSystem ds1, SP::DynamicalSystem ds2)
{
  ...
  _secondOrderTimeDerivativeTerms = std::make_shared<siconos::algebra::SiconosVector>(_dotjachq->size(0)));
  ...
  std::shared_ptr<siconos::algebra::SimpleMatrix> jachqTdot = std::make_shared<siconos::algebra::SimpleMatrix>(ySize, xSize));
  bool endl = false;
