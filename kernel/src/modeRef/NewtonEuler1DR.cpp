...
*/
void NewtonEuler1DR::NIcomputeJachqTFromContacts(std::shared_ptr<siconos::algebra::SiconosVector> q1)
{
...
void NewtonEuler1DR::NIcomputeJachqTFromContacts(std::shared_ptr<siconos::algebra::SiconosVector> q1, std::shared_ptr<siconos::algebra::SiconosVector> q2)
{
  double Nx = _Nc->getValue(0);
  double Ny = _Nc->getValue(1);
  double Nz = _Nc->getValue(2);
...
void NewtonEuler1DR::initialize(Interaction& inter)
{
  ...
  //proj_with_q  _jachqProj = std::make_shared<siconos::algebra::SimpleMatrix>(_jachq->size(0),_jachq->size(1)));
