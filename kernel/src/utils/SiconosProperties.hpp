...
struct VertexAccess {
  ...
  std::pair<iterator, iterator> elements(G& g) { return g.vertices(); }

  size_t size(G& g) { return g.vertices_number(); }

  bool isElem(G& g, typename G::VDescriptor& vd) { return g.is_vertex(g.bundle(vd)); }
};
...
struct EdgeAccess {
  ...
  std::pair<iterator, iterator> elements(G& g) { return g.edges(); }
...
struct VertexOrEdge {
  typedef typename boost::mpl::if_<boost::is_same<typename G::VIndexAccess, IndexMap>,
                                   VertexAccess<G>, EdgeAccess<G> >::type Access;
};
...
struct SwapProperties {
  typedef
      typename boost::mpl::if_<IsPointer<T>, SwapPointedValues<T>, SwapValues<T> >::type type;
};
...
struct GetPointedValue {
  ...
  typename RemovePointer<T>::type& operator()(T a) { return *a; }
};
...
template <typename T>
struct GetProperty {
  typedef typename boost::mpl::if_<IsPointer<T>, GetPointedValue<T>, GetValue<T> >::type type;
};
...
class Properties {
 public:
  ...
  typedef
      typename std::iterator_traits<typename std::vector<T>::iterator>::reference reference;
  ...
  */
   ...
   */
  void insert(const key_type& v, T t) { (*_store)[v] = t; };
  ...
  /** data access from a SiconosGraph vertex descriptor or edge
    ...
    */
  reference operator[](const key_type& v) { return (*_store)[v]; };

  /** data access from a SiconosGraph vertex descriptor or edge
      descriptor
      \param v a SiconosGraph::VDescriptor or
      SiconosGraph::EDescriptor according to IndexMap type */
  value_type at(const key_type& v) { return _store->at(v); };
  ...
  /** find data associated with the given key
      ...
      \return an iterator
   */
  reference find(const key_type& v) { return _store->find(v).second; };
   ...
   */
  inline bool hasKey(const key_type& v) { return _store->find(v) != _store->end(); };
...
class VertexSPProperties : public VertexProperties<std::shared_ptr<T>, G> {
 public:
  ...
  /** data access from a SiconosGraph vertex descriptor or edge
      ...
      \return the element in the vector
    */
  inline T& getRef(const key_type& v) { return *((*this->_store)[v]).get(); };
};
...
class SubProperties {
 ...
 public:
  SubProperties(RefProperties& p, G& g) : _properties(p), _g(g) {}

  reference operator[](const key_type& v) { return _properties[_g.descriptor0(v)]; }
};
