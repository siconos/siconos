...
BlockMatrix::BlockMatrix(const SiconosMatrix& m)
...
BlockMatrix::BlockMatrix(const std::vector<SP::SiconosMatrix>& m, unsigned int row,
                         unsigned int col)
    : SiconosMatrix(siconos::BLOCK), _dimRow(0), _dimCol(0) {
  if (m.size() != (row * col))
...
BlockMatrix::BlockMatrix(SP::SiconosMatrix A, SP::SiconosMatrix B, SP::SiconosMatrix C,
                         SP::SiconosMatrix D)
    : SiconosMatrix(siconos::BLOCK), _dimRow(0), _dimCol(0) {
...
// return the boost sparse matrix of the block (i, j)
const SparseCoordinateMat BlockMatrix::getSparseCoordinate(unsigned int row,
...
std::string BlockMatrix::toString() const { return ::toString(*this); }
...
void BlockMatrix::addSimple(unsigned int& indRow, unsigned int& indCol,
    ...
    for (it2 = it1.begin(); it2 != it1.end(); ++it2) {
      if ((*it2)->isBlock())  // if the sub-block is also a BlockMatrix ...
        ...
        if (numM == siconos::DENSE)
          noalias(*(*it2)->dense()) += ublas::subrange(*m.dense(), indRow, indRow + currentRow,
                                                       indCol, indCol + currentCol);
        else if (numM == siconos::TRIANGULAR)
          noalias(*(*it2)->triang()) += ublas::subrange(
              *m.triang(), indRow, indRow + currentRow, indCol, indCol + currentCol);
        else if (numM == siconos::SYMMETRIC)
          noalias(*(*it2)->sym()) += ublas::subrange(*m.sym(), indRow, indRow + currentRow,
                                                     indCol, indCol + currentCol);
        else if (numM == siconos::SPARSE)
          noalias(*(*it2)->sparse()) += ublas::subrange(
              *m.sparse(), indRow, indRow + currentRow, indCol, indCol + currentCol);
        else if (numM == siconos::BANDED)
          noalias(*(*it2)->banded()) += ublas::subrange(
...
void BlockMatrix::subSimple(unsigned int& indRow, unsigned int& indCol,
                            const SiconosMatrix& m) {
...
void BlockMatrix::trans() { THROW_EXCEPTION("not yet implemented."); }

void BlockMatrix::trans(const SiconosMatrix& m) { THROW_EXCEPTION("not yet implemented."); }

void BlockMatrix::PLUFactorizationInPlace() {
  THROW_EXCEPTION("not yet implemented for Block Matrices.");
}
void BlockMatrix::Factorize() { THROW_EXCEPTION("not yet implemented for Block Matrices."); }
...
size_t BlockMatrix::nnz(double tol) {
  ...
  BlocksMat::iterator2 it2;
  for (it = _mat->begin1(); it != _mat->end1(); ++it) {
    for (it2 = it.begin(); it2 != it.end(); ++it2) nnz += (**it2).nnz();
  }
  return nnz;
}
