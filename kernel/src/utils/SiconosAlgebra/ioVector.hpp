...
// Note FP : ios::scientific result in file.good() always false, even if writing process goes
// well ... Don't know why. Note MB : ios::scientific -> fmtflags, std::ios:out -> openmode
/* stdc++ :
 mode
  Flags describing the requested i/o mode for the file.
  This is an object of the bitmask member type openmode that consists of a combination of the
 following member constants: member constant	stands for	access
  in *	input	File open for reading: the internal stream buffer supports input operations.
  out	output	File open for writing: the internal stream buffer supports output operations.
  binary	binary	Operations are performed in binary mode rather than text.
  ate	at end	The output position starts at the end of the file.
  app	append	All output operations happen at the end of the file, appending to its existing
 contents.
 ...
 */
bool read(const std::string &fileName, SiconosVector &m, const openmode &mode = ASCII_IN,
          int precision = 15, const std::string &inputType = "python",
...
bool write(const std::string &fileName, const SiconosVector &m,
           const openmode &mode = ASCII_OUT, int precision = 15,
