...
SimpleMatrix::SimpleMatrix() : SiconosMatrix(siconos::DENSE) {
  mat.Dense = new DenseMat(ublas::zero_matrix<double>());
}

// parameters: dimensions and type.
SimpleMatrix::SimpleMatrix(unsigned int row, unsigned int col, UBLAS_TYPE typ,
...
SimpleMatrix::SimpleMatrix(unsigned int row, unsigned int col, double inputValue,
                           UBLAS_TYPE typ, unsigned int upper, unsigned int lower)
    : SiconosMatrix(typ) {
...
// SimpleMatrix::SimpleMatrix(const std::vector<double>& v, unsigned int row, unsigned int col,
// UBLAS_TYPE typ, unsigned int lower, unsigned int upper):
...
//       || (typ == BANDED && ( (v.size()) != (unsigned int)(std::max)(row,
//       col)*(lower+1+upper) ) ))
...
//        constructor(UBLAS_TYPE, int row, int col) with UBLAS_TYPE = SPARSE");
...
// Copy constructors
SimpleMatrix::SimpleMatrix(const SimpleMatrix& m) : SiconosMatrix(m.num()) {
...
SimpleMatrix::SimpleMatrix(const SiconosMatrix& m) : SiconosMatrix(m.num()) {
  ...
  if (m.ipiv()) _ipiv.reset(new VInt(*(m.ipiv())));
...
SimpleMatrix::SimpleMatrix(const DenseMat& m) : SiconosMatrix(siconos::DENSE) {
  mat.Dense = new DenseMat(m);
}

SimpleMatrix::SimpleMatrix(const TriangMat& m) : SiconosMatrix(siconos::TRIANGULAR) {
  mat.Triang = new TriangMat(m);
}

SimpleMatrix::SimpleMatrix(const SymMat& m) : SiconosMatrix(siconos::SYMMETRIC) {
  mat.Sym = new SymMat(m);
}

SimpleMatrix::SimpleMatrix(const SparseMat& m) : SiconosMatrix(siconos::SPARSE) {
  mat.Sparse = new SparseMat(m);
}

SimpleMatrix::SimpleMatrix(const SparseCoordinateMat& m) : SiconosMatrix(SPARSE_COORDINATE) {
  mat.SparseCoordinate = new SparseCoordinateMat(m);
}

SimpleMatrix::SimpleMatrix(const BandedMat& m) : SiconosMatrix(siconos::BANDED) {
  mat.Banded = new BandedMat(m);
}

SimpleMatrix::SimpleMatrix(const ZeroMat& m) : SiconosMatrix(siconos::ZERO) {
  mat.Zero = new ZeroMat(m);
}

SimpleMatrix::SimpleMatrix(const IdentityMat& m) : SiconosMatrix(siconos::IDENTITY) {
...
const DenseMat SimpleMatrix::getDense(unsigned int, unsigned int) const {
  if (_num != siconos::DENSE) THROW_EXCEPTION(" the current matrix is not a Dense matrix");
...
const SparseMat SimpleMatrix::getSparse(unsigned int, unsigned int) const {
  if (_num != siconos::SPARSE) THROW_EXCEPTION("the current matrix is not a Sparse matrix");
...
const BandedMat SimpleMatrix::getBanded(unsigned int, unsigned int) const {
  if (_num != siconos::BANDED) THROW_EXCEPTION("the current matrix is not a Banded matrix");

  return *mat.Banded;
}

const ZeroMat SimpleMatrix::getZero(unsigned int, unsigned int) const {
  if (_num != siconos::ZERO) THROW_EXCEPTION("the current matrix is not a Zero matrix");
...
double* SimpleMatrix::getArray(unsigned int, unsigned int) const {
  if (_num == siconos::SPARSE) THROW_EXCEPTION("not yet implemented for sparse matrix.");
...
void SimpleMatrix::resize(unsigned int row, unsigned int col, unsigned int lower,
                          unsigned int upper, bool preserve) {
  if (_num == siconos::DENSE) {
...
std::string SimpleMatrix::toString() const { return ::toString(*this); }
...
// // x and y blocks
// void private_prod(SPC::SiconosMatrix A, const unsigned int startRow, SPC::BlockVector x,
// SP::BlockVector y, bool init)
// {
//   assert(!(A->isPLUFactorizedInPlace()) && "A is PLUFactorizedInPlace in prod !!");
...
// // x and y blocks
// void private_prod(SPC::SiconosMatrix A, const unsigned int startRow, SPC::SiconosVector x,
// SP::BlockVector y, bool init)
// {
//   assert(!(A->isPLUFactorizedInPlace()) && "A is PLUFactorizedInPlace in prod !!");
...
// void private_addprod(SPC::BlockVector x, SPC::SiconosMatrix A, unsigned int startRow,
// unsigned int startCol, SP::SiconosVector y)
// {
//   assert(!(A->isPLUFactorizedInPlace()) && "A is PLUFactorizedInPlace in prod !!");
//   VectorOfVectors::const_iterator it;
...
// void private_prod(SPC::SiconosVector x, SPC::SiconosMatrix A, unsigned int startCol,
// SP::BlockVector  y, bool init)
// {
//   assert(!(A->isPLUFactorizedInPlace()) && "A is PLUFactorizedInPlace in prod !!");
...
// void private_prod(SPC::BlockVector x, SPC::SiconosMatrix A, unsigned int startCol,
// SP::SiconosVector  y, bool init)
// {
//   assert(!(A->isPLUFactorizedInPlace()) && "A is PLUFactorizedInPlace in prod !!");
...
// void private_addprod(double a, SPC::SiconosMatrix A, unsigned int startRow, unsigned int
// startCol, SPC::SiconosVector x, SP::SiconosVector y)
// {
//   assert(!(A->isPLUFactorizedInPlace()) && "A is PLUFactorizedInPlace in prod !!");
...
// void private_prod(double a, SPC::SiconosMatrix A, unsigned int startRow, SPC::SiconosVector
// x, SP::SiconosVector  y, bool init)
// {
//   assert(!(A->isPLUFactorizedInPlace()) && "A is PLUFactorizedInPlace in prod !!");
