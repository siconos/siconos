                                ...
                                const std::vector<std::size_t> &start)
{
  ...
  auto numIn = input_matrix.num();
  auto numOut = output_matrix->num();

  if (numOut == UBLAS_TYPE::ZERO ||
      numOut == UBLAS_TYPE::IDENTITY)  // if output_matrix = 0 or Identity => read-only
  ...
  if (numIn == UBLAS_TYPE::BLOCK && numOut == UBLAS_TYPE::BLOCK)
    THROW_EXCEPTION(
        "not yet implemented for input_matrix and output_matrix both BlockMatrix. "
        "Try to use setBlock on the sub-matrices?");

  if (numOut == UBLAS_TYPE::BLOCK)  // if output_matrix is a BlockMatrix.
  {
    // Steps:
    ...
    //
    //        => The blocks concerned in output_matrix, are those between (block) rows
    //        blockStart0 and blockEnd0
    //           and (block) columns blockStart1 and blockEnd1.
    //
    // C - Loop through the concerned blocks (name = currentBlock) of output_matrix and call
    ...
    // A - Block-Row position: we look for the block of output_matrix that include index
    // start[2] and end[2].
    //
    unsigned int blockStart0 = 0;
    auto tab = output_matrix->tabRow();
    while (start[2] >= (*tab)[blockStart0] && blockStart0 < tab->size()) blockStart0++;
    // Relative position in the block blockStart0 of the first element to be set.
    unsigned int posOut0 = start[2];
    if (blockStart0 != 0) posOut0 -= (*tab)[blockStart0 - 1];

    unsigned int blockEnd0 = blockStart0;
    while (end[2] > (*tab)[blockEnd0] && blockEnd0 < tab->size()) blockEnd0++;

    // Size of the last sub-block in the column of block
    unsigned int lastBlockSize0 = end[2];
    if (blockEnd0 != 0) lastBlockSize0 -= (*tab)[blockEnd0 - 1];

    // B - Block-Col position: we look for the block of output_matrix that include index
    // start[3] and end[3].
    unsigned int blockStart1 = 0;
    tab = output_matrix->tabCol();
    while (start[3] >= (*tab)[blockStart1] && blockStart1 < tab->size()) blockStart1++;
    // Relative position in the block blockStart1 of the first element to be set.
    unsigned int posOut1 = start[3];
    if (blockStart1 != 0) posOut1 -= (*tab)[blockStart1 - 1];

    unsigned int blockEnd1 = blockStart1;
    while (end[3] > (*tab)[blockEnd1] && blockEnd1 < tab->size()) blockEnd1++;

    // Size of the last sub-block in the row of block
    unsigned int lastBlockSize1 = end[3];
    if (blockEnd1 != 0) lastBlockSize1 -= (*tab)[blockEnd1 - 1];
    ...
    auto currentBlock = output_matrix->block(blockStart0, blockStart1);

    // dim of the subBlock of currentBlock to be set.
    std::vector<std::size_t> subSize(2);
    // indices of the first element of input_matrix (resp. currentBlock) to be read (resp. set)
    // (same as start for input_matrix and output_matrix).
    std::vector<std::size_t> currentPos(4);
    ...
    while (numRow != blockEnd0 + 1) {
      while (numCol != blockEnd1 + 1) {
        ...
        // subSize[0] is only required for the first block in the row, after it remains
        // constant.
  ...
  else if (numIn == UBLAS_TYPE::BLOCK)  // If input_matrix is a BlockMatrix.
  {
    // Same process as for numOut == 0

    unsigned int blockStart0 = 0;
    auto tab = input_matrix.tabRow();
    while (start[0] >= (*tab)[blockStart0] && blockStart0 < tab->size()) blockStart0++;
    // Relative position in the block blockStart0 of the first element to be set.
    unsigned int posOut0 = start[0];
    if (blockStart0 != 0) posOut0 -= (*tab)[blockStart0 - 1];

    unsigned int blockEnd0 = blockStart0;
    while (end[0] > (*tab)[blockEnd0] && blockEnd0 < tab->size()) blockEnd0++;
    ...
    while (start[1] >= (*tab)[blockStart1] && blockStart1 < tab->size()) blockStart1++;
    // Relative position in the block blockStart1 of the first element to be set.
    unsigned int posOut1 = start[1];
    if (blockStart1 != 0) posOut1 -= (*tab)[blockStart1 - 1];

    unsigned int blockEnd1 = blockStart1;
    while (end[1] > (*tab)[blockEnd1] && blockEnd1 < tab->size()) blockEnd1++;
    ...
    // The current considered block
    auto currentBlock = input_matrix.block(blockStart0, blockStart1);
