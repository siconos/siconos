...
def plot_min_max_cf_work(filename):
    ...
    cf_work_min_max = []
    for t in times_e:
        cf_work_min_max.append([0., 0., 0. , 0.])
    ...
    for t in times_list:
        ...
        if cf_work_n.shape[0] > 0:
            if sum == True:
                cf_work_min_max[id_t_abs] = [np.sum(cf_work_n),
                                             np.sum(cf_work_n),
                                             np.sum(cf_work_t),
                                             np.sum(cf_work_t),
                                             ]
            else:
                cf_work_min_max[id_t_abs] = [np.max(np.where(cf_work_n > 0, cf_work_n, 0.0)),
...
def plot_energy(filename):
    ...
    plt.subplot(121)
    plt.plot(time[start_index:], force_work[start_index:],
             label='force work (potential energy)' + nsl)

    plt.plot(time[start_index:], kinetic[start_index:], label='kinetic' + nsl)
    plt.plot(time[start_index:], normal_contact_work[start_index:],
             label='normal contact work' + nsl)
    plt.plot(time[start_index:], tangent_contact_work[start_index:],
             label='tangent contact work' + nsl)

    plt.plot(time[start_index:], -force_work[start_index:] + kinetic[start_index:] - normal_contact_work[start_index:]
             - tangent_contact_work[start_index:], label='energy_balance' + nsl, linestyle='dotted')
    plt.xlabel('time')
    plt.ylabel('Energy/Work')
    plt.legend()
    ...
    plt.plot(time[start_index:], force_work[start_index:] ,
             label='force work (potential energy)' + nsl)
    plt.fill_between(time[start_index:], force_work[start_index:],
                     force_work[start_index:] - kinetic[start_index:], alpha=0.3, label='kinetic energy')
    plt.plot(time[start_index:], force_work[start_index:] - kinetic[start_index:])
    plt.fill_between(time[start_index:],
                     force_work[start_index:] - kinetic[start_index:],
                     force_work[start_index:] - kinetic[start_index:]
                     + normal_contact_work[start_index:]
                     , alpha=0.3, label='normal contact work')
    plt.plot(time[start_index:], force_work[start_index:]
             - kinetic[start_index:] + normal_contact_work[start_index:])
    plt.fill_between(time[start_index:],
                     force_work[start_index:] - kinetic[start_index:]
                     + normal_contact_work[start_index:] ,
                     force_work[start_index:] - kinetic[start_index:]
                     + normal_contact_work[start_index:] + tangent_contact_work[start_index:]
                     , alpha=0.3, label='tangent contact work')
    ...
    plt.fill_between(time[start_index:],
                     ...
                     , alpha=0.3, label='numerical loss')
    ...
    if with_details:
        ...
        plt.plot(time[start_index:], tangent_contact_work[start_index:],
                 label='tangent contact work' + nsl)
        plt.plot(time[start_index:], normal_contact_work_negative[start_index:],
                 label='normal contact work (only negative part)' + nsl)
        plt.plot(time[start_index:], tangent_contact_work_negative[start_index:],
