...
def resolve_path(source_dir, header_path):
    ...
    for path, dirlist, filelist in os.walk(os.path.join(source_dir, "kernel/src")):
...
def get_headers(targets):
    ...
    all_headers = [
        h
        for h in itertools.chain(*(input_headers[target] for target in ordered_targets))
    ]
    return all_headers
...
def write_register_with_bases(dest_file, with_base):
    for target in sorted({t: None for c, p, t in with_base}.keys()):
        with_base_s = [
            c
            for c, p, t in sorted(with_base, key=lambda k: (k[1], k[0]))
            if t == target
        ]
        dest_file.write("\n")
