...
void siconos_io(Archive& ar, GlobalFrictionContact& v, unsigned int version) {
  SERIALIZE(
      v,
      (_contactProblemDim)(_sizeGlobalOutput)(_globalVelocities)(_b)(_H)(_mu)(_numerics_solver_options),
      ar);
...
#define MAKE_SICONOS_IO_PROPERTIES(CLASS)                                                   \
  template <class Archive>                                                                  \
  void siconos_io(Archive& ar, siconos::VertexProperties<CLASS, _DynamicalSystemsGraph>& p, \
                  unsigned int version) {                                                   \
    SERIALIZE(p, (_g)(_stamp), ar);                                                         \
    siconos_property_io(ar, p);                                                             \
  }                                                                                         \
  template <class Archive>                                                                  \
  void siconos_io(Archive& ar, siconos::VertexProperties<CLASS, _InteractionsGraph>& p,     \
                  unsigned int version) {                                                   \
    SERIALIZE(p, (_g)(_stamp), ar);                                                         \
    siconos_property_io(ar, p);                                                             \
  }                                                                                         \
  template <class Archive>                                                                  \
  void siconos_io(Archive& ar, siconos::EdgeProperties<CLASS, _DynamicalSystemsGraph>& p,   \
                  unsigned int version) {                                                   \
    SERIALIZE(p, (_g)(_stamp), ar);                                                         \
    siconos_property_io(ar, p);                                                             \
  }                                                                                         \
  template <class Archive>                                                                  \
  void siconos_io(Archive& ar, siconos::EdgeProperties<CLASS, _InteractionsGraph>& p,       \
                  unsigned int version) {                                                   \
    SERIALIZE(p, (_g)(_stamp), ar);                                                         \
    siconos_property_io(ar, p);                                                             \
  }
...
#define MAKE_SICONOS_IO_SP_PROPERTIES(CLASS)                                                  \
  template <class Archive>                                                                    \
  void siconos_io(Archive& ar, siconos::VertexSPProperties<CLASS, _DynamicalSystemsGraph>& p, \
                  unsigned int version) {                                                     \
    SERIALIZE(p, (_g)(_stamp), ar);                                                           \
    siconos_property_io(ar, p);                                                               \
  }                                                                                           \
  template <class Archive>                                                                    \
  void siconos_io(Archive& ar, siconos::VertexSPProperties<CLASS, _InteractionsGraph>& p,     \
                  unsigned int version) {                                                     \
    SERIALIZE(p, (_g)(_stamp), ar);                                                           \
    siconos_property_io(ar, p);                                                               \
  }
