...
AC_PREREQ(2.57)
echo
# Libxml: no include files (FORTRAN libraries)
...
do dynlib=no;  
   ...
   if test -r "$ac_dir/bin/xml2-config" ; then
       ...
       if test $? -eq 0; then
	  AC_MSG_CHECKING(Libxml library  : The minimal required version is $LIBXML_VER)
          echo
       else
          LIBXML2_INCLUDES="`$ac_dir/bin/xml2-config --cflags`"
          LIBXML2_LIBRARIES="`$ac_dir/bin/xml2-config --libs`"
          libxml_lib="yes"
          AC_MSG_RESULT([yes - Found a dynamic library $ac_dir/lib/libxml2.$libsuffix]) 
	  break;  
       fi    	
    fi
done
