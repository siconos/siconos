...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    m(k, 0) = k * t->getH();
    // position
    LagrangianNLDS* ball = static_cast<LagrangianNLDS*>(b2d.getNonSmoothDynamicalSystem()->getDynamicalSystem(0));
    m(k, 1) = (ball->getQ())(0);
    // position
    m(k, 2) = (ball->getQ())(1);
    LagrangianNLDS* ground = static_cast<LagrangianNLDS*>(b2d.getNonSmoothDynamicalSystem()->getDynamicalSystem(1));
    m(k, 3) = (ground->getQ())(0);
    // position
