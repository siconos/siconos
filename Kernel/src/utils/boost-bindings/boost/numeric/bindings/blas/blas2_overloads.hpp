...
{
  BLAS_SGEMV(&TRANS, &m, &n, (&alpha), (a_ptr), &lda, (x_ptr), &incx, (&beta), (y_ptr), &incy) ;
}
inline
...
{
  BLAS_DGEMV(&TRANS, &m, &n, (&alpha), (a_ptr), &lda, (x_ptr), &incx, (&beta), (y_ptr), &incy) ;
}
inline
...
{
  BLAS_CGEMV(&TRANS, &m, &n, complex_ptr(&alpha), complex_ptr(a_ptr), &lda, complex_ptr(x_ptr), &incx, complex_ptr(&beta), complex_ptr(y_ptr), &incy) ;
}
inline
void gemv(char TRANS, const int& m, const int& n, const complex_d& alpha, const complex_d* a_ptr, const int& lda, const complex_d* x_ptr, const int& incx, const complex_d& beta, complex_d* y_ptr, const int& incy)
{
  BLAS_ZGEMV(&TRANS, &m, &n, complex_ptr(&alpha), complex_ptr(a_ptr), &lda, complex_ptr(x_ptr), &incx, complex_ptr(&beta), complex_ptr(y_ptr), &incy) ;
}
...
{
  BLAS_SGER(&m, &n, &alpha, x_ptr, &incx, y_ptr, &incy, a_ptr, &lda) ;
}
inline
...
{
  BLAS_DGER(&m, &n, &alpha, x_ptr, &incx, y_ptr, &incy, a_ptr, &lda) ;
}
/*
  inline
