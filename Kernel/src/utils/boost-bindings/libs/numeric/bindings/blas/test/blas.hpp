...
void check(Iterator0 begin0, Iterator0 end0, Iterator1 begin1)
{
  ...
  if (it0 != end0 || it1 != end1)
  {
    ...
    std::copy(begin0, end0, std::ostream_iterator< typename Iterator0::value_type >(std::cerr, " "));
    std::cerr << std::endl;
    std::copy(begin1, end1, std::ostream_iterator< typename Iterator1::value_type >(std::cerr, " "));
    std::cerr << std::endl;
    exit(1);
  }
