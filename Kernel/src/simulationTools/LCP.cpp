...
void LCP::computeQBlock(UnitaryRelation* UR, unsigned int pos)
{
  ...
  if (osiType == "Moreau" || osiType == "Lsodar")
  {
    ...
    else if (relationType == "LagrangianLinearR")
    {
      ...
      RuntimeException::selfThrow("LCP::computeQBlock, not yet implemented for first order relations of subtype " + relationType);
  ...
  if (UR->getRelationType() == "Lagrangian")
  {
    ...
    if (nslawType == NEWTONIMPACTNSLAW)
    {
      ...
      else if (simulationType == "EventDriven")
      {
        ...
        RuntimeException::selfThrow("LCP::computeQBlock not yet implemented for relation of type " + relationType + " and non smooth law of type " + nslawType + " for a simulaton of type " + simulationType);
    }
    else if (nslawType == NEWTONIMPACTFRICTIONNSLAW)
    {
      ...
      else RuntimeException::selfThrow("LCP::computeQBlock not yet implemented for relation of type " + relationType + " and non smooth law of type " + nslawType + " for a simulaton of type " + simulationType);

    }
    else
      RuntimeException::selfThrow("LCP::computeQBlock not yet implemented for relation of type " + relationType + " and non smooth law of type " + nslawType);
  }
}
