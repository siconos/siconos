...
// Default constructor
FrictionContact2D::FrictionContact2D(): OneStepNSProblem(), w(NULL), z(NULL), M(NULL), q(NULL),
  isWAllocatedIn(false), isZAllocatedIn(false), isMAllocatedIn(false), isQAllocatedIn(false)
{
  nspbType = FrictionContact2D_OSNSP;
}
  ...
  isWAllocatedIn(false), isZAllocatedIn(false), isMAllocatedIn(false), isQAllocatedIn(false)
{
  ...
  if (osNsPbXml != NULL)
  {
    FrictionContact2DXML * xmllcp = (static_cast<FrictionContact2DXML*>(osNsPbXml));
    ...
    if (xmllcp->hasQ() && xmllcp->hasM() && ((xmllcp->getM()).size(0) != (xmllcp->getQ()).size()))
      RuntimeException::selfThrow("FrictionContact2D: xml constructor, inconsistent sizes between given q and M");
  ...
  else RuntimeException::selfThrow("FrictionContact2D: xml constructor, xml file=NULL");
}

// Constructor from a set of data
FrictionContact2D::FrictionContact2D(Strategy* newStrat, const string& newSolver, const string& newSolvingMethod,
...
void FrictionContact2D::setW(const SimpleVector& newValue)
{
  if (dim != newValue.size())
    RuntimeException::selfThrow("FrictionContact2D: setW, inconsistent size between given w size and problem size. You should set dim before");
  ...
  if (w == NULL)
  {
    ...
    RuntimeException::selfThrow("FrictionContact2D: setW, w size differs from dim");

  *w = newValue;
}

void FrictionContact2D::setWPtr(SimpleVector* newPtr)
{
  if (dim != newPtr->size())
    RuntimeException::selfThrow("FrictionContact2D: setWPtr, inconsistent size between given w size and problem size. You should set dim before");
...
void FrictionContact2D::setZ(const SimpleVector& newValue)
{
  if (dim != newValue.size())
    RuntimeException::selfThrow("FrictionContact2D: setZ, inconsistent size between given z size and problem size. You should set dim before");
...
void FrictionContact2D::setZPtr(SimpleVector* newPtr)
{
  if (dim != newPtr->size())
    RuntimeException::selfThrow("FrictionContact2D: setZPtr, inconsistent size between given z size and problem size. You should set dim before");
...
void FrictionContact2D::setM(const SiconosMatrix& newValue)
{
  if (dim != newValue.size(0) || dim != newValue.size(1))
    RuntimeException::selfThrow("FrictionContact2D: setM, inconsistent size between given M size and problem size. You should set dim before");
...
void FrictionContact2D::setMPtr(SiconosMatrix* newPtr)
{
  if (dim != newPtr->size(0) || dim != newPtr->size(1))
    RuntimeException::selfThrow("FrictionContact2D: setMPtr, inconsistent size between given M size and problem size. You should set dim before");
...
void FrictionContact2D::setQ(const SimpleVector& newValue)
{
  if (dim != newValue.size())
    RuntimeException::selfThrow("FrictionContact2D: setQ, inconsistent size between given q size and problem size. You should set dim before");
...
void FrictionContact2D::setQPtr(SimpleVector* newPtr)
{
  if (dim != newPtr->size())
    RuntimeException::selfThrow("FrictionContact2D: setQPtr, inconsistent size between given q size and problem size. You should set dim before");
...
void FrictionContact2D::initialize()
{
...
void FrictionContact2D::computeAllBlocks()
{
  // --- get topology ---
  Topology * topology = strategy->getModelPtr()->getNonSmoothDynamicalSystemPtr()->getTopologyPtr();
  ...
  for (iter = listInteractions.begin(); iter != listInteractions.end(); ++iter)
  {
    ...
    for (itDS = vDS.begin(); itDS != vDS.end(); itDS++)
    {
      ...
      if (Osi->getType() == MOREAU_INTEGRATOR)
        ...
        RuntimeException::selfThrow("FrictionContact2D::computeAllBlocks not yet implemented for Integrator of type " + Osi->getType());
    }
    ...
    //computeDiagonalBlocksLagrangianR(RCurrent, sizeInteraction,vDS,W,h,currentMatrixBlock);
    else RuntimeException::selfThrow("FrictionContact2D::computeAllBlocks not yet implemented for relation of type " + relationType);
...
void FrictionContact2D::computeDiagonalBlocksLinearTIR(Relation * R, const unsigned int& sizeInteraction, vector<DynamicalSystem*> vDS,
...
void FrictionContact2D::computeExtraDiagonalBlocksLinearTIR(Relation * RCurrent, Relation* RLinked, const unsigned int& sizeInteraction,
...
void FrictionContact2D::computeDiagonalBlocksLagrangianR(Relation * R, const unsigned int& sizeInteraction, vector<DynamicalSystem*> vDS,
    map<DynamicalSystem*, SiconosMatrix*> W, const double& h, SiconosMatrix* currentMatrixBlock)
{
...
void FrictionContact2D::computeExtraDiagonalBlocksLagrangianR(Relation * RCurrent, Relation* RLinked, const unsigned int& sizeInteraction,
    const unsigned int& linkedInteractionSize, vector<DynamicalSystem*> commonDS,
    map<DynamicalSystem*, SiconosMatrix*> W, const double& h, SiconosMatrix* coupledInteractionsBlock)
{
  // Warning: we suppose that at this point, G and/or h have been computed through plug-in mechanism.
...
void FrictionContact2D::updateBlocks()
{
  // --- get topology ---
  Topology * topology = strategy->getModelPtr()->getNonSmoothDynamicalSystemPtr()->getTopologyPtr();
  ...
  for (iter = listInteractions.begin(); iter != listInteractions.end(); ++iter)
  {
    ...
    {}// Nothing to be done, blocks allready computed
      ...
      computeDiagonalBlocksLagrangianR(RCurrent, sizeInteraction, vDS, W, h, currentMatrixBlock);
    else RuntimeException::selfThrow("FrictionContact2D::updateBlocks not yet implemented for relation of type " + relationType);
...
void FrictionContact2D::preFrictionContact2D(const double& time)
{
  IN("FrictionContact2D::preFrictionContact2D()\n");
  // compute M and q operators for FrictionContact2D problem
  ...
  OUT("FrictionContact2D::preFrictionContact2D()\n");
}

void FrictionContact2D::assembleM() //
{
...
{
  IN("FrictionContact2D::compute(void)\n");

  // --- Prepare data for FrictionContact2D computing ---
  preFrictionContact2D(time);
  ...
  OUT("FrictionContact2D::compute(void)\n");
}

void FrictionContact2D::postFrictionContact2D(const SimpleVector& w, const SimpleVector &z)
{
  // --- get topology ---
  Topology * topology = strategy->getModelPtr()->getNonSmoothDynamicalSystemPtr()->getTopologyPtr();
  ...
  for (iter = listInteractions.begin(); iter != listInteractions.end(); ++iter)
  {
    ...
    */
    // Get the 'position' of vector corresponding to the current interaction, in the FrictionContact2D output:
...
void FrictionContact2D::display() const
{
  cout << "======= FrictionContact2D display ======" << endl;
  cout << "| dim : " << dim << endl;
  cout << "| FrictionContact2D Matrix M  : " << endl;
  if (M != NULL) M->display();
  else cout << "-> NULL" << endl;
  cout << "| FrictionContact2D vector q : " << endl;
  if (q != NULL) q->display();
  else cout << "-> NULL" << endl;
  cout << "==========================" << endl;
...
void FrictionContact2D::saveNSProblemToXML()
{
  IN("FrictionContact2D::saveNSProblemToXML\n");
  OneStepNSProblem::saveNSProblemToXML();
  if (onestepnspbxml != NULL)
  {
    (static_cast<FrictionContact2DXML*>(onestepnspbxml))->setM(*M);
    (static_cast<FrictionContact2DXML*>(onestepnspbxml))->setQ(*q);
  }
  else RuntimeException::selfThrow("FrictionContact2D::saveNSProblemToXML - OneStepNSProblemXML object not exists");
  OUT("FrictionContact2D::saveNSProblemToXML\n");
}

void FrictionContact2D::saveMToXML()
{
  IN("FrictionContact2D::saveMToXML\n");
  if (onestepnspbxml != NULL)
  {
    (static_cast<FrictionContact2DXML*>(onestepnspbxml))->setM(*M);
  }
  else RuntimeException::selfThrow("FrictionContact2D::saveMToXML - OneStepNSProblemXML object not exists");
  OUT("FrictionContact2D::saveMToXML\n");
}

void FrictionContact2D::saveQToXML()
{
  IN("FrictionContact2D::saveQToXML\n");
  if (onestepnspbxml != NULL)
  {
    (static_cast<FrictionContact2DXML*>(onestepnspbxml))->setQ(*q);
  }
  else RuntimeException::selfThrow("FrictionContact2D::saveQToXML - OneStepNSProblemXML object not exists");
  OUT("FrictionContact2D::saveQToXML\n");
}
