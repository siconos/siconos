...
int FrictionContact2D::compute(double time)
{
  ...
  if (sizeOutput != 0)
  {
    ...
    if (solver->useBlocks()) // Use solver block
      //  info = pfc_2D_solver_block((int)sizeOutput/2, Mspbl , q->getArray() , &solvingMethod , reaction->getArray() , velocity->getArray() ,mu->getArray());
      RuntimeException::selfThrow("FrictionContact2D::compute - Solver block not yet available.");
    else // Use classical solver
      info = pfc_2D_driver((int)sizeOutput / 2, M->getArray(), q->getArray(), &solvingMethod  , reaction->getArray(), velocity->getArray(), mu->getArray());
