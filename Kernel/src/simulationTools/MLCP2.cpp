...
void MLCP2::computeDSBlock(SP::DynamicalSystem DS)
{
  ...
  {
    DSBlocks[DS] = (cpp11ns::static_pointer_cast<Moreau2> (Osi))->W(DS); // get its W matrix ( pointer link!)
    (*DSBlocks[DS]) *= -1.0;
    DSBlocks[DS]->display();
  }
}
...
void MLCP2::computeq(double time)
{
  ...
  for (itCurrent = indexSet->begin(); itCurrent !=  indexSet->end(); ++itCurrent)
  {
    ...
    //update e(ti+1)
    SP::SiconosVector  e = cpp11ns::static_pointer_cast<FirstOrderLinearR>((*itCurrent)->relation())->e();
    q->addBlock(pos, *e);
  }
