...
protected:
  ...
  // --- Plug-in for scleronomic-holonomic constraints case -> h(q), G(q) ---
  /** LagrangianScleronomousR plug-in to compute h(q) (scleronomic case)
  ...
  /** LagrangianScleronomousR plug-in to compute G0(q), gradient of h according to q (scleronomic case)
   * @param sizeQ: size of q = sum of the sizes of all the DynamicalSystems involved in the interaction
   * @param q : pointer to the first element of q
