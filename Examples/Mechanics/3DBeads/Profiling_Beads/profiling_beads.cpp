...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    for (j = 0; j < etage; j++)
    {
      for (k = 0; k < cote; k++)
      {
        for (i = 0; i < cote; i++)
        {
          if (j % 2 == 0)
          {
            (*(q0[k * cote + i + j * cote * cote]))(0) = -0.6 + 2 * k * R;
            (*(q0[k * cote + i + j * cote * cote]))(1) = -0.6 + 2 * i * R;
    ...
    for (iter_k = 1 ; iter_k < N + 1  ; ++iter_k)
    {
      ...
      ti2 = clock();
      cout << "Computation Timei: " << (ti2 - ti1) / (double)CLOCKS_PER_SEC << endl << endl;
    ...
    t2 = clock();
    cout << "Final Computation Time: " << (t2 - t1) / (double)CLOCKS_PER_SEC << endl << endl;
