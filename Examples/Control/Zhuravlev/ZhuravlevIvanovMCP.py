...
class ZI(object):
    ...
    def computeJacg(self, l0, l1):
        ...
        B = self.Jacg
        B[1, 0] = 2*self._g/(1-self._kappa*l0*l1)**2
        B[1, 2] = 2*(self._g*self._kappa*l0**2)/(1-self._kappa*l0*l1)**2
        B[0, 0] = -2*self._kappa*l1*(v_gamma) - self.gamma*self.h*self._kappa*l0*l1*B[1, 0]
        B[0, 2] = -2*self._kappa*l0*(v_gamma) - self.gamma*self.h*self._kappa*l0*l1*B[1, 2]
        return B
