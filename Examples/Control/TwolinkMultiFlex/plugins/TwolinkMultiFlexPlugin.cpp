...
extern "C" void U(double time, unsigned int sizeOfq, const double *q, const  double *velocity, double *U, unsigned int sizeZ, double* z)
{
  ...
  double NNL0 = -m2 * l1 * l2 * sin(q[1]) * (velocity[0] * velocity[1] + velocity[1] * velocity[1] / 2) + K1 * (q[0] - q[2]);
  double NNL1 = m2 * l1 * l2 * sin(q[1]) * velocity[0] * velocity[0] / 2 + K2 * (q[1] - q[3]);
...
extern "C" void U1(double time, unsigned int sizeOfq, const double *q, const  double *velocity, double *U, unsigned int sizeZ, double* z)
{
  double m11 = m1 * (l1 * l1 / 4) + I1 + I2 + m2 * (l1 * l1 + (l2 * l2 / 4) + l1 * l2 * cos(q[1]));
  double m12 = (m2 * l2 * l2 / 4) + I2 + m2 * l1 * l2 * cos(q[1]) / 2;
  double m22 = (m2 * l2 * l2 / 4) + I2;
  ...
  double b0 = 0.5 * sin(2 * PI * z[8] / P + PI / 2);
  double b1 = 0.5 * 2 * PI * cos(2 * PI * z[8] / P + PI / 2) / P;
  ...
  double qd31 = 0;
  double qd41 = 0;
  double qd2 = b3 * t3 * t3 * t3 + b2 * t3 * t3 + b1 * t3 + b0;
  double qd12 = 3 * b3 * t3 * t3 + 2 * b2 * t3 + b1;
  double qd22 = 6 * b3 * t3 + 2 * b2;
  double qd32 = 6 * b3;
  double qd42 = 0;
  ...
  double dT11 = dmc21 * qr21 + dmc22 * qr22 + mc21 * qr31 + mc22 * qr32;
  double dT12 = da3 + da4;
  double dT13 = -l2 * cos(q[0] + q[1]) * (q[2] + q[3]) * gamma1 * s1 - l2 * sin(q[0] + q[1]) * (velocity[0] + velocity[1]) * gamma1 * s2 + grad12 * gamma1 * (x2 - qr21) + grad22 * gamma1 * (y2 - qr22);
  ...
  double s12 = (velocity[2] - thetad11) + gamma2 * (q[2] - thetad1);
  double s22 = (velocity[3] - thetad12) + gamma2 * (q[3] - thetad2);
  ...
  double V1 = (s1 * s1 * (d11 * mc11 + d21 * mc21) + 2 * s1 * s2ly * (d12 * mc11 + d22 * mc21) + s2ly * s2ly * (d12 * mc12 + d22 * mc22)) / 2 + J1 * s12 * s12 + J2 * s22 * s22;
  z[6] = V1 + gamma1 * gamma2 * ((q[0] - xd) * (q[0] - xd) + (q[1] - yd) * (q[1] - yd) + (q[2] - thetad1) * (q[2] - thetad1) + (q[3] - thetad2) * (q[3] - thetad2)) + (q[0] - xd - q[2] + thetad1) * K1 * (q[0] - xd - q[2] + thetad1) / 2 + (q[1] - yd - q[3] + thetad2) * K2 * (q[1] - yd - q[3] + thetad2) / 2;
...
extern "C" void U3(double time, unsigned int sizeOfq, const double *q, const  double *velocity, double *U, unsigned int sizeZ, double* z)
{
  ...
  double qd11 = (3 * c3 * td * td + 2 * c2 * td) / (P / 4);
  double qd21 = (6 * c3 * td + 2 * c2) / ((P / 4) * (P / 4));
  double qd31 = (6 * c3) / ((P / 4) * (P / 4) * (P / 4));
  double qd41 = 0;
  double qd2 = 0;
  double qd12 = 0;
  double qd22 = 0;
  ...
  double V1 = (s1ly * s1ly * (d11 * mc11 + d21 * mc21) + 2 * s1ly * s2 * (d12 * mc11 + d22 * mc21) + s2 * s2 * (d12 * mc12 + d22 * mc22)) / 2 + (J1 * s21 * s21 + J2 * s22 * s22) / 2;
  z[6] = V1 + gamma1 * gamma2 * ((q[0] - xd) * (q[0] - xd) + (q[1] - yd) * (q[1] - yd) + (q[2] - thetad1) * (q[2] - thetad1) + (q[3] - thetad2) * (q[3] - thetad2)) + (q[0] - xd - q[2] + thetad1) * K1 * (q[0] - xd - q[2] + thetad1) / 2 + (q[1] - yd - q[3] + thetad2) * K2 * (q[1] - yd - q[3] + thetad2) / 2;
...
extern "C" void U4(double time, unsigned int sizeOfq, const double *q, const  double *velocity, double *U, unsigned int sizeZ, double* z)
{
  double m11 = m1 * (l1 * l1 / 4) + I1 + I2 + m2 * (l1 * l1 + (l2 * l2 / 4) + l1 * l2 * cos(q[1]));
  double m12 = (m2 * l2 * l2 / 4) + I2 + m2 * l1 * l2 * cos(q[1]) / 2;
  double m22 = (m2 * l2 * l2 / 4) + I2;
  ...
  double ddT11 = ddmc21 * qr21 + ddmc22 * qr22 + 2 * dmc21 * qr31 + 2 * dmc22 * qr32 + mc21 * qr41 + mc22 * qr42;
  double ddT12 = dda3 + dda4;
  double ddT13 = -l2 * cos(q[0] + q[1]) * (velocity[0] + velocity[1]) * (velocity[0] + velocity[1]) * gamma1 * s1bar - l2 * cos(q[0] + q[1]) * (dv0 + dv1) * gamma1 * s1bar - l2 * cos(q[0] + q[1]) * (velocity[0] + velocity[1]) * gamma1 * (x2 - qr21) - l2 * sin(q[0] + q[1]) * (velocity[0] + velocity[1]) * (velocity[0] + velocity[1]) * gamma1 * s2 - l2 * sin(q[0] + q[1]) * (dv0 + dv1) * gamma1 * s2 - l2 * sin(q[0] + q[1]) * (velocity[0] + velocity[1]) * gamma1 * (y2 - qr22) + grad12 * gamma1 * (x3 - qr31) + grad22 * gamma1 * (y3 - qr32) - l2 * cos(q[0] + q[1]) * (velocity[0] + velocity[1]) * gamma1 * (x2 - qr21) - l2 * sin(q[0] + q[1]) * (velocity[0] + velocity[1]) * gamma1 * (y2 - qr22);
  ...
  // double ld = (m2*l1*l2*sin(q[1])*velocity[0]*(d12-d22/2)-a21-(mc21/mc11)*(m2*l1*l2*sin(q[1])*velocity[0]*(d11-d21/2)-a11))*s1-gamma1*mc21*s1/mc11+10;//*(k*P/2-time);

  // control law
  U[0] = 0;//grad21*(Kf*z[4]-ld);
  U[1] = 0;//grad22*(Kf*z[4]-ld);
  U[2] = -(J1 * thetar21 + K1 * (thetad1 - xd) - gamma1 * s12); //+grad21*(Kf*z[4]-ld);
  U[3] = -(J2 * thetar22 + K2 * (thetad2 - yd) - gamma1 * s22); //+grad22*(Kf*z[4]-ld);
...
extern "C" void U5(double time, unsigned int sizeOfq, const double *q, const  double *velocity, double *U, unsigned int sizeZ, double* z)
{
  double m11 = m1 * (l1 * l1 / 4) + I1 + I2 + m2 * (l1 * l1 + (l2 * l2 / 4) + l1 * l2 * cos(q[1]));
  double m12 = (m2 * l2 * l2 / 4) + I2 + m2 * l1 * l2 * cos(q[1]) / 2;
  double m22 = (m2 * l2 * l2 / 4) + I2;
  ...
  double qd32 = 6 * c3 / ((k * P - z[8]) * (k * P - z[8]) * (k * P - z[8]));
  double qd42 = 0;
  ...
  z[12] = 0.01 + (-mc11 * ld + fabs(mc11 * ld)) / 2;

  double thetad1 = xd + (T01 + T02 - T03 + grad11 * (z[18] - ld)) / K1;
  double thetad2 = yd + (T11 + T12 - T13 + grad12 * (z[18] - ld)) / K2;
  double thetad11 = xd1 + (dT01 + dT02 - dT03) / K1;
  double thetad12 = yd1 + (dT11 + dT12 - dT13) / K2;
  double thetad21 = xd2 + (ddT01 + ddT02 - ddT03) / K1;
  double thetad22 = yd2 + (ddT11 + ddT12 - ddT13) / K2;
...
extern "C" void U6(double time, unsigned int sizeOfq, const double *q, const  double *velocity, double *U, unsigned int sizeZ, double* z)
{
  double m11 = m1 * (l1 * l1 / 4) + I1 + I2 + m2 * (l1 * l1 + (l2 * l2 / 4) + l1 * l2 * cos(q[1]));
  double m12 = (m2 * l2 * l2 / 4) + I2 + m2 * l1 * l2 * cos(q[1]) / 2;
  double m22 = (m2 * l2 * l2 / 4) + I2;
  ...
  double qd21 = -0.5 * (2 * PI / P) * (2 * PI / P) * cos(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) - 0.5 * (2 * PI / P) * sin(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) + (time - k * P) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (PI / 2 - b) / (ep * ep) - sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep)));
  double qd31 = 0.5 * (2 * PI / P) * (2 * PI / P) * (2 * PI / P) * sin(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) - 0.5 * (2 * PI / P) * (2 * PI / P) * cos(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * 3 * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) + (time - k * P) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (PI / 2 - b) / (ep * ep) - sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep))) - 0.5 * (2 * PI / P) * sin(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * (4 * cos(b * td + (PI / 2 - b) * td * td) * 2 * (PI / 2 - b) / (ep * ep) - 3 * sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) + (time - k * P) * (-4 * sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (PI / 2 - b) / (ep * ep) - cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep)));
  double qd41 = 0.5 * (2 * PI / P) * (2 * PI / P) * (2 * PI / P) * (2 * PI / P) * cos(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) + 0.5 * (2 * PI / P) * (2 * PI / P) * (2 * PI / P) * sin(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * 6 * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) + (time - k * P) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (PI / 2 - b) / (ep * ep) - sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep))) - 0.5 * (2 * PI / P) * (2 * PI / P) * cos(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * 3 * (2 * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) + (time - k * P) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (PI / 2 - b) / (ep * ep) - sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep))) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) + (time - k * P) * (2 * cos(b * td + (PI / 2 - b) * td * td) * (PI / 2 - b) / (ep * ep) - sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep))) - 0.5 * (2 * PI / P) * (2 * PI / P) * cos(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * 4 * (sin(b * td + (PI / 2 - b) * td * td) + (time - k * P) * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep)) * (2 * cos(b * td + (PI / 2 - b) * td * td) * 2 * (PI / 2 - b) / (ep * ep) - 2 * sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) + (2 * cos(b * td + (PI / 2 - b) * td * td) * (PI / 2 - b) / (ep * ep) - sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep)) + (time - k * P) * (-4 * sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (PI / 2 - b) / (ep * ep) - cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep))) - 0.5 * (2 * PI / P) * sin(PI / 2 + 2 * PI * (k * P + (time - k * P) * sin(b * td + (PI / 2 - b) * td * td)) / P) * (-14 * sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * 2 * (PI / 2 - b) / (ep * ep) - 4 * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) + (time - k * P) * (sin(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) - 7 * cos(b * td + (PI / 2 - b) * td * td) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (b / ep + 2 * (PI / 2 - b) * td / ep) * (PI / 2 - b) / (ep * ep) - 4 * sin(b * td + (PI / 2 - b) * td * td) * (PI / 2 - b) / (ep * ep) * (PI / 2 - b) / (ep * ep)));
  ...
  double qd32 = -0.5 * (2 * PI / P) * (2 * PI / P) * (2 * PI / P) * cos(PI / 2 + 2 * PI * time / P);
...
extern "C" void h3(unsigned int sizeOfq, const double* q, unsigned int sizeOfY, double* y, unsigned int sizeOfZ, double* z)
{
...
extern "C" void G3(unsigned int sizeOfq, const double* q, unsigned int sizeOfY, double* G, unsigned int sizeOfZ, double* z)
{
