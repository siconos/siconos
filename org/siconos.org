...
The source code (zip and tar.gz) can be found on github : https://github.com/siconos/siconos/releases
...
We have also improved our documentation at http://siconos.gforge.inria.fr even if it still lacks some polish.

As usual, your feedback comments are welcome.

We have also started to feed our YouTube channel, https://www.youtube.com/channel/UCgv2siTCJeSdWFPTDk71Iyw
...
---
Main changes: (see https://github.com/siconos/siconos/blob/master/Changelog)
...
Une toute nouvelle version de Siconos 4.0.0 est sortie !! Elle est maintenant distribuée en licence Apache 2.0 

http://siconos.gforge.inria.fr

Qu'est ce que Siconos ?

Siconos est un logiciel de calcul scientifique open--source dédié a la modélisation, la simulation et le commande des systèmes dynamiques non-lisses en C++ et en Python. Il s'adresse principalement aux applications suivantes:
 + systèmes mécaniques (rigides ou déformables) avec du contact unilatéral, du frottement de Coulomb, des impacts (mécanique non-lisse, dynamique des contacts, systèmes multi-corps ou encore matériaux granulaires)
 + circuits électriques commutes comme les circuits électriques avec des composants idéaux ou linéaires par morceaux (convertisseur de puissance, PLL, convertisseur DAC)
 + systèmes dynamiques contrôlés par mode glissant
D'autres applications sont possibles en théorie des systèmes et de la commande (systèmes hybrides, inclusions différentielles, contrôle optimal avec contraintes sur l'état), en optimisation  (systèmes de complémentarité et inégalités variationnelles), informatique graphique, ...

Le code source est disponible sur github : https://github.com/siconos/siconos/releases
Nous travaillons actuellement sur la version candidate 4.1.0 que l'on peut "fourcher" sur github

Les paquets binaires pour Linux devraient être de nouveau  disponibles rapidement

La documentation a été retravaillée et se trouve   http://siconos.gforge.inria.fr 

Comme toujours, vos retours sont les bienvenus.
 
Nous avons aussi commence a nourrir le canal Youtube  https://www.youtube.com/channel/UCgv2siTCJeSdWFPTDk71Iyw Si vous aussi, vous avez des applications a montrer, n'hésitez a pas a contribuer.

Cordialement
Vincent A. pour l'équipe de développement.
