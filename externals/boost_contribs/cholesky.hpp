 ...
 * \param A square symmetric positive definite input matrix (only the lower triangle is
 * accessed) \param L lower triangular output matrix \return nonzero if decompositon fails (the
 * value ist 1 + the numer of the failing row)
 */
...
size_t cholesky_decompose(const MATRIX& A, TRIA& L) {
  ...
  for (size_t k = 0; k < n; k++) {
    double qL_kk =
        A(k, k) - inner_prod(project(row(L, k), range(0, k)), project(row(L, k), range(0, k)));
    ...
    if (qL_kk <= 0) {
      ...
      project(cLk, range(k + 1, n)) =
          (project(column(A, k), range(k + 1, n)) -
           prod(project(L, range(k + 1, n), range(0, k)), project(row(L, k), range(0, k)))) /
          L_kk;
    }
  }
  return 0;
 ...
 * \param A input: square symmetric positive definite matrix (only the lower triangle is
 * accessed) \param A output: the lower triangle of A is replaced by the cholesky factor
...
size_t cholesky_decompose(MATRIX& A) {
  ...
  for (size_t k = 0; k < n; k++) {
    ...
    if (qL_kk <= 0) {
      ...
      project(cLk, range(k + 1, n)) = (project(column(A_c, k), range(k + 1, n)) -
