...
struct vct_access_traits {
  typedef typename boost::numeric::bindings::value_type<V>::type val_t;
  typedef val_t& ref_t;
...
struct times_plus {
  ...
  T operator()(size_t i) const { return s * (T)i + a; }
  T operator()(size_t i, size_t j) const { return s * ((T)i + a) + (T)j + b; }
};
...
void init_m(M& m, F f = F()) {
  ...
  for (std::size_t i = 0; i < sz1; ++i)
    for (std::size_t j = 0; j < sz2; ++j) elem_m(m, i, j) = f(i, j);
}
...
void print_m(M const& m, char const* ch = nullptr) {
  ...
  size_t sz2 = boost::numeric::bindings::size_column(m);
  for (std::size_t i = 0; i < sz1; ++i) {
    for (std::size_t j = 0; j < sz2; ++j) std::cout << elem_m(m, i, j) << ", ";
    std::cout << std::endl;
  }
  //  std::cout << std::endl;
}
...
void print_m_data(M const& m, char const* ch = nullptr) {
  ...
  std::copy(begin_value(m), end_value(m),
            std::ostream_iterator<typename value_type<const M>::type>(std::cout, " "));
  std::cout << std::endl;
}
