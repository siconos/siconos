...
contains
  ...
  subroutine dlsodar2c(C_F, NEQ, Y, T, TOUT, ITOL, RTOL, ATOL, ITASK, ISTATE, IOPT, RWORK, LRW, IWORK, LIW,&
    ...
    integer(c_int), intent(inout), target :: RTOL, ATOL
    integer(c_int), intent(inout) :: ITASK, IOPT, JT
  ...
  subroutine hem52c(NQ,NV,NU,NL,C_FPROB,T,Q,V,U,A,RLAM,TEND,H,RTOL,ATOL,ITOL,C_SOLOUT,IOUT,WK,LWK,IWK,&
    ...
    integer(c_int), intent(inout) :: NQ, NV, NU, NL
    ...
    integer(c_int), intent(inout) :: ITOL, IOUT
    integer(c_int), intent(inout) :: LWK, LIWK
    real(c_double), intent(inout), target :: WK(LWK)
    integer(c_int), intent(inout), target :: IWK(LIWK)
