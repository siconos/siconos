...
MACRO(LIBRARY_PROJECT_SETUP)
  ...
  ELSE(${PROJECT_NAME}_SRCS)
    ...
    IF(${PROJECT_NAME}_ADD_SRCS) 
     MESSAGE(STATUS "Sources files manually with var ${PROJECT_NAME}_ADD_SRCS")
     FOREACH(_FILE ${${PROJECT_NAME}_ADD_SRCS})
        FILE(GLOB _GFILE ${_FILE})
        IF(_GFILE)
          LIST(APPEND _ALL_FILES ${_GFILE})
        ELSE(_GFILE)
          MESSAGE(STATUS "WARNING : file NOT FOUND : ${_FILE}")
        ENDIF(_GFILE)
      ENDFOREACH(_FILE ${${PROJECT_NAME}_ADD_SRCS})
    ENDIF(${PROJECT_NAME}_ADD_SRCS)
  ...
  IF(NOT ${PROJECT_NAME}_HDRS)
   FOREACH(_DIR ${_ALL_DIRS})
    SET(DIR_IS_EXCLUDED FALSE)
    FOREACH(_EXCLUDE_DIR ${${PROJECT_NAME}_HDRS_EXCLUDE_DIR})
     IF(";${_EXCLUDE_DIR};" MATCHES ";${_DIR};")
      SET(DIR_IS_EXCLUDED TRUE)
     ENDIF()
    ENDFOREACH()
    IF(NOT ${DIR_IS_EXCLUDED})
     FILE(GLOB _HDRS  ${_DIR}/*.h ${_DIR}/*.hpp)
     IF(_HDRS)
      LIST(APPEND ${PROJECT_NAME}_HDRS ${_HDRS})
     ENDIF(_HDRS)
    ENDIF(NOT ${DIR_IS_EXCLUDED})
   ENDFOREACH(_DIR ${_ALL_DIRS})
