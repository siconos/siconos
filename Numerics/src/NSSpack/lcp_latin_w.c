/* Siconos-Numerics version 2.0.0, Copyright INRIA 2005-2006.
...
/*!\fn  void lcp_latin_w( int *nn, double *vec, double *qq,  double *z, double *w, int *info, int *iparamLCP, double *dparamLCP)
...
void lcp_latin_w(int *nn, double *vec, double *qq,  double *z, double *w, int *info, int *iparamLCP, double *dparamLCP)
{
  ...
  while ((iter1 < itt) && (err1 > errmax))
  {
    ...
    alpha = omega;
    dscal_((integer*)&n , &alpha , z , &incx);

    alpha = 1.0 - omega;
    daxpy_((integer*)&n, &alpha, zn, &incx, z, &incy);


    alpha = omega;
    dscal_((integer*)&n , &alpha , w , &incx);

    alpha = 1.0 - omega;
    daxpy_((integer*)&n, &alpha, wn, &incx, w, &incy);


    dcopy_((integer*)&n, w, &incx, wn, &incy);
    dcopy_((integer*)&n, z, &incx, zn, &incy);
