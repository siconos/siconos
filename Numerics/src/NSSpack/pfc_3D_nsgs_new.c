 ...
 *
 * \fn  pfc_3D_nsgs_new( int *nn , double *M , double *q , double *z , double *w ,
...
void pfc_3D_nsgs_new(int *nn , double *M , double *q , double *z , double *w , int *info, int *iparamLCP , double *dparamLCP)
{
  ...
  {
    fprintf(stderr, "Numerics, pfc_3D_nsgs_new failed. Unknown solver type.\n");
    exit(EXIT_FAILURE);
  }
  ...
  double *MBlock    = malloc(mm * sizeof(*MBlock));
  if (MBlock == NULL)
  {
  ...
  if (wOld == NULL)
  {
    fprintf(stderr, "pfc_3D_nsgs_new, memory allocation for wOld failed.\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (ispeak > 0)
  {
    if (err > tol)
    {
      printf(" Numerics, pfc_3D_nsgs_new: no convergence after %i iterations\n" , iter);
      printf(" The residue is : %e \n", err);
      *info = 1;
    }
    else
    {
      printf(" Numerics, pfc_3D_nsgs_new: convergence after %i iterations\n" , iter);
      printf(" The residue is : %e \n", err);
      *info = 0;
    }
  }
