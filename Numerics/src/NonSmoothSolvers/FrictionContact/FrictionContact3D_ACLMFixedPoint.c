...
void frictionContact3D_ACLMFixedPoint(FrictionContactProblem* problem, double *reaction, double *velocity, int* info, SolverOptions* options)
{
  ...
  while ((iter < itermax) && (hasNotConverged > 0))
  {
    ...
    internalsolver_options->dparam[0] = max(error/100.0, options->dparam[0]/problem->numberOfContacts);
    //internalsolver_options->dparam[0] = options->dparam[0]/problem->numberOfContacts;
