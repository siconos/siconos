...
static int test_dgels(void) {
  ...
  /* Local arrays */
  double a[LDA * N] = {1.44, -9.96, -7.55, 8.34,  7.08,  -5.45, -7.84, -0.28,
                       3.24, 8.09,  2.52,  -5.70, -4.39, -3.24, 6.27,  5.28,
                       0.74, -1.19, 4.53,  3.83,  -6.64, 2.06,  -2.47, 4.70};
  double b[LDB * NRHS] = {8.58, 8.26,  8.48,  -5.28, 5.72,  8.93,
  ...
  print_vector_norm("Residual sum of squares for the solution", m - n, nrhs, &b[n], ldb);
...
static int test_dgesv(void) {
  ...
  /* Local arrays */
  int ipiv[N];
  double a[LDA * N] = {6.80,  -2.11, 5.66,  5.97,  8.23,  -6.05, -3.30, 5.36, -4.44,
                       1.08,  -0.45, 2.58,  -2.70, 0.27,  9.04,  8.32,  2.71, 4.35,
                       -7.17, 2.14,  -9.67, -5.14, -7.26, 6.08,  -6.87};
  double b[LDB * NRHS] = {4.02, 6.19, -8.22, -7.57, -3.03, -1.56, 4.00, -8.67,
...
static int test_dpotrf(void) {
  ...
  int n = N, lda = LDA;
  int info = 0;
  double a[LDA * N] = {2, -1, 0, -1, 2, -1, 0, -1, 2};
...
static int test_dgesvd(void) {
  ...
  double LeftSingularVectors[LDU * N] = {
      -0.59, -0.40, -0.03, -0.43, -0.47, 0.29,  0.26,  0.24,  -0.6,  0.24,
      -0.35, 0.58,  0.36,  -0.22, -0.45, -0.69, 0.39,  -0.02, 0.31,  -0.75,
      0.23,  0.33,  0.16,  0.38,  0.23,  -0.36, -0.31, 0.16,  -0.52, -0.65};

  for (int i = 0; i < LDU * N; ++i)
    if (fabs(u[i] - LeftSingularVectors[i]) > tol) exit(1);

  double RightSingularVectors[LDVT * N] = {
      -0.25, 0.81, -0.26, 0.4,   -0.22, -0.4, 0.36,  0.7,   -0.45, 0.14,  -0.69, -0.25, -0.22,
      0.25,  0.59, -0.37, -0.37, 0.39,  0.43, -0.63, -0.41, -0.1,  -0.49, -0.62, -0.44};
