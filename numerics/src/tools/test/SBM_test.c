...
int SBM_extract_component_3x3_all(void) {
  ...
  size_t col_components_size = 1;
  SBM_extract_component_3x3(M, N, row_components, row_components_size, col_components,
                            col_components_size);
  SBM_print(N);
  ...
  size_t col_components_size_T = 2;
  SBM_extract_component_3x3(M, T, row_components_T, row_components_size_T, col_components_T,
  ...
  SBM_extract_component_3x3(M, NT, row_components_NT, row_components_size_NT,
                            col_components_NT, col_components_size_NT);
  SBM_print(NT);
  ...
  size_t col_components_TN[2] = {0};
  size_t col_components_size_TN = 1;
  SBM_extract_component_3x3(M, TN, row_components_TN, row_components_size_TN,
                            col_components_TN, col_components_size_TN);
  SBM_print(TN);
  SBM_free(TN, SBM_FREE_ALL);
...
static int SBM_gemm_without_allocation_test_2(NumericsMatrix **MM, double alpha, double beta) {
  ...
  dense_gemm_by_hand(alpha, M1->matrix0, M3->matrix0, M1->size0, M3->size1, M1->size1, beta,
                     C2ref->matrix0);
  double err = dense_comparison(C2.matrix0, C2.size0, C2.size1, C2ref->matrix0);
...
static int SBM_gemm_without_allocation_test_4(NumericsMatrix **MM, double alpha, double beta) {
  ...
  if (info == 0) printf("Step 3 ( C = alpha*A*B + beta*C, SBM storage, non square) ok ...\n");
...
static int SBM_gemm_without_allocation_test_5(NumericsMatrix **MM, double alpha, double beta) {
  ...
  double err = dense_comparison(C7->matrix0, C7->size0, C7->size1, C3ref->matrix0);
...
int SBM_gemm_without_allocation_all(void) {
  ...
  func_t funcs[] = {SBM_gemm_without_allocation_test_1, SBM_gemm_without_allocation_test_2,
                    ...
                    SBM_gemm_without_allocation_test_7};

  // The pairs of parameters to be tested
  double params[][2] = {{1., 0.}, {1., 1.}, {0., 1.}, {0.5, 0.5}};
  int nb_params = sizeof(params) / sizeof(params[0]);
  int nb_funcs = sizeof(funcs) / sizeof(funcs[0]);

  for (int i = 0; i < nb_params; i++) {
    double x = params[i][0];
    double y = params[i][1];

    for (int j = 0; j < nb_funcs; j++) {
      info = funcs[j](NMM, x, y);
      if (info != 0) {
...
static int SBM_multiply_test1(double tol) {
  ...
  double beta = 0.0;
  double alpha = 1.0;
  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, m, n, alpha, M_dense, n, M_dense,
              n, beta, C_dense, n);
...
static int SBM_multiply_test2(double tol) {
  ...
  double beta = 0.0;
  double alpha = 1.0;
  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, m, n, alpha, M2_dense, n,
              M10_dense, n, beta, C2_dense, n);
...
static int SBM_multiply_test3(double tol) {
  ...
  double beta = 0.0;
  double alpha = 1.0;
  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, m, n, alpha, M2_dense, n, M4_dense,
              n, beta, C2_dense, n);
...
static void add_initial_value_square_1a(NumericsMatrix *M) {
  ...
  for (i = 0; i < 4; i++) {
    for (j = 4; j < 6; j++) NM_entry(M, i, j, 2.0);
  }
  for (i = 4; i < 6; i++) {
    for (j = 4; j < 6; j++) NM_entry(M, i, j, 3.0);
  }
  for (i = 4; i < 6; i++) {
    for (j = 6; j < 8; j++) NM_entry(M, i, j, 4.0);
  }
  for (i = 6; i < 8; i++) {
    for (j = 0; j < 4; j++) NM_entry(M, i, j, 5.0);
  }
  for (i = 6; i < 8; i++) {
...
static void add_initial_value_square_SBM_1(SparseBlockStructuredMatrix *M) {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 4; j < 6; j++) CHECK_RETURN(SBM_entry(M, i, j, 2.0));
  }
  for (int i = 4; i < 6; i++) {
    for (int j = 4; j < 6; j++) SBM_entry(M, i, j, 3.0);
  }
  for (int i = 4; i < 6; i++) {
    for (int j = 6; j < 8; j++) SBM_entry(M, i, j, 4.0);
  }
  for (int i = 6; i < 8; i++) {
    for (int j = 0; j < 4; j++) SBM_entry(M, i, j, 5.0);
  }
  for (int i = 6; i < 8; i++) {
    for (int j = 6; j < 8; j++) SBM_entry(M, i, j, 6.0);
  }
}
...
static int SBM_entry_test2(double tol) {
  ...
  bool res = SBM_entry(M2->matrix1, 0, 8, 1.0);
  if (res) {  // res must be false, only 8 rows in the matrix.
    NM_free(M2);
    return 1;
  }
  res = SBM_entry(M2->matrix1, 8, 0, 1.0);
  if (res) {  // res must be false, only 8 cols in the matrix.
    NM_free(M2);
    return 1;
  }
  res = SBM_entry(M2->matrix1, 0, 7, 1.0);
  if (res) {  // res must be false, (0,7) corresponds to a null-block
    NM_free(M2);
    return 1;
  }

  res = SBM_entry(M2->matrix1, 0, 2, 1.0);
  NM_free(M2);
  if (!res) return 1;
...
int SBM_entry_all(void) {
  ...
  if (info == 1) {
    printf("========= Ends SBM tests SBM_entry (1) :  Unsuccessfull ========= \n");
    return info;
  }
  info = SBM_entry_test2(tol);
  if (info == 1) {
    printf("========= Ends SBM tests SBM_entry (2) :  Unsuccessfull ========= \n");
    return info;
  }
