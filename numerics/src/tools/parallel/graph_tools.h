 ...
 *  \param[in] M matrix representing the graph to be colored  (M_ij != 0 iff nodes i and j are connected)
 *  \param[out] n_colors number of colors obtained coloring the graph
 *  \param[out] set_sizes array of size n_colors containing the size of each color set
 ...
 *  \param[in] M matrix representing the graph to be colored  (M_ij != 0 iff nodes i and j are connected)
 *  \param[out] n_colors number of colors obtained coloring the graph
 *  \param[out] sum_sizes array of size n_colors + 1 containing the aggregated sizes of each color set, with sum_sizes[0] = 0
 *  \param[out] inv_permutation array of size n such that: color set i = { inv_permutation[j] for sum_sizes[i] <= j < sum_sizes[i + 1] }
 *  \return 0 if succeed
 */
int color_graph_permut(int n, NumericsMatrix *M, size_t *n_colors, size_t **sum_sizes, size_t *inv_permutation);

/** EQUITABLY color a graph and return reordered lines. Equitable means the size difference of two color sets is at most 1.
 *
 *  \param[in] n number of lines of the matrix M
 *  \param[in] M matrix representing the graph to be colored  (M_ij != 0 iff nodes i and j are connected)
 *  \param[out] n_colors number of colors obtained coloring the graph
 ...
 *  \param[in] M matrix representing the graph to be colored (two nodes are connected iff the block of size `nc`x`nc` at i,j is full of zeros) 
 *  \param[out] n_colors number of colors obtained coloring the graph
 *  \param[out] set_sizes array of size n_colors containing the size of each color set
 *  \param[out] set_indices pointers to arrays containing the elements of each color set
 *  \return 0 if succeed
