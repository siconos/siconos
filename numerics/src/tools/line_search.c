...
double line_search_generic(int n, double theta, double preRHS, search_data* ls_data,
                           unsigned searchtype, sn_ls_fn ls_fn) {
  double theta_ref = theta;

  if (ls_data->nm_ref_data) get_non_monotone_ref(ls_data->nm_ref_data, &theta_ref);
  ...
  if (ls_data->nm_ref_data) {
    if (isfinite(alpha))
      ...
      zero_nm_data((nm_ref_struct*)ls_data->nm_ref_data);
  }
...
void update_non_monotone_ref(void* nm_ref_data, double cur_merit) {
  ...
  if (data->m < data->M) {
    ...
    for (int i = 0; i < data->M - 1; ++i)
      data->previous_thetas[i] = data->previous_thetas[i + 1];
