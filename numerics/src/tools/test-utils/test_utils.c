...
void print_test_info(int test_id, TestCase* current, const char* msg) {
  printf("test %i (%s, on %s) %s.\n", test_id,
         solver_options_id_to_name(current->options->solverId), current->filename, msg);
  printf("\n");
}
...
void print_tests_collection_report(TestCase* collection, int n_failed, int* failed_tests,
  ...
  printf("\n Succeeded tests ids: [ ");
  for (int t = 0; t < n_succeeded; t++) printf("%i, ", succeeded_tests[t]);
  printf("] :\n");

  printf("\n Failed tests ids: [ ");
  for (int t = 0; t < n_failed; t++) printf("%i, ", failed_tests[t]);
  printf("] :\n");
  ...
  for (int t = 0; t < n_failed; t++) {
    ...
    if (collection[failed_tests[t]].will_fail == 1)
      ...
      print_test_info(failed_tests[t], &collection[failed_tests[t]],
                      " was expected to succeed");
    else if (collection[failed_tests[t]].will_fail == 2)  // Or is unstable.
      print_test_info(failed_tests[t], &collection[failed_tests[t]],
                      " is unstable and has failed");
  }

  for (int t = 0; t < n_succeeded; t++) {
    if (collection[succeeded_tests[t]].will_fail == 1)
      print_test_info(succeeded_tests[t], &collection[succeeded_tests[t]],
                      " was expected to fail but has succeeded");
    else if (collection[failed_tests[t]].will_fail == 2)  // Or is unstable.
      print_test_info(failed_tests[t], &collection[failed_tests[t]],
                      " is unstable and has suceeded");
  }
}

TestCase* build_test_collection_generic(int n_data, const char** data_collection,
                                        int n_solvers, int* solvers_ids) {
...
int run_test_collection(TestCase* collection, int number_of_tests,
  ...
  for (int test_num = 0; test_num < number_of_tests; ++test_num) {
    ...
    if (info)  // info != 0 --> test is unsuccesful
    {
      ...
      if (collection[test_num].will_fail == 0) out = 1;
    } else
  ...
  // report (on screen)
  print_tests_collection_report(collection, n_failed, failed_tests, n_succeeded,
                                succeeded_tests);
