...
#define DEBUG_BEGIN(M)                     \
  DEBUG_PREFIX;                            \
  if (debug_counter < 24) debug_counter++; \
  DEBUG_INTERNAL_PRINTF("-= BEGIN ==== %s", M)
#define DEBUG_END(M)                      \
  if (debug_counter > 0) debug_counter--; \
  DEBUG_PREFIX;                           \
  DEBUG_INTERNAL_PRINTF("-= END   ==== %s", M)
...
#define DEBUG_PRINTF(_fmt, ...)                                              \
  ({                                                                         \
    DEBUG_PREFIX;                                                            \
    DEBUG_INTERNAL_PRINTF(DEBUG_WHERESTR _fmt, DEBUG_WHEREARG, __VA_ARGS__); \
  })
...
#define DEBUG_PRINT_MAT_SMALL_STR(NAME, M, nrows, ncols, ...)                           \
  ...
  DEBUG_EXPR_WE(double* _arr_[] = {__VA_ARGS__}; for (unsigned i = 0; i < nrows; ++i) { \
    for (unsigned k = 0; k < sizeof(_arr_) / sizeof(double*); ++k) {                    \
      DEBUG_PRINTF(ANSI_COLOR_YELLOW "% 1.0e " ANSI_COLOR_RESET, _arr_[k][i])           \
    }                                                                                   \
    for (unsigned j = 0; j < ncols; ++j) {                                              \
      if (fabs(M[i + j * nrows]) > 2.2e-16) {                                           \
        DEBUG_PRINTF(ANSI_COLOR_YELLOW " % 2.f " ANSI_COLOR_RESET, M[i + j * nrows])    \
      } else {                                                                          \
...
#define DEBUG_PRINT_MAT_ROW_MAJOR(M, nrows, ncols) \
  DEBUG_PRINT_MAT_ROW_MAJOR_STR(#M, M, nrows, ncols)
...
#define DEBUG_PRINT_MAT_ROW_MAJOR_NCOLS(M, nrows, ncols, ncols_to_display) \
  DEBUG_PRINT_MAT_ROW_MAJOR_NCOLS_STR(#M, M, nrows, ncols, ncols_to_display)

#define DEBUG_PRINT_MAT_ROW_MAJOR_NCOLS_SMALL_STR(NAME, M, nrows, ncols, ncols_to_display) \
  DEBUG_PRINT(#NAME " matrix\n");                                                          \
  DEBUG_EXPR_WE(for (unsigned i = 0; i < nrows; ++i) {                                     \
    for (unsigned j = 0; j < ncols_to_display; ++j) {                                      \
      if (fabs(M[i * ncols + j]) > 2.2e-16) {                                              \
        DEBUG_PRINTF(ANSI_COLOR_YELLOW "% 1.0e " ANSI_COLOR_RESET, M[i * ncols + j])       \
      } else {                                                                             \
        DEBUG_PRINT(ANSI_COLOR_BLUE " 0     " ANSI_COLOR_RESET)                            \
      }                                                                                    \
    }                                                                                      \
    DEBUG_PRINT("\n")                                                                      \
  });
...
#define DEBUG_PRINT_MAT_ROW_MAJOR_SMALL_NCOLS(M, nrows, ncols, ncols_to_display) \
  DEBUG_PRINT_MAT_ROW_MAJOR_SMALL_NCOLS_STR(#M, M, nrows, ncols, ncols_to_display)

#define DEBUG_PRINT_VEC_STR(MSG, V, size)            \
  DEBUG_PRINT(MSG " vector\n");                      \
  DEBUG_EXPR_WE(for (unsigned i = 0; i < size; ++i){ \
      DEBUG_PRINTF(ANSI_COLOR_GREEN "% 2.2e " ANSI_COLOR_RESET, V[i])} DEBUG_PRINT("\n"));
...
#define DEBUG_PRINT_VEC_INT_STR(MSG, V, size)        \
  DEBUG_PRINT(MSG " vector\n");                      \
  DEBUG_EXPR_WE(for (unsigned i = 0; i < size; ++i){ \
      DEBUG_PRINTF(ANSI_COLOR_CYAN "%d " ANSI_COLOR_RESET, V[i])} DEBUG_PRINT("\n"));

#define DEBUG_PRINT_VEC_INT(V, size) DEBUG_PRINT_VEC_INT_STR(#V, V, size)

#define DEBUG_OR_VERBOSE(X) \
  if (verbose > 0) {        \
    X;                      \
  } else {                  \
    DEBUG_EXPR_WE(X);       \
