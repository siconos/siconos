...
#include "rolling_fc2d_projection.h"  // for rolling_fc2d_projectio...
...
void rolling_fc2d_projection_update(int contact, RollingFrictionContactProblem* problem,
  ...
  */
  ...
  rolling_fc2d_local_problem_fill_M(problem, localproblem, contact);
  ...
  rolling_fc2d_local_problem_compute_q(problem, localproblem, reaction, contact);
...
int rolling_fc2d_projectionOnCone_solve(RollingFrictionContactProblem* localproblem,
                                        double* reaction, SolverOptions* options) {
  DEBUG_BEGIN("rolling_fc2d_projectionOnCone_solve(...)\n");
...
#ifdef DEBUG_MESSAGES
  display_status_rolling_cone(projectionOn2DRollingCone(reaction, mu_i, mu_r_i));
#else
  projectionOn2DRollingCone(reaction, mu_i, mu_r_i);
#endif

  DEBUG_EXPR(NV_display(reaction, 3););
  DEBUG_END("rolling_fc2d_projectionOnCone_solve(...)\n");
  return 0;
}
...
void rolling_fc2d_projectionOnConeWithLocalIteration_initialize(
  ...
  /* printf("rolling_fc2d_projectionOnConeWithLocalIteration_initialize. Allocation of
   * dwork\n"); */
  if (!localsolver_options->dWork || localsolver_options->dWorkSize < nc) {
    if (localsolver_options
      ...
      unsigned int d_size = nc + 10 * nc;
      localsolver_options->dWork =
          (double*)realloc(localsolver_options->dWork, d_size * sizeof(double));
      localsolver_options->dWorkSize = d_size;
      unsigned int i_size = 3 * nc;
      localsolver_options->iWork =
          (int*)realloc(localsolver_options->iWork, i_size * sizeof(int));
      ...
      localsolver_options->dWork =
          (double*)realloc(localsolver_options->dWork, size * sizeof(double));
  ...
  for (size_t i = 0; i < nc; i++) {
    ...
    if (localsolver_options
        ...
        SICONOS_FRICTION_3D_NSGS_LOCALSOLVER_USE_TRIVIAL_SOLUTION_TRUE) {
      /* The part of MGlobal which corresponds to the current block is copied into MLocal */
      rolling_fc2d_local_problem_fill_M(problem, localproblem, i);
      double* MLocal = localproblem->M->matrix0;
      unsigned int pos = i * 10 + nc;
      double* MLocal_LU = &(localsolver_options->dWork[pos]);
      memcpy(MLocal_LU, MLocal, 9 * sizeof(double));
      int* ipiv = &(localsolver_options->iWork[i * 3]);
      int info = 0;
      DGETRF(3, 3, MLocal_LU, 3, ipiv, &info);
      assert(!info);
    }
  }
};
...
static int rolling_fc2d_check_trivial_solution(unsigned int contact, unsigned int nc,
  ...
  unsigned int pos = contact * 10 + nc;
  double* MLocal_LU = &(localsolver_options->dWork[pos]);
  int* ipiv = &(localsolver_options->iWork[3 * contact]);
  /* double * tmp = localsolver_options->dWork[pos+25]; */
  memcpy(reaction, q, 3 * sizeof(double));
  int info = 0;
  DGETRS(LA_NOTRANS, 3, 1, MLocal_LU, 3, ipiv, reaction, 3, &info);
  assert(!info);
...
int rolling_fc2d_projectionOnConeWithLocalIteration_solve(
  ...
  DEBUG_BEGIN("rolling_fc2d_projectionOnConeWithLocalIteration_solve(...)\n");
  ...
  numerics_printf_verbose(
      2, "--  rolling_fc2d_projectionOnConeWithLocalIteration_solve contact = %i",
      options->iparam[SICONOS_FRICTION_3D_CURRENT_CONTACT_NUMBER]);
  numerics_printf_verbose(2,
                          "--  rolling_fc2d_projectionOnConeWithLocalIteration_solve | "
  ...
  while ((localerror > localtolerance) && (localiter < iparam[SICONOS_IPARAM_MAX_ITER])) {
    ...
    memcpy(reaction_k, reaction, 3 * sizeof(double));
    ...
    while (!success && (ls_iter < ls_itermax)) {
       ...
       * mu_r_i)); */
      /* #else */
      status = projectionOn2DRollingCone(&reaction[0], mu_i, mu_r_i);
      /* #endif */
      /* DEBUG_EXPR(NV_display(reaction,5);); */
      /* velocity <- q  */
    ...
    /* compute local error */
    localerror = a2;
    /* rolling_fc2d_unitary_compute_and_add_error(reaction , velocity, */
    /*                                            mu_i, mu_r_i, */
    /*                                            &localerror, worktmp); */
    /* DEBUG_EXPR(NV_display(reaction,5);); */
    /* DEBUG_EXPR(NV_display(velocity,5);); */
...
static int rfc2d_poc_solve_wrap(void* problem, double* reaction,
                                double* velocity, SolverOptions* options) {
  int info = NUMERICS_OK;
  (void)velocity;
  info = rolling_fc2d_projectionOnCone_solve((RollingFrictionContactProblem*)problem, reaction, options);
  return info;
}

static void rfc2d_poc_free_wrap(void* problem, SolverOptions* options) {
  /* Cleanup if needed */
  (void)problem;
  (void)options;
}
...
static int rfc2d_poc_li_solve_wrap(void* problem, double* reaction,
                                   double* velocity, SolverOptions* options) {
  int info = NUMERICS_OK;
  (void)velocity;
  info = rolling_fc2d_projectionOnConeWithLocalIteration_solve((RollingFrictionContactProblem*)problem, reaction, options);
  return info;
}

static void rfc2d_poc_li_free_wrap(void* problem, SolverOptions* options) {
  /* Cleanup if needed */
  (void)problem;
  (void)options;
}
