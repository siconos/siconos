...
/**
   ...
   iparam[SICONOS_IPARAM_PREALLOC] : bool activate the update in the loop (0:false default
   choice) iparam[SICONOS_VI_IPARAM_DECREASE_RHO] : use rho_k * tau * min(1.0,a2/(rho_k*a1)) to
   decrease rho; commented in the code
...
/**
   Fixed Point Projection solver for variational inequality problem based on the De Saxce
   Formulation
   ...
   iparam[SICONOS_IPARAM_PREALLOC] : bool activate the update in the loop (0:false default
   choice) iparam[SICONOS_VI_IPARAM_DECREASE_RHO] : use rho_k * tau * min(1.0,a2/(rho_k*a1)) to
   decrease rho; commented in the code dparam[SICONOS_VI_DPARAM_RHO] : rho  parameter.
...
/**
   Hyperplane Projection solver for variational inequality problem based on the De Saxce
   Formulation
...
/** VI Solver based on the Newton-Josephy method globalized with a line-search type algorithm
 * using the Qi merit function.
 *
 *  \param problem the variational inequality problem to solve
 *  \param[in,out] x as input, the initial guess; as output the solution if
 *  the algorithm is successful
 ...
 *  \param options the solver options
 */
void vi_box_AVI_LSA(VariationalInequality* problem, double* x, double* F, int* info,
 ...
 *  \param options the solver options
 */
void vi_box_path(VariationalInequality* problem, double* z, double* F, int* info,
                 SolverOptions* options);
...
*/
int checkTrivialCase_vi(VariationalInequality* problem, double* x, double* fx,
                        SolverOptions* options);
