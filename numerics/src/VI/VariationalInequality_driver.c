...
int variationalInequality_driver(VariationalInequality* problem, double* x, double* w,
  ...
  if (info == 0) {
    double error;
    variationalInequality_computeError(problem, x, w, options->dparam[SICONOS_DPARAM_TOL],
                                       options, &error);
  ...
  switch (options->solverId) {
    ...
    case SICONOS_VI_EG: {
      numerics_printf_verbose(1,
                              " ========================== Call ExtraGradient (EG) solver for "
                              "VI problem ==========================\n");
    ...
    case SICONOS_VI_FPP: {
      numerics_printf_verbose(1,
                              " ========================== Call Fixed Point Projection (FPP) "
                              "solver for VI problem ==========================\n");
      variationalInequality_FixedPointProjection(problem, x, w, &info, options);
      break;
    }
    case SICONOS_VI_HP: {
      numerics_printf_verbose(1,
                              " ========================== Call Hyperplane Projection (HP) "
                              "solver for VI problem ==========================\n");
...
int checkTrivialCase_vi(VariationalInequality* problem, double* x, double* w,
                        SolverOptions* options) {
  int n = problem->size;
  ...
  if (nnorm > fmin(options->dparam[SICONOS_DPARAM_TOL], 1e-12)) return 1;
