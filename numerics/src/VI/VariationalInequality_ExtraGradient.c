...

{
  ...
  if (error < *tolerance) {
    if (verbose > 0)
      printf(
          "--------------- VI - Extra Gradient (EG) - Iteration %i "
          "Residual = %14.7e < %7.3e\n",
    ...
    if (options->iparam[SICONOS_VI_IPARAM_ERROR_EVALUATION] ==
      ...
      if (absolute_error > options->dparam[SICONOS_DPARAM_TOL]) {
        ...
        *tolerance = error / absolute_error * options->dparam[SICONOS_DPARAM_TOL];
        if (verbose > 0)
          printf(
              "--------------  VI - Extra Gradient (EG)- We modify the required incremental "
              "precision to reach accuracy to %e\n",
              *tolerance);
        hasNotConverged = 1;
      } else {
        if (verbose > 0)
          printf(
              "-------------- VI - Extra Gradient (EG) - The incremental precision is "
              "sufficient to reach accuracy to %e\n",
              *tolerance);
      }
    }
  } else {
    if (verbose > 0)
      printf(
          "--------------- VI - Extra Gradient (EG) - Iteration %i rho = %14.7e error = "
          "%14.7e > %10.5e \n",
...
void variationalInequality_ExtraGradient(VariationalInequality *problem, double *x, double *w,
  ...
  if (dparam[SICONOS_VI_DPARAM_RHO] > 0.0) {
    ...
    if (verbose > 0) {
      printf(
          "--------------- VI - Extra Gradient (EG) - Variable stepsize with starting rho = "
          "%14.7e \n",
          rho);
    }
  }
  ...
  *info = hasNotConverged;
  if (verbose > 0) {
    printf(
        "--------------- VI - Extra Gradient (EG) - #Iteration %i Final Residual = %14.7e\n",
        iter, error);
  }
...
void variationalInequality_ExtraGradient_set_default(SolverOptions *options) {
  ...
  options->iparam[SICONOS_VI_IPARAM_DECREASE_RHO] =
      0;  // use rho_k * tau * min(1.0,a2/(rho_k*a1)) to decrease rho; commented in the code
  options->iparam[SICONOS_VI_IPARAM_ERROR_EVALUATION] =
      SICONOS_VI_ERROR_EVALUATION_LIGHT_WITH_FULL_FINAL;
  options->iparam[SICONOS_VI_IPARAM_ERROR_EVALUATION_FREQUENCY] =
      0;  // Rk FP : set but not used
  options->iparam[SICONOS_VI_IPARAM_ACTIVATE_UPDATE] =
      0;  // activate the update in the loop (0:false default choice)
