 ...
 */
#include "mc2d_onecontact_nonsmooth_Newton_solvers.h"  // for computeNonsmoo...
...
static void mc2d_AC_initialize(MohrCoulomb2DProblem* problem,
  ...
  if (options->iparam[PLASTICITY_NSN_FORMULATION] ==
    ...
    Function = &(mc2d_computeAlartCurnierSTD);
             ...
             PLASTICITY_NSN_FORMULATION_JEANMOREAU_STD) {
    Function = &(mc2d_computeAlartCurnierJeanMoreau);
  } else if (options->iparam[PLASTICITY_NSN_FORMULATION] ==
             PLASTICITY_NSN_FORMULATION_NULL) {
    Function = NULL;
  }
  ...
  if (options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN ||
      options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN_GP) {
  ...
  } else if (options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN_GP_HYBRID) {
  ...
  for (size_t contact = 0; contact < nc; contact++) {
    ...
    numerics_printf(
        "mc2d_AC_initialize "
        " mc2d_compute rho for contact = %i",
        contact);
...
void mc2d_onecontact_nonsmooth_Newton_solvers_initialize(MohrCoulomb2DProblem* problem,
  ...
  if(localsolver_options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN||
     localsolver_options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN_GP ||
     localsolver_options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN_GP_HYBRID)
  {
    mc2d_AC_initialize(problem, localproblem,localsolver_options);
  }
  else
  {
    numerics_error("mc2d_onecontact_nonsmooth_Newton_solvers_initialize", "Unknown formulation type.");
  }
}
...
int mc2d_onecontact_nonsmooth_Newton_solvers_solve(MohrCoulomb2DProblem* localproblem,
  ...
  numerics_printf_verbose(
      2, "--------------- mc2d_onecontact_nonsmooth_Newton_solvers_solve starts");
  ...
  if (q[0] > -DBL_EPSILON) {
    ...
    numerics_printf_verbose(
        2, "--------------- mc2d_onecontact_nonsmooth_Newton_solvers_solve ends");
    return info;
  }

  if (options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN) {
    info = mc2d_onecontact_nonsmooth_Newton_solvers_solve_direct(localproblem, local_reaction,
                                                                 options);
  } else if (options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN_GP) {
    info = mc2d_onecontact_nonsmooth_Newton_solvers_solve_damped(localproblem, local_reaction,
                                                                 options);
  } else if (options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN_GP_HYBRID) {
    if (options->iparam[PLASTICITY_NSN_HYBRID_STRATEGY] ==
      ...
      info = mc2d_onecontact_nonsmooth_Newton_solvers_solve_hybrid(localproblem,
                                                                   local_reaction, options);
    } else {
      numerics_error("mc2d_onecontact_nonsmooth_Newton_solvers_solve",
  ...
  if (info > 0) {
    if (verbose > 0) {
      if (options->iparam[SICONOS_IPARAM_MAX_ITER] ==
        ...
        numerics_warning(
            "mc2d_onecontact_nonsmooth_Newton_solvers_solve",
        ...
        numerics_warning("mc2d_onecontact_nonsmooth_Newton_solvers_solve",
...
void mc2d_onecontact_nonsmooth_Newton_solvers_computeError(int n, double* velocity,
...
#ifdef DEBUG_CHECK
static int mc2d_onecontact_nonsmooth_Newton_AC_debug(double* R, double* velocity, double mu,
                                                     double* rho, double* MLocal, double* F,
                                                     double* A, double* B, double* AWplusB,
                                                     int* iparam) {
  double AWpB[9];
  if (iparam[PLASTICITY_NSN_FORMULATION] !=
          PLASTICITY_NSN_FORMULATION_JEANMOREAU_GENERATED &&
      iparam[PLASTICITY_NSN_FORMULATION] !=
          PLASTICITY_NSN_FORMULATION_NULL) {
    double Fg[3] = {0., 0., 0.};
...
void mc2d_onecontact_nonsmooth_Newton_AC_update(int contact, MohrCoulomb2DProblem* problem,
                                                MohrCoulomb2DProblem* localproblem,
...
int mc2d_onecontact_nonsmooth_Newton_solvers_solve_direct(MohrCoulomb2DProblem* localproblem,
  ...
  numerics_printf_verbose(
      2, "--------------- mc2d_onecontact_nonsmooth_Newton_solvers_solve_direct starts");
  ...
  if (options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN ||
      options->solverId == MOHR_COULOMB_2D_ONECONTACT_NSN_GP) {
    rho = &options->dWork[3 * iparam[PLASTICITY_CURRENT_CONTACT_NUMBER]];
  } else {
    int nc = options->dWorkSize / 4;
    rho = &options->dWork[3 * iparam[PLASTICITY_CURRENT_CONTACT_NUMBER] + nc];
  }
  ...
  for (inew = 0; inew < itermax; ++inew) {
...
#ifdef DEBUG_CHECK
    mc2d_onecontact_nonsmooth_Newton_AC_debug(R, velocity, mu, rho, MLocal, F, A, B, AWplusB,
                                              iparam);
#endif
    ...
    if (info_solv3x3) {
      if (verbose > 0)
        numerics_warning("mc2d_onecontact_nonsmooth_Newton_solvers_solve_direct",
                         "contact %i 3x3 linear system is irregular # iteration = %"
                         " error = %.10e \n",
                         iparam[PLASTICITY_CURRENT_CONTACT_NUMBER], inew,
    ...
    if (verbose > 1)
      printf(
          "---------------    mc2d_onecontact_nonsmooth_Newton_solvers_solve_direct  -- "
          "contact %i # iteration = %i  error = %.10e \n",
          iparam[PLASTICITY_CURRENT_CONTACT_NUMBER], inew,
          dparam[SICONOS_DPARAM_RESIDU]);
...
static int LineSearchGP(MohrCoulomb2DProblem* localproblem,
...
int mc2d_onecontact_nonsmooth_Newton_solvers_solve_damped(MohrCoulomb2DProblem* localproblem,
  ...
  numerics_printf_verbose(
      2, "--------------- mc2d_onecontact_nonsmooth_Newton_solvers_solve_damped starts");
  ...
  for (inew = 0; inew < itermax; ++inew) {
    ...
    if (info_solv3x3) {
      if (verbose > 0)
        numerics_warning("mc2d_onecontact_nonsmooth_Newton_solvers_solve_damped",
                         "contact %i 3x3 linear system is irregular # iteration = %"
                         " error = %.10e \n",
                         iparam[PLASTICITY_CURRENT_CONTACT_NUMBER], inew,
                         dparam[SICONOS_DPARAM_RESIDU]);
    ...
    numerics_printf_verbose(2, "-- contact %i # iteration = %i  error = %.10e",
                            iparam[PLASTICITY_CURRENT_CONTACT_NUMBER], inew,
                            dparam[SICONOS_DPARAM_RESIDU]);
...
int mc2d_onecontact_nonsmooth_Newton_solvers_solve_hybrid(MohrCoulomb2DProblem* localproblem,
                                                          ...
                                                          SolverOptions* options) {
  numerics_printf_verbose(
      2, "--------------- mc2d_onecontact_nonsmooth_Newton_solvers_solve_hybrid starts");
  ...
  DEBUG_PRINTF("mc2d_onecontact_nonsmooth_Newton_solvers_solve_hybrid current_error= %e\n",
               current_error);
  ...
  if (!(options->iparam[PLASTICITY_NSN_HYBRID_STRATEGY] ==
        ...
        options->iparam[PLASTICITY_NSN_HYBRID_STRATEGY] ==
            PLASTICITY_NSN_HYBRID_STRATEGY_NSN_AND_PLI_NSN_LOOP)) {
    numerics_error("mc2d_onecontact_nonsmooth_Newton_solvers_solve_hybrid",
                   "Unknown local nsn hybrid solver");
  }
  ...
  if (options->iparam[PLASTICITY_NSN_HYBRID_STRATEGY] ==
    ...
    if (current_error <= options->dparam[SICONOS_DPARAM_TOL]) {
      ...
      DEBUG_END("mc2d_onecontact_nonsmooth_Newton_solvers_solve_hybrid\n");
      return info;
    }
  }
  ...
  if (options->iparam[PLASTICITY_NSN_HYBRID_STRATEGY] ==
    ...
    while (loop < max_loop && current_error >= options->dparam[SICONOS_DPARAM_TOL]) {
      loop++;
      DEBUG_PRINTF("PLASTICITY_ONECONTACT_NSN_GP_HYBRID:  loop = %i\n", loop);
      ...
      options->iparam[SICONOS_IPARAM_MAX_ITER] = newton_iteration_number;
      info = mc2d_onecontact_nonsmooth_Newton_solvers_solve_damped(localproblem,
                                                                   local_reaction, options);
      DEBUG_PRINTF("NSN solver  ended with residual = %e\n",
                   options->dparam[SICONOS_DPARAM_RESIDU]);
    ...
    if (loop == max_loop && max_loop != 1) {
      if (verbose > 0) {
        printf(
            "Maximum number of loop (%i) in PLASTICITY_ONECONTACT_NSN_GP_HYBRID has "
            "been reached for contact %i with error = %e \n",
            max_loop, options->iparam[PLASTICITY_CURRENT_CONTACT_NUMBER],
            current_error);
      }
    }
  }
  ...
  options->dparam[SICONOS_DPARAM_RESIDU] = current_error;
  DEBUG_PRINTF("mc2d_onecontact_nonsmooth_Newton_solvers_solve_hybrid END current_error= %e\n",
               current_error);
  DEBUG_END("mc2d_onecontact_nonsmooth_Newton_solvers_solve_hybrid\n")
  return info;
}
...
void mc2d_onecontact_nsn_gp_set_default(SolverOptions* options) {
  ...
  options->iparam[PLASTICITY_NSN_FORMULATION] =
      PLASTICITY_NSN_FORMULATION_ALARTCURNIER_STD;
  /* Choice of line -search method */
  options->iparam[PLASTICITY_NSN_LINESEARCH] =
      PLASTICITY_NSN_LINESEARCH_GOLDSTEINPRICE;
