...
extern void plasticity_2d_set_internalsolver_tolerance(PlasticityProblem* problem,
...
static PlasticityProblem* plasticity_2d_nsgs_local_problem_new(PlasticityProblem* problem,
  ...
  if (problem->model_type == PLASTICITY_MODEL_DRUCKER_PRAGER) {
    switch (localsolver_options->solverId) {
      case PLASTICITY_2D_ONECONE_ProjectionOnCone:
        ...
        if (!localsolver_options->dWork ||
            localsolver_options->dWorkSize < to_size_t(problem->numberOfCones)) {
          localsolver_options->dWork = (double*)realloc(
              localsolver_options->dWork, problem->numberOfCones * sizeof(double));
      ...
      default:
        ...
        numerics_error("plasticity_2d_nsgs_local_problem_new", "Unknown internal solver: %s",
    ...
    switch (localsolver_options->solverId) {
      ...
      default:
        plasticity_2d_local_problem_free(localproblem, problem);
        numerics_error("plasticity_2d_nsgs_local_problem_new",
    ...
    plasticity_2d_local_problem_free(localproblem, problem);
    numerics_error("plasticity_2d_nsgs_local_problem_new", "Unknown plasticity model type");
    return NULL;
  }
...
static void plasticity_2d_nsgs_update_wrapper(unsigned int block, void* problem,
                                              void* local_problem, double* var_z,
                                              SolverOptions* options) {
  if (!local_problem) {
    numerics_error("plasticity_2d_nsgs_update_wrapper", "Local problem is NULL");
    return;
  }
  PROFILE_START();
  plasticity_2d_nsgs_update(block, (PlasticityProblem*)problem,
...
static void plasticity_2d_nsgs_solve_local_wrapper(void* local_problem,
                                                   SolverOptions* localsolver_options,
                                                   double* var_z_local,
  ...
  if (local_solver && local_problem) {
    ...
    numerics_error("plasticity_2d_nsgs_solve_local_wrapper",
...
static void plasticity_2d_nsgs_accept_local_wrapper(void* local_problem,
                                                    SolverOptions* options, unsigned int block,
                                                    int iter, double* var_z_global,
                                                    double* var_z_local) {
  (void)local_problem;
  (void)iter;
...
void plasticity_2d_nsgs_generic(PlasticityProblem* problem, double* stress,
  ...
  if (!problem || !stress || !info || !options) {
    numerics_error("plasticity_2d_nsgs_generic", "Invalid input arguments");
    return;
  }

  if (options->numberOfInternalSolvers < 1) {
    numerics_error("plasticity_2d_nsgs_generic",
  ...
  void* original_matrix1 =
      nsgs_generic_extract_diagonal_blocks(problem->M, problem->dimension);

  /* Create local problem and get local solver function */
  SolverPtr local_solver = NULL;
  PlasticityProblem* localproblem =
      plasticity_2d_nsgs_local_problem_new(problem, options, &local_solver);
  if (!localproblem) {
    *info = 1;
    return;
  }
  ...
  NSGSProblemData problem_data = {.nb_blocks = problem->numberOfCones,
                                  .q = problem->q,
                                  .M = problem->M,
  ...
  NSGSLocalToolkit toolkit = {
      ...
      .use_relaxation =
          options->iparam[PLASTICITY_NSGS_RELAXATION] == PLASTICITY_NSGS_RELAXATION_TRUE,
...
REGISTER_SOLVER(PLASTICITY_2D_NSGS_GENERIC, "PLASTICITY_2D_NSGS_GENERIC",
                ...
                plasticity_2d_nsgs_generic_init_wrap, plasticity_2d_nsgs_generic_solve_wrap,
                plasticity_2d_nsgs_generic_free_wrap, NULL, /* error function */
