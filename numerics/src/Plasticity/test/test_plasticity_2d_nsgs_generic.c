...
int main(int argc, char** argv) {
  ...
  printf("  Model type: %s\n",
         ...
         : problem->model_type == PLASTICITY_MODEL_VON_MISES    ? "Von Mises"
                                                                : "Unknown");
  ...
  int info_generic;
  plasticity_2d_nsgs_generic(problem, stress_generic, strain_generic, &info_generic,
                             options_generic);
  ...
  for (int i = 0; i < 9 && i < n; i++) {
    printf("  [%d] legacy=%12.8e  generic=%12.8e  diff=%12.8e\n", i, stress_legacy[i],
           stress_generic[i], fabs(stress_legacy[i] - stress_generic[i]));
  }
