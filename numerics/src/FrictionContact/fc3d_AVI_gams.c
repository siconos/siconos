...
static int FC3D_gams_inner_loop_condensed(
    unsigned iter, idxHandle_t Xptr, gamsxHandle_t Gptr, optHandle_t Optr, gmoHandle_t gmoPtr,
    char* sysdir, char* model, const char* base_name, double* restrict reaction,
    double* restrict velocity, double* restrict tmpq, double* restrict lambda_r,
    double* restrict lambda_y, NumericsMatrix* W, double* restrict q, NumericsMatrix* Wtmat,
    NumericsMatrix* Emat, NumericsMatrix* Akmat, SolverOptions* options) {
  char msg[GMS_SSSIZE];
  int status;
  ...
  if ((status =
           NV_to_GDX(Xptr, "guess_lambda_r", "guess for lambda_r", lambda_r, Akmat->size0))) {
    printf("Model data not written\n");
    infos[1] = (double)-ETERMINATE;
    goto fail;
  }
  ...
  if ((status =
           NV_to_GDX(Xptr, "guess_lambda_y", "guess for lambda_y", lambda_y, Akmat->size0))) {
    printf("Model data not written\n");
    infos[1] = (double)-ETERMINATE;
    goto fail;
  }
...
static int fc3d_AVI_gams_base(FrictionContactProblem* problem, double* reaction,
  ...
  int status;
  char sysdir[GMS_SSSIZE], model[GMS_SSSIZE], msg[GMS_SSSIZE], template_filename[GMS_SSSIZE],
      hdf5_filename[GMS_SSSIZE], log_filename[GMS_SSSIZE], base_filename[GMS_SSSIZE];
  ...
  Akmat.size1 = size;
  Akmat.matrix2->triplet = cs_spalloc(NB_APPROX * problem->numberOfContacts, size,
                                      NB_APPROX * problem->numberOfContacts * 3, 1, 1);
  ...
  while (!done && (iter < maxiter)) {
    iter++;
    total_residual = 0.;
    filename_datafiles(iter, options->solverId, base_filename, sizeof(template_filename),
                       template_filename, log_filename);
    ...
    int solverStat = FC3D_gams_inner_loop_condensed(
        iter, Xptr, Gptr, Optr, gmoPtr, sysdir, model, template_filename, reaction, velocity,
        tmpq, lambda_r, lambda_y, problem->M, problem->q, &Wtmat, &Emat, &Akmat, options);
    ...
    SN_logh5_vec_double(problem->numberOfContacts, predicted_angles, "predicted_angles",
                        logger_s->group);
    ...
    for (unsigned i3 = 0, i = 0; i3 < size; ++i, i3 += 3) {
      ...
      //      rad2deg(atan2(reaction[i3+2], reaction[i3+1])));
    ...
    for (unsigned i3 = 0, i = 0; i3 < size; ++i, i3 += 3) {
      ...
      DEBUG_PRINTF("Contact %d, old r = [%.*e; %.*e; %.*e]\n", i, DECIMAL_DIG,
                   reaction_old[i3 + 0], DECIMAL_DIG, reaction_old[i3 + 1], DECIMAL_DIG,
                   reaction_old[i3 + 2]);
      DEBUG_PRINTF("Contact %d, new r = [%.*e; %.*e; %.*e]\n", i, DECIMAL_DIG, ri[0],
                   DECIMAL_DIG, ri[1], DECIMAL_DIG, ri[2]);
      DEBUG_PRINTF("Contact %d, del r = [%.*e; %.*e; %.*e]\n", i, DECIMAL_DIG,
                   reaction_old[i3 + 0] - ri[0], DECIMAL_DIG, reaction_old[i3 + 1] - ri[1],
                   DECIMAL_DIG, reaction_old[i3 + 2] - ri[2]);
       ...
       * the boundaries)*/
      // if ((res < TOL2) && ((ri[0] < TOL_RN) || ((ri[1]*ri[1] + ri[2]*ri[2]) <
      // (1.-10*DBL_EPSILON)*mu*mu * ri[0]*ri[0])))
      if (false) {
      ...
      //      else if (ri[0] > TOL_RN) // if r= 0 :(
        ...
        DEBUG_PRINTF("contact %d, delta angle = %g, theta_r = %.*e, theta_u = %.*e\n", i,
                     rad2deg(delta_angle), DECIMAL_DIG, rad2deg(atan2(ri[2], ri[1])),
        ...
        double middle_point[] = {(cos(minus_r_angle) + cos(angle)) / 2.,
                                 (sin(minus_r_angle) + sin(angle)) / 2.};

#ifdef SMALL_APPROX
        cs_entry(Ak_triplet, p + offset_row, i3,
                 -mu * (hypot(middle_point[0], middle_point[1])));
        ...
        {
          cs_entry(Ak_triplet, p + offset_row, i3 + 1,
                   cos(minus_r_angle - M_PI / 2)); /* XXX there are formulas for this ... */
        ...
        DEBUG_PRINTF("contact %d, lambda_r = %g; lambda_y = %g\n", i,
                     lambda_r[offset_row_bck + pos], lambda_y[offset_row_bck + pos]);
        DEBUG_PRINTF("contact %d, ui = [%g; %g; %g]\n", i, ui[0], ui[1], ui[2]);
        DEBUG_PRINTF("contact %d, ui_res = [%g; %g; %g]\n", i,
                     -lambda_r[offset_row_bck + pos] * mu +
                         (ui[0] + mu * lambda_y[offset_row_bck + pos]),
                     -lambda_r[offset_row_bck + pos] * cos(angle_pos) + ui[1],
                     -lambda_r[offset_row_bck + pos] * sin(angle_pos) + ui[2]);
        double norm_rt = ri[0] * mu;
        DEBUG_PRINTF("contact %d, yi_res = [%g; %g; %g]\n", i,
                     -lambda_y[offset_row_bck + pos] * mu +
                         ((-ui[1]) * ri[1] + (-ui[2]) * ri[2]) / ri[0],
                     -lambda_y[offset_row_bck + pos] * cos(angle_pos) + ui[1],
                     -lambda_y[offset_row_bck + pos] * sin(angle_pos) + ui[2]);
        /* now we write the estimate for y here */
        ui[0] = ((-ui[1]) * ri[1] + (-ui[2]) * ri[2]) / ri[0];
        ui[1] = ui[0] * ri[1] / ri[0];
        ui[2] = ui[0] * ri[2] / ri[0];
        DEBUG_PRINTF("new y[i] = %g %g %g\n", ui[0], ui[1], ui[2]);
        DEBUG_PRINTF("new \bar{y}[i] = %g %g %g\n", ui[0] * mu / ui[0], ui[1] * mu / ui[0],
                     ui[2] * mu / ui[0]);
        DEBUG_PRINTF(" ||y_t|| = %g <= %g = mu*y_n; diff = %g\n",
                     sqrt(ui[1] * ui[1] + ui[2] * ui[2]), ui[0] * mu,
                     ui[0] * mu - sqrt(ui[1] * ui[1] + ui[2] * ui[2]));
        DEBUG_PRINTF(
            "contact %d, row entry %d, last_entry: angle = %g, norm = %g; coeff = %g, %g, "
            "%g\n",
            i, offset_row, rad2deg(atan2(middle_point[1], middle_point[0])),
            hypot(middle_point[0], middle_point[1]),
            -mu * hypot(middle_point[0], middle_point[1]), -middle_point[0], -middle_point[1]);
        double xx[] = {1,
                       -middle_point[0] / ((1 + hypot(middle_point[0], middle_point[1])) / 2),
                       -middle_point[1] / ((hypot(middle_point[0], middle_point[1]) + 1) / 2)};
        xtmp[i3] = 1. / mu;
        xtmp[i3 + 1] = xx[1];
        xtmp[i3 + 2] = xx[2];
        /*         Akmat.size0 = offset_row + p + 1;
                DEBUG_PRINTF("contact %d, checking feasibility, Ak size = (%d,%d)\n", i,
           Akmat.size0, Akmat.size1); double* xtmp2 = (double*)calloc(Akmat.size0,
           sizeof(double)); double* pts = (double*)calloc(size, sizeof(double)); xtmp[i3]
           = 1./mu; xtmp[i3+1] = xx[1]; xtmp[i3+2] = xx[2]; DEBUG_PRINT_VEC(xtmp, size);
      ...
      bad_angle: {
        ...
        if (ri[0] < TOL_RN) {
          ...
          DEBUG_PRINTF("new y[i] = %g %g %g\n", ui[0], ui[1], ui[2]);
        } else if ((ri[1] * ri[1] + ri[2] * ri[2]) <
                   (1. + 1e-10) * mu * mu * ri[0] *
                       ri[0]) /* We should have r \in int K and u = y = 0 = dual(y) */
        {
    ...
    for (unsigned ii = 0; ii < Akmat.size0; ++ii) {
      if (xtmp2[ii] < -DBL_EPSILON)
        printf("constraint violation at row %d, value %g\n", ii, xtmp2[ii]);
    }
    free(xtmp);
    free(xtmp2);
    ...
    //    optSetDblStr(solverOptPtr, "expand_delta", fmax(1e-13, fmin(1e-10,
    //    total_residual*1e-7)));
    optWriteParameterFile(solverOptPtr, msg);
    //    optSetDblStr(solverOptPtr, "convergence_tolerance", 1e-12);
    ...
    SN_logh5_vec_double(problem->numberOfContacts, delta_angles, "delta_angles",
                        logger_s->group);
    SN_logh5_vec_double(problem->numberOfContacts, real_angles, "real_angles",
                        logger_s->group);
    SN_logh5_vec_double(problem->numberOfContacts, residual_contact, "residual_contact",
                        logger_s->group);
    /*  XXX buggy here, implement a SN_logh5_vec_integer */
    SN_logh5_vec_uint64(problem->numberOfContacts, type_contact, "type_contact",
                        logger_s->group);
    SN_logh5_vec_uint64(problem->numberOfContacts, type_contact_avi, "type_contact_avi",
                        logger_s->group);
...
TERMINATE:
  ...
  free(delta_angles);  // status = fc3d_compute_error(problem, reaction, velocity,
                       // options->dparam[SICONOS_DPARAM_TOL], options,
...
static int fc3d_gams_path_solve_wrap(void* problem, double* reaction, double* velocity,
...
REGISTER_SOLVER(FC3D_GAMS_PATH, "FC3D_GAMS_PATH",
                "GAMS PATH solver for 3D Friction Contact (AVI formulation)",
                NULL,                             /* init_wrap */
                fc3d_gams_path_solve_wrap, NULL,  /* free_wrap */
                NULL,                             /* err_fn */
                fc3d_gams_path_set_default, 1000, /* default_max_iter */
                1e-6,                             /* default_tol */
                0)                                /* is_local_solver */

static int fc3d_gams_pathvi_solve_wrap(void* problem, double* reaction, double* velocity,
                                       SolverOptions* options) {
  int info = NUMERICS_OK;
...
REGISTER_SOLVER(FC3D_GAMS_PATHVI, "FC3D_GAMS_PATHVI",
                "GAMS PATHVI solver for 3D Friction Contact (AVI formulation)",
                NULL,                               /* init_wrap */
                fc3d_gams_pathvi_solve_wrap, NULL,  /* free_wrap */
                NULL,                               /* err_fn */
                fc3d_gams_pathvi_set_default, 1000, /* default_max_iter */
                1e-6,                               /* default_tol */
                0)                                  /* is_local_solver */
