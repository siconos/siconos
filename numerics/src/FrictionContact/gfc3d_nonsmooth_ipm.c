               ...
               int* restrict info, SolverOptions* restrict options, const char* problem_name)
{
  ...
  double *dz = (double*)calloc(nd,sizeof(double));
  ...
  double *z_plus_dz = data->tmp_vault_nd[4];
  ...
  numerics_printf_verbose(-1, "| it  |  dualgap | pinfeas | dinfeas |  <z, r> | complem | prj err | barpram | alpha_p | alpha_d |  sigma  |  |dv|   |  |dz|   |  |dr|   | ls prim | ls dual | ls comp |");
  ...
  while(iteration < max_iter)
  {
    ...
    */
    ...
    projerr = projectionError(z, reaction, n, tol);
    ...
    if ( totalresidual <= tol )
    {
      ...
      for (int i = 0; i < n; i++)
      {
      	unitur = cblas_ddot(3, z+3*i, 1, reaction+3*i, 1);
      	if (unitur<0)
    ...
    // u_plus_phiu = u + phi(u) - z
    for(unsigned int i = 0; i < nd; ++ i)
    {
      if(i % d == 0)
      {
        u_plus_phiu[i] = velocity[i] + sqrt(velocity[i+1]*velocity[i+1]+velocity[i+2]*velocity[i+2]) - z[i];
      }
      u_plus_phiu[i] = velocity[i] - z[i];
    }
     ...
     * Case of a non-reduced system. Building the right-hand side related to the first system
       ...
       [           z o r         ]  nd        complemConstraint
       [  (u-Hv-w) - (I - phi'(u))(u+phi(u)-z)  ]  nd        primalConstraint
    */
    ...
    case SICONOS_FRICTION_3D_IPM_IPARAM_LS_3X3_NOSCAL:
    {
      ...
      NumericsMatrix * arrow_z = Arrow_repr(z, nd, n) ;
      ...
      NM_insert(J, eye_phiu, m + nd, m);
      NM_insert(J, minus_Ht, 0, m + nd);
      NM_insert(J, arrow_z, m, m + nd);
      ...
      JA_prod(z, reaction, nd, n, complemConstraint);

      // Modify primalConstraint = (u-Hv-w) - (I - phi'(u))(u+phi(u)-z) (before: primalConstraint = u-Hv-w)
      NM_gemv(-1.0, eye_phiu, u_plus_phiu, 1.0, primalConstraint);
    ...
    // alpha_primal = getStepLength(z, dz, nd, n, 1.0); //gmm);
    ...
    cblas_dcopy(nd, z, 1, z_plus_dz, 1);
    cblas_dcopy(nd, reaction, 1, r_plus_dr, 1);
    cblas_daxpy(nd, alpha_primal, dz, 1, z_plus_dz, 1);
    cblas_daxpy(nd, alpha_dual, d_reaction, 1, r_plus_dr, 1);

    /* affine barrier parameter */
    barr_param_a = cblas_ddot(nd, z_plus_dz, 1, r_plus_dr, 1) / n / 3;
    ...
    /*      without NT scaling */
    /*      rhs = - */
    /*      [               Mv - H'r - f               ]  m         dualConstraint */
    /*      [ z o r + da^z o da^r - 2 * sigma * mu * e ]  nd        complemConstraint */
    /*      [   (u-Hv-w) - (I - phi'(u))(u+phi(u)-z)   ]  nd        primalConstraint */
    ...
    numerics_printf_verbose(-1, "| %3i%c| %8.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e | %.1e |",
                            ...
                            fabs(dz[cblas_idamax(nd, dz, 1)]),
                            fabs(d_reaction[cblas_idamax(nd, d_reaction, 1)]),
			    LS_norm_p, LS_norm_d, LS_norm_c);

    /* ----- Update variables ----- */

    if (NV_isnan(d_globalVelocity, m) | NV_isnan(d_velocity, nd) | NV_isnan(d_reaction, nd) | NV_isnan(dz, nd))
    {
      hasNotConverged = 2;
      break;
    }
    ...
    cblas_daxpy(nd, alpha_primal, dz, 1, z, 1);


    if (NV_isnan(globalVelocity, m) | NV_isnan(velocity, nd) | NV_isnan(reaction, nd) | NV_isnan(z, nd))
    {
      hasNotConverged = 2;
      break;
    }
  ...
  if(Ht) Ht = NM_free(Ht);
  if(eye_phiu) eye_phiu = NM_free(eye_phiu);
