...
void rolling_fc3d_unitary_compute_and_add_error(double* restrict r, double* restrict u,
                                                double mu, double mur, double* restrict error,
                                                double* worktmp) {
...
int rolling_fc3d_compute_error(RollingFrictionContactProblem* problem, double* reaction,
  ...
  NM_gemv(1.0, problem->M, reaction, 1.0, velocity);
  ...
  *error = 0.;
  ...
  for (ic = 0, ic5 = 0; ic < nc; ic++, ic5 += 5) {
    rolling_fc3d_unitary_compute_and_add_error(reaction + ic5, velocity + ic5, mu[ic], mur[ic],
                                               error, worktmp);
