...
int rolling_fc2d_driver(RollingFrictionContactProblem* problem, double* reaction,
  ...
  if (options == NULL) numerics_error("rolling_fc2d_driver", "null input for solver options");
  ...
  switch (options->solverId) {
    ...
    case SICONOS_ROLLING_FRICTION_2D_NSGS: {
      numerics_printf(
          " ========================== Call NSGS solver for Rolling Friction-Contact 2D "
          "problem ==========================\n");
...
int rolling_fc2d_checkTrivialCase(RollingFrictionContactProblem* problem, double* velocity,
  ...
  numerics_printf(
      "rolling_fc2d rolling_fc2d_checkTrivialCase, take off, trivial solution reaction = 0, "
      "velocity = q.\n");
  return 0;
}
