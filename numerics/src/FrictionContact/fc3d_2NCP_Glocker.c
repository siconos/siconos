...
void computeMGlocker() {
  ...
  /* row 1 */
  MGlocker[1] = -MLocal[2] - sqrt(3.) / 3 * MLocal[1] - 2 * mu_i * MLocal[8] -
                2 * sqrt(3.) / 3 * mu_i * MLocal[7];
  MGlocker[Gsize + 1] =
      MLocal[8] + 1. / 3 * MLocal[4] + sqrt(3.) / 3 * (MLocal[5] + MLocal[7]);
  MGlocker[2 * Gsize + 1] =
      MLocal[8] - 1. / 3 * MLocal[4] - sqrt(3.) / 3 * (MLocal[5] - MLocal[7]);
  ...
  /* row 2 */
  MGlocker[2] = -MLocal[2] + sqrt(3.) / 3 * MLocal[1] - 2 * mu_i * MLocal[8] +
                2 * sqrt(3.) / 3 * mu_i * MLocal[7];
  MGlocker[Gsize + 2] =
      MLocal[8] - 1. / 3 * MLocal[4] + sqrt(3.) / 3 * (MLocal[5] - MLocal[7]);
  MGlocker[2 * Gsize + 2] =
  ...
  /* row 4 */
  for (i = 0; i < Gsize; ++i) MGlocker[i * Gsize + 4] = 0.0;
}
...
void computeGGlocker() {
  ...
  FGlocker[1] += 4. / 3 * reactionGlocker[4] *
                 (2 * reactionGlocker[1] + reactionGlocker[2] - 3 * mu_i * reactionGlocker[0]);
  FGlocker[2] += 4. / 3 * reactionGlocker[4] *
                 (reactionGlocker[1] + 2 * reactionGlocker[2] - 3 * mu_i * reactionGlocker[0]);
  // gGlocker[3] = 0.0;
  /* norm_I of row1*/
  double tmp =
      4. / 3 *
      (reactionGlocker[1] * reactionGlocker[1] + reactionGlocker[1] * reactionGlocker[2] +
       reactionGlocker[2] * reactionGlocker[2]);
  FGlocker[4] += 4 * mu_i * reactionGlocker[0] * reactionGlocker[1] -
                 3 * mu_i * mu_i * reactionGlocker[0] * reactionGlocker[0] +
...
void NCPGlocker_initialize(FrictionContactProblem* problem) {
  /*
    ...
    variables, that will be used to build local problem for each considered contact. Local
    problem is built during call to update (which depends on the storage type for M).
...
void NCPGlocker_update(int contact, FrictionContactProblem* problem,
                       FrictionContactProblem* localproblem, double* reaction,
                       SolverOptions* options) {
  /* Build a local problem for a specific contact
     reaction corresponds to the global vector (size n) of the global problem.
  */
  /* Update glocker/local M,q,reaction ... (functions of global M,q ...) depending on the
   * considered contact */
  ...
  //  - step 1: computes qLocal = qGlobal[in] + sum over a row of blocks in MGlobal of the
  //  products MLocal.reaction,
  ...
  reactionGlocker[1] =
      mu_i * reaction[in] - sqrt(3.) * reaction[it] / 2. - reaction[is] / 2.; /* SigmaP_1 */
  reactionGlocker[2] =
      mu_i * reaction[in] + sqrt(3.) * reaction[it] / 2. - reaction[is] / 2.; /* SigmaP_2 */
...
void computeJacobianGGlocker() {
  ...
  jacobianFGlocker[4] +=
      2. * mu_i *
      (2. * reactionGlocker[1] + 2. * reactionGlocker[2] - 3. * mu_i * reactionGlocker[0]);
...
void computeFGlocker(double** FOut, int up2Date) {
  ...
  cblas_dgemv(CblasColMajor, CblasNoTrans, Gsize, Gsize, 1.0, MGlocker, Gsize, reactionGlocker,
              1, 1.0, FGlocker, 1);
...
double Compute_NCP_error1(int i, double error) {
  ...
  if (Fz > 0) error += Fz;
  if (reactionGlocker[i] < 0) error += reactionGlocker[i];
  if (FGlocker[i] < 0) error += FGlocker[i];
  return error;
}
...
double Compute_NCP_error2(int i, double error) {
  ...
  Fz = FGlocker[i] * reactionGlocker[i];
  if (Fz > 0) error += Fz * Fz;
  error += (sqrt(FGlocker[i] * FGlocker[i] + reactionGlocker[i] * reactionGlocker[i]) -
            FGlocker[i] - reactionGlocker[i]) *
           (sqrt(FGlocker[i] * FGlocker[i] + reactionGlocker[i] * reactionGlocker[i]) -
            FGlocker[i] - reactionGlocker[i]);
  error = sqrt(error);
  return error;
}
...
void NCPGlocker_free(FrictionContactProblem* localproblem,
