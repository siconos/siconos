                 ...
                 double* globalVelocity,  SolverOptions* options)
{
  ...
  gfc3d_balance_go_to_balanced_variables(problem, options,
  ...
  gfc3d_balance_back_to_original_variables(problem, options,
                          reaction, velocity, globalVelocity);

  if (problem!=problem_ori)
  {
    problem->env = globalFrictionContactProblem_balancing_data_free(problem->env);
