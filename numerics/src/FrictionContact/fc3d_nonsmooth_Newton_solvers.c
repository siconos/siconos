...
#include "CSparseMatrix_internal.h"  // for CSparseMatrix_z...
  ...
  for (size_t i = 0, j = 0; i < (size_t)B->size0; i += block_row_size, j += block_col_size) {
    {
      for (size_t col_indx = j; col_indx < block_col_size + j; ++col_indx) {
        for (size_t row_indx = i; row_indx < block_row_size + i; ++row_indx, ++Alocal) {
          CHECK_RETURN(
              CSparseMatrix_zentry(Btriplet, row_indx, col_indx, *Alocal, DBL_EPSILON));
        }
        //        Alocal = ;
      }
...
static void computeSparseBlockAWpB(double *A, NumericsMatrix *W, double *B,
  ...
  for (unsigned int row = 0, ip9 = 0, i0 = 0; row < Wb->filled1 - 1;
       ++row, ip9 += 9, i0 += 3) {
...
int globalLineSearchGP(fc3d_nonsmooth_Newton_solvers *equation, double *reaction,
  ...
  for (unsigned int iter = 0; iter < maxiter_ls; ++iter) {
    ...
    equation->function(equation->data, problemSize, tmp, velocity, mu, rho, F, NULL, NULL);
...
int frictionContactFBLSA(fc3d_nonsmooth_Newton_solvers *equation, double *reaction,
  ...
  fc3d_FischerBurmeisterFunction(
      problemSize, (FischerBurmeisterFun3x3Ptr)fc3d_FischerBurmeisterFunctionGenerated,
  ...
  fc3d_FischerBurmeisterFunction(
      problemSize,
      (FischerBurmeisterFun3x3Ptr)fc3d_FischerBurmeisterGradMeritFunctionGenerated, reaction,
  ...
  if (!isnan(gradmeritfb_dir) && !isinf(gradmeritfb_dir) &&
      gradmeritfb_dir > (-fblsa_rho * norm_dir_exp_p)) {
    if (verbose > 0) {
  ...
  for (unsigned int iter = 0; iter < maxiter_ls; ++iter) {
    ...
    // compute fb
    fc3d_FischerBurmeisterFunction(
        problemSize, (FischerBurmeisterFun3x3Ptr)fc3d_FischerBurmeisterFunctionGenerated, tmp,
        velocity, mu, rho, F, NULL, NULL);
    ...
    fc3d_FischerBurmeisterFunction(
        problemSize,
        (FischerBurmeisterFun3x3Ptr)fc3d_FischerBurmeisterGradMeritFunctionGenerated, tmp,
        velocity, mu, rho, F, NULL, NULL);
...
void fc3d_nonsmooth_Newton_solvers_solve(fc3d_nonsmooth_Newton_solvers *equation,
  ...
  if (problem->M->storageType != NM_DENSE) {
    switch (options->iparam[SICONOS_FRICTION_3D_NSN_LINEAR_SOLVER]) {
