...
GlobalFrictionContactProblem* globalFrictionContact_new_from_filename(const char* filename) {
  ...
  if (is_hdf5) {
    ...
    if (!file) {
      numerics_error_log("GlobalFrictionContactProblem", "Can not open file ", filename);
      return NULL;
    }
...
void globalFrictionContact_free(GlobalFrictionContactProblem* problem) {
  ...
  if (problem->env)
    assert(0 && "globalFrictionContact_free :: problem->env != NULL, don't know what to do");
...
void globalFrictionContact_display(GlobalFrictionContactProblem* problem) {
  ...
  if (problem->mu) {
    printf("mu vector:\n");
    for (i = 0; i < problem->numberOfContacts; i++)
      printf("mu[ %i ] = %12.8e\n", i, problem->mu[i]);
  } else
    printf("No mu vector:\n");
  if (problem->M_inverse) {
