...
GlobalFrictionContactProblem* gfc3d_balancing_problem(GlobalFrictionContactProblem* problem,
  ...
  if (options->iparam[SICONOS_FRICTION_3D_IPARAM_RESCALING] ==
    ...
    data->gamma = 1.0;
  } else if (options->iparam[SICONOS_FRICTION_3D_IPARAM_RESCALING] ==
             SICONOS_FRICTION_3D_RESCALING_BALANCING_M) {
    numerics_printf_verbose(
        1, "---- GFC3D - BALANCING - Rescaling of the problem by balancing M");
  ...
  /* else if
     (options->iparam[SICONOS_FRICTION_3D_IPARAM_RESCALING]==SICONOS_FRICTION_3D_RESCALING_BALANCING_H)
   */
  /* { */
  /*   numerics_printf_verbose(1,"---- GFC3D - BALANCING - Rescaling of the problem by
     balancing H"); */
  ...
  /* } */
    ...
    numerics_printf_verbose(
        1, "---- GFC3D - BALANCING - Rescaling of the problem by balancing MHHT");
...
GlobalFrictionContactProblem_balancing_data* gfc3d_balancing_data_new() {
  GlobalFrictionContactProblem_balancing_data* data =
      malloc(sizeof(GlobalFrictionContactProblem_balancing_data));
  data->B_for_M = NULL;
  data->B_for_H = NULL;
  return data;
}
...
GlobalFrictionContactProblem_balancing_data* gfc3d_balancing_data_free(
    GlobalFrictionContactProblem_balancing_data* data) {
  if (data->B_for_M) data->B_for_M = NM_BalancingMatrices_free(data->B_for_M);
  if (data->B_for_H) data->B_for_H = NM_BalancingMatrices_free(data->B_for_H);
  free(data);
  return NULL;
}
