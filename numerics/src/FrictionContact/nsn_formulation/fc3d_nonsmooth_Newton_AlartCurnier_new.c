...
#include "NonSmoothSolvers/Newton_methods.h"                      // for newton_LSA, function...
...
static void FC3D_compute_error(void* data_opaque, double* reaction, double* velocity,
                               double* Jac_F_merit, double tol, double* err) {
...
static void FC3D_compute_F_merit(void* data_opaque, double* reaction, double* velocity,
                                 double* F) {
  FC3D_Newton_data* dat = (FC3D_Newton_data*)data_opaque;
  FrictionContactProblem* problem = dat->problem;
  fc3d_nonsmooth_Newton_solvers* equation = dat->equation;

  equation->function(equation->data, problem->M->size0, reaction, velocity,
                     equation->problem->mu, dat->rho, F, dat->Ax, dat->Bx);
  dat->AwpB_data_computed = true;
}

static void FC3D_compute_AWpB(void* data_opaque, double* reaction, double* velocity,
                              double* workV1, double* workV2, NumericsMatrix* AWpB) {
  FC3D_Newton_data* dat = (FC3D_Newton_data*)data_opaque;
...
void fc3d_nonsmooth_Newton_AlartCurnier_new(FrictionContactProblem* problem, double* reaction,
  ...
  if (options->iparam[SICONOS_FRICTION_3D_NSN_HYBRID_STRATEGY] ==
    ...
    newton_LSA(problemSize, reaction, velocity, info, (void*)&opaque_data, options,
               &functions_AC);
  } else if (options->iparam[SICONOS_FRICTION_3D_NSN_HYBRID_STRATEGY] ==
