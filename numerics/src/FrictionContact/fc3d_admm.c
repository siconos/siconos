...
#include "debug.h"
const char* const   SICONOS_GLOBAL_FRICTION_3D_ADMM_STR = "FC3D ADMM";
...
void fc3d_ADMM_init(GlobalFrictionContactProblem* problem, SolverOptions* options)
{
...
void fc3d_ADMM_free(GlobalFrictionContactProblem* problem, SolverOptions* options)
{
  if (options->dWork)
  {
    free(options->dWork);
...
void fc3d_ADMM(GlobalFrictionContactProblem* restrict problem, double* restrict reaction,
  ...
  while ((iter < itermax) && (hasNotConverged > 0))
    {
      ...
      cblas_daxpy(m, -1.0, z, 1, reaction , 1);
      ...
      if (residual < tolerance)
      {
        ...
        fc3d_compute_error(problem,  reaction, u, z,  tolerance, options, norm_q, &error);
