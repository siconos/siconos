...
#include "FrictionContact_options.h"            // for SICONOS_FRICTION_3D_FPP
...
void fc3d_fixedPointProjection(FrictionContactProblem* problem, double* reaction,
  ...
  if (verbose > 0) {
    printf(
        "--------------- FC3D -  Fixed Point Projection (FPP) - Variable stepsize with "
        "starting rho = %14.7e \n",
        rho);
  }
  ...
  if (isVariable) {
    while ((iter < itermax) && (hasNotConverged > 0)) {
      ...
      if (verbose > 0)
        printf(
            "--------------- FC3D -  Fixed Point Projection (FPP) - Iteration %i rho = %14.7e "
            "\tError = %14.7e\n",
            iter, rho, error);
  ...
  if (verbose > 0)
    printf(
        "--------------- FC3D - Fixed Point Projection (FPP) - #Iteration %i Final Residual = "
        "%14.7e\n",
        iter, error);
