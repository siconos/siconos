...
void ACPsi(GlobalFrictionContactProblem *problem, AlartCurnierFun3x3Ptr computeACFun3x3,
  ...
  /* compute AC function */
  fc3d_AlartCurnierFunction(m, computeACFun3x3, reaction, velocity, problem->mu, rho,
                            psi + n + m, NULL, NULL);
}
...
CS_INT initACPsiJacobian(CSparseMatrix *M, CSparseMatrix *H, CSparseMatrix *A,
  ...
  for (int e = 0; e < B->nz; ++e) {
    CHECK_RETURN(CSparseMatrix_entry(J, B->i[e] + M->m + H->n, B->p[e] + M->n + A->n,
                                     rescaling * B->x[e]));
  }
...
int _globalLineSearchSparseGP(GlobalFrictionContactProblem *problem,
  ...
  for (unsigned int iter = 0; iter < maxiter_ls; ++iter) {
    ...
    ACPsi(problem, computeACFun3x3, tmp,               /* v */
    ...
    if (C1 && C2) {
      numerics_printf_verbose(1,
                              "---- GFC3D - NSN_AC - global line search success. Number of ls "
                              "iteration = %i  alpha = %.10e, q = %.10e",
                              iter, alpha[0], q);
  ...
  numerics_printf_verbose(1,
                          "---- GFC3D - NSN_AC - global line search unsuccessful. Max number "
                          "of ls iteration reached  = %i  with alpha = %.10e",
                          maxiter_ls, alpha[0]);
...
void gfc3d_nonsmooth_Newton_AlartCurnier(GlobalFrictionContactProblem *problem,
  ...
  numerics_printf_verbose(1, "---- GFC3D - NSN_AC - 1-norm of M = %g norm of q = %g ",
                          NM_norm_1(problem->M), norm_q);
  numerics_printf_verbose(1, "---- GFC3D - NSN_AC - inf-norm of M = %g ",
                          NM_norm_inf(problem->M));

  numerics_printf_verbose(1, "---- GFC3D - NSN_AC - 1-norm of H = %g norm of b = %g ",
                          NM_norm_1(problem->H), norm_b);
  ...
  if (options->iparam[SICONOS_FRICTION_3D_NSN_MEMORY_ALLOCATED] == 0) {
    ...
    options->dWorkSize = (2 * problem->M->size0 +
                          /* for dWork part used inside gfc3d_compute_error */
                          m +            /* F */
                          3 * m +        /* A */
                          3 * m +        /* B */
                          m +            /* rho */
  ...
  J = cs_spalloc(NM_triplet(problem->M)->n + A_.m + B_.m,
                 ...
                 NM_triplet(problem->M)->nzmax + 2 * NM_triplet(problem->H)->nzmax + 2 * A_.n +
                     A_.nzmax + B_.nzmax,
                 1, 1);
  ...
  DEBUG_PRINTF("NM_triplet(problem->M)->n + A_.m + B_.m = %li\n",
               NM_triplet(problem->M)->n + A_.m + B_.m);
  DEBUG_PRINTF(
      "NM_triplet(problem->M)->nzmax + 2*NM_triplet(problem->H)->nzmax + 2*A_.n + A_.nzmax + "
      "B_.nzmax= %li\n",
      NM_triplet(problem->M)->nzmax + 2 * NM_triplet(problem->H)->nzmax + 2 * A_.n + A_.nzmax +
          B_.nzmax);
  ...
  fc3d_AlartCurnierFunction(m, computeACFun3x3, reaction, velocity, problem->mu, rho, F, A, B);

  double rescaling = 1e+00;

  CS_INT Astart = initACPsiJacobian(NM_triplet(problem->M), NM_triplet(problem->H), &A_, &B_,
                                    J, rescaling);
  ...
  DEBUG_EXPR(for (unsigned int i = 0; i < m; ++i) printf("rho[%i]=%e\t", i, rho[i]);
             printf("\n"););
  ...
  while (iter++ < itermax) {
    ...
    /* compute A & B */
    fc3d_AlartCurnierFunction(m, computeACFun3x3, reaction_k, velocity_k, problem->mu, rho, F,
                              A, B);
    /* update J */
    updateACPsiJacobian(NM_triplet(problem->M), NM_triplet(problem->H), &A_, &B_, J, Astart);
    ...
    if (info_solver > 0) {
      ...
      /* info[0] = 2; */
      /* CHECK_RETURN(!CSparseMatrix_check_triplet(NM_triplet(NM_J))); */
    }

    /* Check the quality of the solution */
    if (verbose > 0) {
      cblas_dcopy_msan(problem_size, psi, 1, tmp3, 1);
      NM_gemv(1., NM_J, rhs, 1., tmp3);
      linear_solver_residual = cblas_dnrm2(problem_size, tmp3, 1);
      numerics_printf_verbose(1,
                              "---- GFC3D - NSN_AC iteration %d, linear_solver_residual = %g ",
    ...
    switch (options->iparam[SICONOS_FRICTION_3D_NSN_LINESEARCH]) {
      ...
      case SICONOS_FRICTION_3D_NSN_LINESEARCH_GOLDSTEINPRICE:
        /* Goldstein Price */
        //      info_ls =
        _globalLineSearchSparseGP(
            problem, computeACFun3x3, solution, rhs, problem->mu, rho, F, psi, Jcsc, tmp2,
            &alpha, options->iparam[SICONOS_FRICTION_3D_NSN_LINESEARCH_MAX_ITER]);
        break;
      /* case SICONOS_FRICTION_3D_NSN_LINESEARCH_ARMIJO: */
      /*   /\* FBLSA *\/ */
      /*   info_ls = frictionContactFBLSA(equation, reaction, velocity, problem->mu, rho, F,
       * Ax, Bx, */
    ...
    if (!(iter % erritermax)) {
      gfc3d_compute_error(problem, reaction_k, velocity_k, globalVelocity_k, tolerance,
                          options, norm_q, norm_b, &(options->dparam[SICONOS_DPARAM_RESIDU]));
    }

    if (options->dparam[SICONOS_DPARAM_RESIDU] >= tolerance) {
      numerics_printf_verbose(1, "---- GFC3D - NSN_AC iteration %d, residual = %g >= %g", iter,
...
REGISTER_SOLVER_3VAR(GFC3D_NSN_AC, "GFC3D_NSN_AC",
                     ...
                     gfc3d_nsn_ac_init_wrap, gfc3d_nsn_ac_solve_wrap, gfc3d_nsn_ac_free_wrap,
                     NULL,                     /* error function */
