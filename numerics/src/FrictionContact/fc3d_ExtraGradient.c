...
#include "FrictionContact_options.h"            // for SICONOS_FRICTION_3D_EG
...
void fc3d_ExtraGradient(FrictionContactProblem* problem, double* reaction, double* velocity,
  ...
  if (dparam[SICONOS_FRICTION_3D_NSN_RHO] > 0.0) {
    ...
    if (verbose > 0) {
      printf(
          "--------------- FC3D - Extra Gradient (EG) - Fixed stepsize with  rho = %14.7e \n",
          rho);
    }
  } else {
    ...
    if (verbose > 0) {
      printf(
          "--------------- FC3D - Extra Gradient (EG) - Variable stepsize with starting rho = "
          "%14.7e \n",
          rho);
    }
  }
  ...
  if (!isVariable) {
    ...
    while ((iter < itermax) && (hasNotConverged > 0)) {
      ...
      if (verbose > 0) {
        printf(
            "--------------- FC3D - Extra Gradient (EG) - Iteration %i rho = %14.7e \tError = "
            "%14.7e\n",
            iter, rho, error);
      }
  ...
  if (isVariable) {
    while ((iter < itermax) && (hasNotConverged > 0)) {
      ...
      while (!success && (ls_iter < ls_itermax)) {
        ...
        for (contact = 0; contact < nc; ++contact) {
          int pos = contact * nLocal;
          double normUT = sqrt(velocity_k[pos + 1] * velocity_k[pos + 1] +
                               velocity_k[pos + 2] * velocity_k[pos + 2]);
          /* reaction[pos] = reaction_k[pos] -  rho_k * (velocity_k[pos] + mu[contact] *
           * normUT); */
        ...
        for (contact = 0; contact < nc; ++contact) {
          ...
          double normUT = sqrt(velocitytmp[pos + 1] * velocitytmp[pos + 1] +
                               velocitytmp[pos + 2] * velocitytmp[pos + 2]);
          double normUT_k = sqrt(velocity_k[pos + 1] * velocity_k[pos + 1] +
                                 velocity_k[pos + 2] * velocity_k[pos + 2]);
        ...
        /* { */
        /*   int pos = contact * nLocal; */
        /*   double  normUT = sqrt(velocity_k[pos + 1] * velocity_k[pos + 1] + velocity_k[pos +
         * 2] * velocity_k[pos + 2]); */
      ...
      for (contact = 0; contact < nc; ++contact) {
        ...
        /* reaction[pos] = reaction_k[pos] -  rho_k * (velocitytmp[pos] + mu[contact] *
         * normUT); */
  ...
  if (verbose > 0) {
    printf(
        "--------------- FC3D -  Extra Gradient (EG) - #Iteration %i Final Residual = "
        "%14.7e\n",
        iter, error);
  }
