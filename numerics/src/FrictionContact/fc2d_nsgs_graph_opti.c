...
static inline double light_error_squared(const double* restrict localreaction,
                                         const double* restrict oldreaction) {
...
#pragma omp parallel default(none)                                                   \
    ...
    shared(problem, diagonal_blocks, diagonal_block_determinant, z, partition_size,  \
               partitions, iter)                                                     \
...
#pragma omp for reduction(+ : light_error_sum)
          for (int v = 0; v < partition_size[color]; v++) {
            ...
            fc2d_nsgs_buildLocalProblem_parallel(contact, problem, diagonal_blocks,
                                                 local_problem, z);

            /* Solve local problem */
            fc2d_nsgs_local_solve(local_problem->M->matrix0,
                                  diagonal_block_determinant[contact], local_problem->q,
...
#pragma omp parallel default(none) private(contact, pos, local_problem, localreaction) \
    shared(problem, diagonal_blocks, diagonal_block_determinant, z, partition_size,    \
               partitions, iter)                                                       \
    shared(iparam, light_error_sum, n_colors, norm_r, nc, error, options, tolerance,   \
               has_not_converged, norm_q, w, itermax)
...
static void fc2d_nsgs_buildLocalProblem_2(unsigned int contact,
                                          FrictionContactProblem* problem,
                                          double* blocks_contiguous, double* diagonal_blocks,
                                          size_t* index1_data, size_t* index2_data,
                                          LinearComplementarityProblem* local_problem,
                                          double* reaction) {
...
static double* fc2d_nsgs_compute_local_problem_determinant_2(unsigned int nc,
...
void test_solver(FrictionContactProblem* problem, double* z, double* w, int* info,
  ...
  for (size_t color = 0; color < n_colors; color++)
  {
    ...
    for (unsigned int contact = sum_sizes[color]; contact < sum_sizes[color + 1]; contact++)
    {
      ...
      for (size_t blockNum = SBM_permuted->index1_data[contact]; blockNum <
  SBM_permuted->index1_data[contact + 1]; blockNum++)
      {
  ...
  double* diagonal_block_determinant =
      fc2d_nsgs_compute_local_problem_determinant_2(nc, diagonal_blocks);
  ...
  for (unsigned int line = 0; line < problem->numberOfContacts; line++) {
    if (max_blocks_in_one_line < (problem->M->matrix1->index1_data[line + 1] -
                                  problem->M->matrix1->index1_data[line])) {
      max_blocks_in_one_line = (problem->M->matrix1->index1_data[line + 1] -
                                problem->M->matrix1->index1_data[line]);
    }
  }
...
#pragma omp parallel default(none)                                                       \
    ...
    shared(problem, diagonal_blocks, diagonal_block_determinant, blocks_contiguous, z,   \
               sum_sizes, n_colors, n_iter)                                              \
    shared(light_error_sum, norm_r, iparam, error, norm_q, tolerance, has_not_converged, \
               iter, options, w, nc)
  {
    // printf("Thread num: %d\n", omp_get_thread_num());
...
#pragma omp for  // reduction(+:light_error_sum,norm_r)
        for (unsigned int permuted_contact = sum_sizes[color];
          ...
          fc2d_nsgs_buildLocalProblem_2(permuted_contact, problem, blocks_contiguous_local,
                                        diagonal_blocks_local, index1_data, index2_data,
                                        local_problem, z);

          /* printf("Contact %d, q[0] = %e, q[1] = %e\n", contact, local_problem->q[0],
          local_problem->q[1]); printf("M->matrix0 = %e %e %e %e \n",
          local_problem->M->matrix0[0], local_problem->M->matrix0[1],
          local_problem->M->matrix0[2], local_problem->M->matrix0[3]); printf("%e %e\n",
          diagonal_block_determinant[permuted_contact], problem->mu[permuted_contact]);
          printf("%e %e\n", localreaction[0], localreaction[1]); */

          /* Solve local problem */
          fc2d_nsgs_local_solve(local_problem->M->matrix0,
                                diagonal_block_determinant[permuted_contact], local_problem->q,
                                problem->mu[permuted_contact], localreaction);
