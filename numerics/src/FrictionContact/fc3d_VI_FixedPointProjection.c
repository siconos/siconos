...
void fc3d_VI_FixedPointProjection(FrictionContactProblem *problem, double *reaction,
  ...
  fc3d_compute_error(problem, reaction, velocity, options->dparam[SICONOS_DPARAM_TOL], options,
                     norm_q, &error);
  ...
  if (verbose > 0) {
    printf(
        "--------------- FC3D - VI Fixed Point Projection (VI_FPP) - #Iteration %i Final "
        "Residual = %14.7e\n",
...
void fc3d_VI_FixedPointProjection_Cylinder(FrictionContactProblem *problem, double *reaction,
  ...
  fc3d_Tresca_compute_error(problem, reaction, velocity,
                            options->dparam[SICONOS_DPARAM_RESIDU], options, norm_q, &error);
