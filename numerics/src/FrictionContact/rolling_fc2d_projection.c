...
int rolling_fc2d_projectionOnCone_solve(
  RollingFrictionContactProblem* localproblem, double* reaction, SolverOptions * options)
{
  ...
  for(int i = 0; i < 3; i++) velocity[i]
      = MLocal[i + 0 * 3] * reaction[0] + qLocal[i]
  ...
  SolverOptions* localsolver_options)
{
  ...
  /* printf("rolling_fc2d_projectionOnConeWithLocalIteration_initialize. Allocation of dwork\n"); */
...
void rolling_fc2d_projectionOnConeWithLocalIteration_free(RollingFrictionContactProblem * problem, RollingFrictionContactProblem * localproblem, SolverOptions* localsolver_options)
{
...
static int  rolling_fc2d_check_trivial_solution(
  ...
  SolverOptions* options)
{
  ...
  int trivial = rolling_fc2d_check_trivial_solution(options->iparam[SICONOS_FRICTION_3D_CURRENT_CONTACT_NUMBER],
  ...
  int status = -1;
  numerics_printf_verbose(2,"--  rolling_fc2d_projectionOnConeWithLocalIteration_solve contact = %i", options->iparam[SICONOS_FRICTION_3D_CURRENT_CONTACT_NUMBER]);
  numerics_printf_verbose(2,"--  rolling_fc2d_projectionOnConeWithLocalIteration_solve | localiter \t| rho \t\t\t| error\t\t\t| status\t|");
