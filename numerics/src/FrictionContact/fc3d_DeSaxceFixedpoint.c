...
#include "FrictionContact_options.h"            // for SICONOS_FRICTION_3D_DSFP
...
void fc3d_DeSaxceFixedPoint(FrictionContactProblem* problem, double* reaction,
  ...
  if (dparam[SICONOS_FRICTION_3D_NSN_RHO] > 0.0) {
    ...
    if (verbose > 0) {
      printf(
          "--------------- FC3D - DeSaxce Fixed Point (DSFP) - Fixed stepsize with  rho = "
          "%14.7e \n",
          rho);
    }
  ...
  while ((iter < itermax) && (hasNotConverged > 0)) {
    ...
    if (verbose > 0)
      printf(
          "--------------- FC3D - DeSaxce Fixed Point (DSFP) - Iteration %i rho = %14.7e "
          "\tError = %14.7e\n",
          iter, rho, error);
  ...
  if (verbose > 0)
    printf(
        "--------------- FC3D - DeSaxce Fixed point (DSFP) - #Iteration %i Final Residual = "
        "%14.7e\n",
        iter, error);
