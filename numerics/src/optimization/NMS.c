...
#include "NonSmoothSolvers/Newton_methods.h"  // for functions_LSA, SICONOS_IPARAM_LSA_NONMON...
...
int NMS(NMS_data* data_NMS, void* data, functions_LSA* functions, double* restrict z,
        double* restrict z_N, int force_watchdog_step, int force_d_step_merit_check,
        double check_ratio) {
  double theta_iter;
  ...
  if (data_NMS->n < data_NMS->n_max) {
    ...
    if (force_d_step_merit_check ||
        (dist <= data_NMS->delta * (1.0 + check_ratio))) /* try a d_step */
    {
      ...
      cblas_dgemv(CblasColMajor, CblasTrans, n, n, 1.0, H, n, F_merit, 1, 0.0, JacThetaF_merit,
                  1);
      ...
      if (force_d_step_merit_check ||
          theta_iter < data_NMS->merit_incr * data_NMS->ref_merit * (1.0 + check_ratio)) {
        DEBUG_PRINTF("NMS d_step accepted theta_iter = %2.2e < tol = %2.2e\n", theta_iter,
                     data_NMS->merit_incr * data_NMS->ref_merit);
        /* we accept the step */
        data_NMS->n++;
        ...
        int criterion =
            check_nmd_criterion(theta_iter, data_NMS->ref_merit, data_NMS->sigma, dotprod);
        if (criterion == 0) {
      ...
      DEBUG_PRINTF("NMS d_step rejected dist = %2.2e; delta = %2.2e\n", dist,
                   data_NMS->delta * (1.0 + check_ratio));
      goto m_step;
    }
  } else /* try the m_step */
  {
  ...
  switch (data_NMS->projected_gradient_search_type) {
    case LINESEARCH:
      ...
      if (dotprod > 0.0) /* check condition on <JacThetaF_merit, z_b(0) - z_b(T_k)>*/
      {
        ...
        //        preRHS = -data_NMS->sigma*data_NMS->merit_bestpoint; /* we expect a plus in
        //        the LS function */
      ...
      alpha_projected_gradient =
          search_Armijo_standalone(n, &data_NMS->merit_bestpoint, preRHS, data_NMS->ls_data);
      if (alpha_projected_gradient > data_NMS->alpha_min_pgrad) {
        /* successful line search */
        theta_iter =
            data_NMS
                ->merit_bestpoint; /* on output ref_merit contains the current value of theta*/
        DEBUG_PRINTF(
            "NMS :: projected_gradient_step succeed: alpha = %2.2e; theta_iter = %2.2e\n",
...
accept_z_N:
  DEBUG_PRINT("z_N z_0\n");
  DEBUG_EXPR_WE(for (unsigned i = 0; i < n; ++i){DEBUG_PRINTF("%e %e\n", z_N[i], z[i])});
...
void free_NMS_data(NMS_data* data) {
  ...
  if (data->path_data) free(data->path_data);
