...
int relay_driver(RelayProblem* problem, double* z, double* w, SolverOptions* options) {
  ...
  if (verbose == 1)
    printf(
        " ========================== Call %s solver for Relayproblem "
        "==========================\n",
  ...
  switch (options->solverId) {
    ...
    case SICONOS_RELAY_AVI_CAOFERRIS_TEST: {
      ...
      /* /\*error *\/ */
      // what should we do for case like SICONOS_RELAY_LEMKE (id) when the real solver is
      // SICONOS_LCP_LEMKE ? Add case above ? No error ?
      /* default: */
      /* { */
  ...
  if (options[0].filterOn > 0)
    info = relay_compute_error(problem, z, w, options[0].dparam[SICONOS_DPARAM_TOL],
                               &(options[0].dparam[SICONOS_DPARAM_RESIDU]));
