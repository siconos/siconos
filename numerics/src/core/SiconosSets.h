...
/** @brief Mainly used to infer the type of set (box, polytope, ...) to properly operate on it
 */
typedef struct {
...
/** @brief box constraints
 *
 * \f$ K = \{ x \in \mathbb{R}^n | lb_i \leq x_i \leq ub_i\quad i = 1..n\}\f$
 */
typedef struct {
...
/** @brief A polytope in terms of (H,K) representation
 *
...
typedef struct {
  int id; /**< id of the structure, usually solver specific */
  NumericsMatrix*
      A;     /**< A matrix in an (A,b) representation of a polytope \f$Ax (\leq,=,\geq) b\f$ */
  double* b; /**< b vector in an (A,b) representation of a polytope \f$Ax (\leq,=,\geq) b\f$ */
  char* type; /**< type of constraint */
} polyhedron_unified;

/** @brief Different possible representations of a polyhedral set. */
typedef union {
  generic_set* set;
  polyhedron* split;
...
/** @brief  possible ids for sets*/
enum SICONOS_SET_ID {
  SICONOS_SET_POSITIVE_ORTHANT,
  SICONOS_SET_BOX,
  SICONOS_SET_POLYHEDRON,
  SICONOS_SET_POLYHEDRON_UNIFIED
};

/** @brief allowed relation types */
 ...
 */
static inline void set_set_id(void* set, int id) { ((generic_set*)set)->id = id; }
