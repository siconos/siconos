...
void mlcp_psor(MixedLinearComplementarityProblem* problem, double* z, double* w, int* info,
  ...
  while ((iter < itermax) && (err > tol)) {
    ...
    for (i = 0; i < n; ++i) {
      ...
      u[i] = -(a[i] + cblas_ddot(n, &A[i], incAx, u, incy) +
               cblas_ddot(m, &C[i], incAx, v, incy)) *
             diagA[i];
    }
    ...
    for (i = 0; i < m; ++i) {
      ...
      v[i] = 0.0;
      // zi = -( q[i] + cblas_ddot( n , &vec[i] , incx , z , incy ))*diag[i];
      vi = -(b[i] + cblas_ddot(n, &D[i], incBx, u, incy) +
             cblas_ddot(m, &B[i], incBx, v, incy)) *
           diagB[i];
    ...
    numerics_printf_verbose(
        1, "---- MLCP - PSOR  - Iteration %i omega = %8.4e, residual = %14.7e, tol = %14.7e",
        omega, iter, err, tol);
    if (verbose == 2) {
  ...
  if (err > tol) {
    numerics_printf_verbose(
        1, "---- MLCP - PSOR  - No convergence after %d iterations with error = %14.7e ", iter,
        err);
    *info = 1;
  } else {
    numerics_printf_verbose(
        1, "---- MLCP - PSOR  - Convergence after %d iterations with error = %14.7e ", iter,
        err);
    *info = 0;
  }
 ...
 * Solver Registration
 * ===========================================================================
 * This registers SICONOS_MLCP_PSOR in the global solver registry.
 */
...
static int mlcp_psor_solve_wrap(void* problem, double* z, double* w, SolverOptions* options) {
  int info = NUMERICS_OK;
  mlcp_psor((MixedLinearComplementarityProblem*)problem, z, w, &info, options);
  return info;
}
