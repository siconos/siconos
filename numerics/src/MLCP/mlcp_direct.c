...
*
* mlcp_direct_addConfigFromWSolution consists in precomputing the linear system from the
current solution.
*
* mlcp_direct, the next LCP instance is solved using the current linear system, two cases can
happen:
...
int mlcp_direct_getNbIWork(MixedLinearComplementarityProblem* problem,
                           SolverOptions* options) {
  return (problem->n + problem->m) *
             (options->iparam[SICONOS_IPARAM_MLCP_NUMBER_OF_CONFIGURATIONS] + 1) +
         options->iparam[SICONOS_IPARAM_MLCP_NUMBER_OF_CONFIGURATIONS] * problem->m;
}

int mlcp_direct_getNbDWork(MixedLinearComplementarityProblem* problem,
                           SolverOptions* options) {
  return problem->n + problem->m +
         (options->iparam[SICONOS_IPARAM_MLCP_NUMBER_OF_CONFIGURATIONS]) *
             ((problem->n + problem->m) * (problem->n + problem->m)) +
         (problem->n + problem->m);
}
...
int internalAddConfig(MixedLinearComplementarityProblem* problem, int* zw, int init) {
  if (verbose) {
    ...
    printf("---------\n");
    for (int i = 0; i < problem->m; i++) printf("zw[%d]=%d\t", i, zw[i]);
    printf("\n");
  }
  if (init) {
...
void mlcp_direct_addConfig(MixedLinearComplementarityProblem* problem, int* zw) {
  if (s_numberOfCC < s_maxNumberOfCC) /*Add a configuration*/
  {
    ...
    {
      spFirstCC =
          (struct dataComplementarityConf*)malloc(sizeof(struct dataComplementarityConf));
      spFirstCC->Usable = 1;
      sp_curCC = spFirstCC;
      spFirstCC->prev = 0;
      spFirstCC->next = 0;
...
void mlcp_direct_addConfigFromWSolution(MixedLinearComplementarityProblem* problem,
                                        double* wSol) {
  int i;
...
int solveWithCurConfig(MixedLinearComplementarityProblem* problem) {
  ...
  for (int i=0;i<s_m;i++)
    if (sp_curCC->zw[i])
      ...
      printf("0");
      printf("\n");*/
  if (sProblemChanged) internalPrecompute(problem);
  if (!sp_curCC->Usable) {
    if (verbose) printf("solveWithCurConfig not usable\n");
    return 0;
  }
#ifdef DIRECT_SOLVER_USE_DGETRI
  cblas_dgemv(CblasColMajor, CblasNoTrans, s_npM, s_npM, ALPHA, sp_curCC->M, s_npM, sQ, INCX,
              BETA, sVBuf, INCY);
  solTest = sVBuf;
#else
  for (lin = 0; lin < s_npM; lin++) sQ[lin] = -problem->q[lin];
  DGETRS(LA_NOTRANS, s_npM, one, sp_curCC->M, s_npM, sp_curCC->IPV, sQ, s_npM, &INFO);
  solTest = sQ;
#endif
  if (INFO) {
    if (verbose) printf("solveWithCurConfig DGETRS failed\n");
    return 0;
  } else {
...
void mlcp_direct(MixedLinearComplementarityProblem* problem, double* z, double* w, int* info,
  ...
  if (!spFirstCC) {
    ...
    do {
      ...
      if (find) {
#ifdef DIRECT_SOLVER_USE_DGETRI
        mlcp_enum_fill_solution(z, z + s_n, w, w + s_n, s_n, s_m, s_nbLines, sp_curCC->zw,
                                sVBuf);
#else
        ...
        if (sp_curCC != spFirstCC) {
          ...
          if (sp_curCC->next) sp_curCC->next->prev = sp_curCC->prev;
