...
typedef struct {
  ...
  double default_tol;
  int is_local_solver;
  int has_3var; /* 1 if solver uses solve3 (3 double* params), 0 if uses solve (2 double*
                   params) */
} SolverEntry;
...
#define REGISTER_SOLVER(id, name_str, desc_str, init_fn, solve_fn, free_fn, err_fn,  \
  ...
  static int _solver_reg_##id(void) {                                                \
    static const SolverEntry entry = {                                               \
        id,     name_str,       desc_str,     init_fn, {.solve = solve_fn}, free_fn, \
        err_fn, set_default_fn, max_iter_val, tol_val, local_val,           0};      \
...
#define REGISTER_SOLVER_3VAR(id, name_str, desc_str, init_fn, solve_fn, free_fn, err_fn, \
  ...
  static int _solver_reg_##id(void) {                                                    \
    static const SolverEntry entry = {                                                   \
        id,        name_str, desc_str,       init_fn,      {.solve3 = solve_fn},         \
        free_fn,   err_fn,   set_default_fn, max_iter_val, tol_val,                      \
        local_val, 1};                                                                   \
    return solver_registry_register(&entry);                                             \
  }
