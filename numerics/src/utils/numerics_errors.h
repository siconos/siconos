...
static inline const char* numerics_error_string(NumericsError err) {
  switch (err) {
    case NUMERICS_OK:
      return "Success";
    case NUMERICS_ERR_NULL_POINTER:
      return "Null pointer";
    case NUMERICS_ERR_INVALID_ARGUMENT:
      return "Invalid argument";
    case NUMERICS_ERR_INVALID_OPTION:
      return "Invalid option";
    case NUMERICS_ERR_INVALID_SOLVER:
      return "Invalid solver";
    case NUMERICS_ERR_NOT_IMPLEMENTED:
      return "Not implemented";
    case NUMERICS_ERR_MEMORY:
      return "Memory allocation failed";
    case NUMERICS_ERR_FILE_IO:
      return "File I/O error";
    case NUMERICS_ERR_MAX_ITER:
      return "Maximum iterations reached";
    case NUMERICS_ERR_DIVERGENCE:
      return "Solver diverged";
    case NUMERICS_ERR_STAGNATION:
      return "Solver stagnated";
    case NUMERICS_ERR_LINEAR_SOLVER:
      return "Linear solver failed";
    case NUMERICS_ERR_LOCAL_SOLVER:
      return "Local solver failed";
    case NUMERICS_ERR_PROJECTION:
      return "Projection failed";
    case NUMERICS_ERR_INFEASIBLE:
      return "Problem infeasible";
    case NUMERICS_ERR_UNBOUNDED:
      return "Problem unbounded";
    case NUMERICS_ERR_ILL_CONDITIONED:
      return "Ill-conditioned problem";
    case NUMERICS_ERR_SINGULAR_MATRIX:
...
#define NUMERICS_CHECK(cond, err, ...)                                             \
  do {                                                                             \
    if (!(cond)) {                                                                 \
      numerics_log_error(__FILE__, __LINE__, __func__, numerics_error_string(err), \
                         __VA_ARGS__);                                             \
      return err;                                                                  \
    }                                                                              \
  } while (0)
...
#define NUMERICS_CHECK_NULL(ptr) \
  NUMERICS_CHECK((ptr) != NULL, NUMERICS_ERR_NULL_POINTER, #ptr " is NULL")
...
void numerics_log_error(const char* file, int line, const char* func, const char* error_type,
                        const char* fmt, ...);

void numerics_log_warning(const char* file, int line, const char* func, const char* fmt, ...);

void numerics_log_info(const char* file, int line, const char* func, const char* fmt, ...);
...
#define CHECK_CONVERGENCE(error, tol, iter)                                               \
  do {                                                                                    \
    if ((error) > (tol)) {                                                                \
      fprintf(stderr,                                                                     \
              "[WARN] %s:%d (%s) - Not converged: error=%.3e > tol=%.3e at iter=%d\n",    \
