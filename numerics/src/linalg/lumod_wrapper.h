...
typedef struct {
  ...
  unsigned* factorized_basis; /**< basis when H was factorized and storing for the info where
                                 the columns of the non basic variables are in U and when a
                                 basic variable exited */
...
static inline unsigned SN_lumod_need_refactorization(int info) {
  return (info == SN_LUMOD_NEED_REFACTORIZATION ? 1 : 0);
}
...
void SN_lumod_replace_row(SN_lumod_dense_data* lumod_data, unsigned index_row,
                          unsigned leaving_indx);
void SN_lumod_delete_row_col(SN_lumod_dense_data* lumod_data, unsigned index_row,
                             unsigned index_col);
int SN_lumod_factorize(SN_lumod_dense_data* lumod_data, unsigned* basis, NumericsMatrix* M,
                       double* covering_vector);
