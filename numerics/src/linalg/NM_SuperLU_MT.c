...
NM_SuperLU_MT_WS* NM_SuperLU_MT_factorize(NumericsMatrix* A) {
  ...
  if (!superlu_mt_ws->options)
    superlu_mt_ws->options = (superlumt_options_t*)SUPERLU_MALLOC(sizeof(superlumt_options_t));
  ...
  dCreate_CompCol_Matrix(superlu_mt_ws->A, C->m, C->n, nnz, C->x, indices, pointers, SLU_NC,
                         SLU_D, SLU_GE);
  ...
  pdgstrf_init(nprocs, EQUILIBRATE, NOTRANS, NO, panel_size, relax, diag_pivot_thresh, usepr,
               drop_tol, superlu_mt_ws->perm_c, superlu_mt_ws->perm_r, work, lwork,
               superlu_mt_ws->A, &SAC, superlu_mt_ws->options, superlu_mt_ws->stat);

  if (verbose > 1) superlu_mt_ws->options->PrintStat = YES;

  pdgstrf(superlu_mt_ws->options, &SAC, superlu_mt_ws->perm_r, superlu_mt_ws->L,
          superlu_mt_ws->U, superlu_mt_ws->stat, &status);
...
int NM_SuperLU_MT_solve(NumericsMatrix* A, double* b, NM_SuperLU_MT_WS* superlu_mt_ws) {
  ...
  if (iter_refinement) {
    ...
    dgstrs(NOTRANS, superlu_mt_ws->L, superlu_mt_ws->U, superlu_mt_ws->perm_r,
           superlu_mt_ws->perm_c, &X, superlu_mt_ws->stat, &status);
  } else {
    dCreate_Dense_Matrix(&B, C->n, 1, b, C->n, SLU_DN, SLU_D, SLU_GE);

    dgstrs(NOTRANS, superlu_mt_ws->L, superlu_mt_ws->U, superlu_mt_ws->perm_r,
           superlu_mt_ws->perm_c, &B, superlu_mt_ws->stat, &status);
  }
  ...
  if (iter_refinement) {
    ...
    if (!status)
      dgsrfs(NOTRANS, superlu_mt_ws->A, superlu_mt_ws->L, superlu_mt_ws->U,
             superlu_mt_ws->perm_r, superlu_mt_ws->perm_c, equed, NULL, NULL, &B, &X, &ferr,
             &berr, superlu_mt_ws->stat, &status);
