...
// Returns the version of a given sub-type of matrix M
// 0 if the type is not set
...
/* Returns the type which has the highest version between type1 and type2, for matrix M */
static NM_types nm_max(const NumericsMatrix* M, NM_types type1, NM_types type2) {
  return NM_version(M, type2) > NM_version(M, type1) ? type2 : type1;
}

// Returns sthe type of highest version in a given matrix
static NM_types NM_latest_id(const NumericsMatrix* M) {
  NM_types t = nm_max(M, nm_max(M, NM_DENSE, NM_SPARSE_BLOCK), NM_SPARSE);
  return t;
}

// Returns the version value of the type of highest version in a given matrix
static version_t NM_max_version(const NumericsMatrix* M) {
  return NM_version(M, NM_latest_id(M));
}

// Set version for M to a value greater than the current highest value
// and set if for type 'id'
...
// Set storage type to the last updated version
...
int NM_prod_mv_3x3(int sizeX, int sizeY, NumericsMatrix* A, double* const x, double* y) {
  ...
  switch (storage) {
    case NM_DENSE:
      cblas_dgemv(CblasColMajor, CblasNoTrans, sizeY, sizeX, alpha, A->matrix0, sizeY, x, 1,
                  beta, y, 1);
      break;
    /* SparseBlock storage */
    case NM_SPARSE_BLOCK:
    ...
    default:
      return numerics_error("NM_prod_mv_3x3", " vector prod(A,x,y) failed, unknown storage\n");
  }
...
void NM_row_prod_no_diag(size_t sizeX, size_t sizeY, int block_start, size_t row_start,
                         NumericsMatrix* A, double* restrict x, double* restrict y,
...
void NM_row_prod_no_diag3(size_t sizeX, int block_start, size_t row_start, NumericsMatrix* A,
      ...
      if (!B->internalData->dWork) {
        ...
        if (A->internalData->dWorkSize != B->internalData->dWorkSize) {
          ...
          B->internalData->dWork =
              realloc(B->internalData->dWork, A->internalData->dWorkSize * sizeof(double));
        }
      }
...
NumericsMatrix* NM_free_not_dense(NumericsMatrix* m) {
  ...
  NM_clear_other_storages(m, NM_DENSE);
  free(m);
  return NULL;
}
...
NumericsMatrix* NM_free_not_SBM(NumericsMatrix* m) {
  ...
  NM_clear_other_storages(m, NM_SPARSE_BLOCK);
  free(m);
  return NULL;
}
    ...
    case NM_SPARSE: {
      ...
      switch (M->matrix2->origin) {
        ...
        case NSM_HALF_TRIPLET: {
          assert(M->matrix2->half_triplet);
          insertion =
              1 - CSparseMatrix_symmetric_zentry(M->matrix2->triplet, i, j, val, threshold);
...
int NM_entry(NumericsMatrix* M, size_t i, size_t j, double val) {
  switch (M->storageType) {
    case NM_DENSE: {
      // column major
      M->matrix0[i + j * to_size_t(M->size0)] = val;
      break;
    }
    case NM_SPARSE_BLOCK: {
      CHECK_RETURN(SBM_entry(M->matrix1, i, j, val));
  ...
  if (to_int(i) > M->size0 - 1) {
    M->size0 = to_int(i) + 1;
  }
  if (to_int(j) > M->size1 - 1) {
    M->size1 = to_int(j) + 1;
  }
...
bool NM_compare(NumericsMatrix* A, NumericsMatrix* B, double tol) {
  ...
  for (int i = 0; i < A->size0; i++) {
    for (int j = 0; j < A->size1; j++) {
      /* DEBUG_PRINTF("error %i %i = %e\n",i,j, fabs(NM_get_value(A, i, j) - NM_get_value(B, i,
       * j))); */
...
void NM_display_storageType(const NumericsMatrix* const m) {
  ...
  switch (m->storageType) {
    ...
    default: {
      fprintf(stderr,
              "display_storageType for NumericsMatrix: matrix type %d not supported!\n",
              m->storageType);
    }
  }
}
...
void NM_read_in_file(NumericsMatrix* const m, FILE* file) {
  ...
  if (storageType == NM_DENSE) {
    ...
    SBM_read_from_file(m->matrix1, file);
...
NumericsMatrix* NM_create_from_file(FILE* file) { return NM_new_from_file(file); }
...
void NM_extract_diag_block3(NumericsMatrix* M, int block_row_nb, double** Block) {
  ...
  switch (storageType) {
    case NM_DENSE: {
      double* Mptr =
          M->matrix0 + (M->size0 + 1) * (block_row_nb + block_row_nb + block_row_nb);
      double* Bmat = *Block;
      /* The part of MM which corresponds to the current block is copied into MLocal */
      Bmat[0] = Mptr[0];
      Bmat[1] = Mptr[1];
...
void NM_copy_diag_block3(NumericsMatrix* M, int block_row_nb, double** Block) {
  ...
  switch (storageType) {
    ...
    case NM_SPARSE_BLOCK: {
      size_t diagPos = SBM_diagonal_block_index(M->matrix1, block_row_nb);
...
NumericsMatrix* NM_new_SBM(size_t size0, size_t size1, SparseBlockStructuredMatrix* m1) {
...
void NM_clearSparseBlock(NumericsMatrix* A) {
  A->matrix1 = SBM_free(A->matrix1, SBM_FREE_ALL);
  /* no need to reset version! */
}

void NM_clearSparse(NumericsMatrix* A) { A->matrix2 = NSM_free(A->matrix2); }
...
void NM_dense_to_sparse(NumericsMatrix* A, NumericsMatrix* B, double threshold) {
  ...
  for (int i = 0; i < A->size0; ++i) {
    for (int j = 0; j < A->size1; ++j) {
      CHECK_RETURN(CSparseMatrix_zentry(B->matrix2->triplet, i, j,
                                        A->matrix0[i + A->size0 * j], threshold));
    }
  }
    ...
    switch (A->storageType) {
      ...
      case NM_SPARSE_BLOCK: {
        ...
        if (A->matrix1) {
          ...
          for (size_t cr = 0; cr < A->matrix1->filled1 - 1; ++cr) {
            for (size_t bn = A->matrix1->index1_data[cr]; bn < A->matrix1->index1_data[cr + 1];
                 ++bn) {
              /* cc : current column */
              size_t cc = A->matrix1->index2_data[bn];
              size_t inbr = A->matrix1->blocksize0[cr];
              size_t roffset = 0;
              size_t coffset = 0;
              if (cr != 0) {
              ...
              size_t inbc = A->matrix1->blocksize1[cc];
              if (cc != 0) {
              ...
              for (unsigned j = 0; j < inbc; ++j) {
                for (unsigned i = 0; i < inbr; ++i) {
                  CHECK_RETURN(CSparseMatrix_entry(A->matrix2->triplet, i + roffset,
                                                   j + coffset,
          ...
          NSM_set_version(A->matrix2, NSM_TRIPLET, NM_version(A, NM_SPARSE_BLOCK));
        } else if (A->matrix0) {
          /* version set in NM_dense_to_sparse */
      ...
      case NM_SPARSE: {
        switch (A->matrix2->origin) {
          case NSM_CSC: {
            ...
            NSM_set_version(A->matrix2, NSM_TRIPLET, NSM_version(A->matrix2, NSM_CSC));
            break;
          }
          case NSM_CSR: {
            assert(A->matrix2->csr);
            A->matrix2->triplet = NM_csr_to_triplet(A->matrix2->csr);
            NSM_set_version(A->matrix2, NSM_TRIPLET, NSM_version(A->matrix2, NSM_CSR));
            break;
          }
          default:
          case NSM_UNKNOWN: {
...
CSparseMatrix* NM_half_triplet(NumericsMatrix* A) {
  ...
  if (numericsSparseMatrix(A)->half_triplet &&
      (NM_max_version(A) > NSM_version(numericsSparseMatrix(A), NSM_HALF_TRIPLET))) {
    NM_clearHalfTriplet(A);
  }
      ...
      case NM_SPARSE: {
        switch (A->matrix2->origin) {
          ...
          case NSM_CSC: {
            ...
            NSM_set_version(A->matrix2, NSM_HALF_TRIPLET, NSM_version(A->matrix2, NSM_CSC));
...
CSparseMatrix* NM_csc(NumericsMatrix* A) {
  ...
  assert(NSM_version(A->matrix2, NSM_TRIPLET) <= NSM_version(A->matrix2, NSM_CSC));
...
CSparseMatrix* NM_csr(NumericsMatrix* A) {
  ...
  if (!numericsSparseMatrix(A)->csr) {
    ...
    switch (A->matrix2->origin) {
      ...
      case NSM_UNKNOWN: {
        ...
        NSM_set_version(A->matrix2, NSM_CSR, NSM_version(A->matrix2, NSM_TRIPLET));
        break;
      }
  ...
  assert(NSM_version(A->matrix2, NSM_TRIPLET) <= NSM_version(A->matrix2, NSM_CSR));
...
void NM_gemv(const double alpha, NumericsMatrix* A, const double* x, const double beta,
             double* y) {
  assert(A);
  assert(x);
  assert(y);
  ...
  switch (A->storageType) {
    case NM_DENSE: {
      cblas_dgemv(CblasColMajor, CblasNoTrans, A->size0, A->size1, alpha, A->matrix0, A->size0,
                  x, 1, beta, y, 1);
...
void NM_tgemv(const double alpha, NumericsMatrix* A, const double* x, const double beta,
              double* y) {
  switch (A->storageType) {
    case NM_DENSE: {
      cblas_dgemv(CblasColMajor, CblasTrans, A->size0, A->size1, alpha, A->matrix0, A->size0,
                  x, 1, beta, y, 1);
      break;
    }
    case NM_SPARSE_BLOCK:
...
int NM_insert(NumericsMatrix* A, const NumericsMatrix* const B, const size_t start_i,
  ...
  size_t end_i = start_i + to_size_t(B->size0);
  size_t end_j = start_j + to_size_t(B->size1);
    ...
    case NM_SPARSE: {
      switch (B->matrix2->origin) {
        ...
        case NSM_CSC: {
          ...
          NSM_set_version(B->matrix2, NSM_TRIPLET, NSM_version(B->matrix2, NSM_CSC));

          break;
        }
        case NSM_CSR: {
          B->matrix2->triplet = NM_csr_to_triplet(B->matrix2->csr);
          NSM_set_version(B->matrix2, NSM_TRIPLET, NSM_version(B->matrix2, NSM_CSR));
          break;
        }
        default: {
      ...
      B->matrix2->origin = NSM_TRIPLET;
      assert(NSM_max_version(B->matrix2) == NSM_version(B->matrix2, B->matrix2->origin));
...
NumericsMatrix* NM_multiply(NumericsMatrix* A, NumericsMatrix* B) {
  ...
  switch (storageType) {
    case NM_DENSE: {
      ...
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, A->size0, B->size1, B->size0, 1.0,
                  A->matrix0, A->size0, B->matrix0, B->size0, 0.0, C->matrix0, A->size0);
...
void NM_gemm(const double alpha, NumericsMatrix* A, NumericsMatrix* B, const double beta,
             NumericsMatrix* C) {
  // Note FP: it would be better to have const A and B but the call to NM_csc
  // prevent this, just because of the change in version.
  NM_types storageType;
  ...
  switch (storageType) {
    case NM_DENSE: {
      ...
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, A->size0, B->size1, B->size0,
                  alpha, A->matrix0, A->size0, B->matrix0, B->size0, beta, C->matrix0,
                  A->size0);
    ...
    case NM_SPARSE_BLOCK: {
      ...
      C_tmp = SBM_free(C_tmp, SBM_FREE_ALL);
      NM_clearSparseBlock(C);
      NM_clearDense(C);
      NM_clearSparseStorage(C);
      C->matrix1 = result;
  ...
  if (!NM_LU_factorized(Ao)) {
    ...
    if (NM_internalData(Ao)->values_sha1_count > 0) {
      if (NM_check_values_sha1(Ao)) {
        numerics_warning("NM_LU_factorize",
    ...
    switch (A->storageType) {
      case NM_DENSE: {
        ...
        numerics_printf_verbose(2,
                                "NM_LU_factorize, we compute factors and keep them in place");
        ...
        if (info > 0) {
          fprintf(stderr,
                  "NM_LU_factorize: LU factorisation DGETRF failed. The %d-th diagonal "
                  "element is 0\n",
                  info);
        } else if (info < 0) {
          printf("info = %d\n", info);
...
int NM_LU_solve(NumericsMatrix* Ao, double* b, size_t nrhs) {
  lapack_int info = 1;
  ...
  if (NM_LU_factorized(A)) {
    ...
    switch (A->storageType) {
      case NM_DENSE: {
        ...
        if (info < 0) {
          numerics_printf_verbose(2,
                                  "NM_LU_solve: dense LU solve DGETRS failed. The %d-th "
        ...
        switch (p->solver) {
          case NSM_CSPARSE: {
            ...
            for (size_t j = 0; j < nrhs; j++) {
              info = !CSparseMatrix_solve((CSparseMatrix_factors*)NSM_linear_solver_data(p),
        ...
        case NM_SPARSE: {
          ...
          switch (p->solver) {
            case NSM_CSPARSE: {
              ...
              numerics_printf_verbose(2,
                                      "NM_LU_solve_matrix_rhs, we solve with given factors");
              ...
              info = !CSparseMatrix_spsolve((CSparseMatrix_factors*)NSM_linear_solver_data(p),
                                            X, NM_csc(B));
              // Invalidation
              cs_spfree(X);
      ...
      if (keep == NM_KEEP_FACTORS) {
        ...
        if (!NM_internalData(A)->isLUfactorized) {
          ...
          if (info > 0) {
            if (verbose >= 2) {
              printf(
                  "NM_gesv: LU factorisation DGETRF failed. The %d-th diagonal element is 0\n",
                  info);
            }
          } else if (info < 0) {
            fprintf(stderr,
                    "NM_gesv: LU factorisation DGETRF failed. The %d-th argument has an "
                    "illegal value, stopping\n",
                    -info);
          } else {
            if (verbose >= 2) {
        ...
        if (info < 0) {
          if (verbose >= 2) {
            printf(
                "NM_gesv: dense LU solve DGETRS failed. The %d-th argument has an illegal "
                "value, stopping\n",
                -info);
          }
        }
        ...
        DGESV(A->size0, 1, mat, A->size0,
              (lapack_int*)NM_iWork(A, A->size0, sizeof(lapack_int)), b, A->size0, &info);
      }
      break;
    }
    ...
    case NM_SPARSE: {
      ...
      switch (p->solver) {
        ...
        case NSM_UMFPACK: {
          ...
          info = (int)UMFPACK_FN(wsolve)(UMFPACK_A, C->p, C->i, C->x, umfpack_ws->x, b,
                                         umfpack_ws->numeric, umfpack_ws->control,
                                         umfpack_ws->info, umfpack_ws->wi, umfpack_ws->wd);
      ...
      if (keep == NM_KEEP_FACTORS) {
        ...
        if (!NM_internalData(A)->isLUfactorized) {
          ...
          if (info > 0) {
            if (verbose >= 2) {
              printf(
                  "NM_posv: Cholesky factorisation DPOTRF failed. The %d-th diagonal element "
                  "is 0\n",
                  info);
            }
          } else if (info < 0) {
            fprintf(stderr,
                    "NM_posv: Cholesky factorisation DPOTRF failed. The %d-th argument has an "
                    "illegal value, stopping\n",
                    -info);
          }
        ...
        if (info < 0) {
          if (verbose >= 2) {
            printf(
                "NM_posv: dense LU solve DGETRS failed. The %d-th argument has an illegal "
                "value, stopping\n",
                -info);
          }
        }
        ...
        if (info > 0) {
          ...
          fprintf(stderr,
                  "NM_posv: Cholesky solver DPOSV failed. The %d-th argument has an illegal "
                  "value, stopping\n",
                  -info);
        }
      }
      ...
      switch (p->solver) {
        case NSM_CSPARSE:
          ...
          if (keep == NM_KEEP_FACTORS) {
            ...
            info = !CSparseMatrix_chol_solve((CSparseMatrix_factors*)NSM_linear_solver_data(p),
                                             NSM_workspace(p), b);
          } else {
...
NumericsMatrix* NM_inverse_diagonal_block_matrix(NumericsMatrix* A, size_t block_number,
  ...
  switch (A->storageType) {
    case NM_SPARSE_BLOCK: {
      ...
      lapack_int* ipiv =
          (lapack_int*)NM_iWork(A_inv, to_size_t(A_inv->size0), sizeof(lapack_int));
    ...
    case NM_SPARSE: {
      ...
      size_t block_size_max = blocksizes[0];
      size_t block_size_min = blocksizes[0];
      ...
      /* When the matrix is block 3x3 diagonal */
        ...
        size_t start_row = 0;
        for (size_t b = 0; b < block_number; b++) {
          const int block_size = to_int(blocksizes[b]);
          ...
          start_row = start_row + to_size_t(block_size);
        }
      }
      break;
    }
...
int NM_compute_balancing_matrices(NumericsMatrix* A, double tol, int itermax,
  ...
  size_t size0 = B->size0;
  size_t size1 = B->size1;
...
int NM_Cholesky_solve(NumericsMatrix* Ao, double* b, size_t nrhs) {
  lapack_int info = 1;
  /* factorization is done on destructible part only if
   * !A->internalData->isLUfactorized */
...
int NM_LDLT_solve(NumericsMatrix* Ao, double* b, size_t nrhs) {
  lapack_int info = 1;
  /* factorization is done on destructible part only if
   * !A->internalData->isLUfactorized */
  ...
  switch (H->storageType) {
    case NM_SPARSE: {
      ...
      if (H->matrix2->origin == NSM_TRIPLET) {
        ...
        for (size_t i = 0; i < n_cones_to_clear; i++) {
...
int NM_is_diagonal_block_matrix(NumericsMatrix* A, size_t* block_number, size_t** blocksizes) {
  ...
  size_t n_component = len_connectedcomponents(connectedcomponents);
  /* print_connectedcomponents(connectedcomponents); */

  int is_diagonal_block_matrix = 1;

  *block_number = 0;
  *blocksizes = (size_t*)malloc(n_component * sizeof(size_t));
  ...
  while (temp != NULL) {
    ...
    size_t len = len_connectedcomponent(connectedcomponent);
