...
void newton_LSA(unsigned n, double* z, double* F, int* info, void* data,
  ...
  assert(((functions->compute_descent_direction) ||
          ...
          functions->compute_H) &&
         "functions_LSA lacks a way to compute a descente direction");
  ...
  if (options->iparam[SICONOS_IPARAM_LSA_SEARCH_CRITERION] == SICONOS_LSA_GOLDSTEIN) {
    goldstein_extra_params* pG =
        (goldstein_extra_params*)malloc(sizeof(goldstein_extra_params));
  ...
  while ((iter < itermax) && (err > tol)) {
    ...
    if (functions->compute_descent_direction) {
      ...
      if (functions->compute_RHS_desc)  // different merit function for the descent
                                        // calc.(usually min)
      {
        ...
        if (log_hdf5) {
          ...
          SN_LOG_VEC(log_hdf5,
                     SN_logh5_vec_double(n, F_merit, "F_merit_desc", logger_s->group));
        }
    ...
    if (!info_dir_search && log_hdf5)
      SN_LOG_VEC(log_hdf5, SN_logh5_vec_double(n, workV1, "desc_direction", logger_s->group));
    ...
    if (functions->compute_RHS_desc ||
        functions->compute_descent_direction)  // need to compute H and F_merit for the merit
    {
      // /!\ maide! workV1 cannot be used since it contains the descent
      // direction ...

      if (functions->compute_JacTheta_merit) {
        functions->compute_JacTheta_merit(data, z, F, F_merit, workV2, JacThetaF_merit,
                                          options);
      } else {
    ...
    if (log_hdf5) {
      SN_LOG_VEC(log_hdf5,
                 SN_logh5_vec_double(n, JacThetaF_merit, "JacThetaF_merit", logger_s->group));
      SN_LOG_SCALAR(log_hdf5, SN_logh5_scalar_integer(info_dir_search, "info_dir_search_solve",
                                                      logger_s->group));
    }
    ...
    if (info_dir_search) {
      ...
      {
        numerics_printf(
            "functions->compute_RHS_desc : no  descent direction found! searching for merit "
            "descent direction");
        cblas_dcopy((int)n, F_merit, incx, workV1, incy);
        cblas_dscal((int)n, -1.0, workV1, incx);
        // info_dir_search = NM_gesv(H, workV1, params->keep_H);
        ...
        if (log_hdf5) {
          SN_LOG_SCALAR(log_hdf5, SN_logh5_scalar_integer(info_dir_search,
                                                          "info_dir_search_solve_meritdesc",
                                                          logger_s->group));
          if (!info_dir_search)
    ...
    tau = 1.0;
    if ((theta_iter > params->sigma * theta) ||
        (info_dir_search > 0 &&
         functions->compute_RHS_desc))  // Newton direction not good enough or min part failed
    {
      if (log_hdf5) {
        SN_LOG_SCALAR(log_hdf5,
                      SN_logh5_scalar_double(theta_iter, "theta_iter", logger_s->group));
        SN_LOG_SCALAR(log_hdf5,
                      SN_logh5_scalar_double(params->sigma * theta, "theta_iter_threshold",
                                             logger_s->group));
      }

      numerics_printf_verbose(2,
                              "--- newton_LSA :: pure Newton direction not acceptable "
                              "theta_iter = %g > %g = theta",
                              theta_iter, theta);
      ...
      if (log_hdf5) {
        SN_LOG_SCALAR(log_hdf5,
                      SN_logh5_scalar_double(preRHS, "preRHS_newton", logger_s->group));
        SN_LOG_SCALAR(log_hdf5,
                      SN_logh5_scalar_double(threshold, "preRHS_threshold", logger_s->group));
      }
    ...
    if (options->iparam[SICONOS_IPARAM_STOPPING_CRITERION] ==
        SICONOS_STOPPING_CRITERION_RESIDU) {
      err = norm_F_merit;
    } else if (options->iparam[SICONOS_IPARAM_STOPPING_CRITERION] ==
               SICONOS_STOPPING_CRITERION_STATIONARITY) {
