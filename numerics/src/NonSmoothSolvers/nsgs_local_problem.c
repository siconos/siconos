...
NSGSLocalProblem* nsgs_local_problem_create(void* global_problem, unsigned int block_id,
  ...
  if (!global_problem || !ops) {
    numerics_error_log("nsgs_local_problem_create", "NULL arguments");
    return NULL;
  }
  ...
  size_t dim = to_size_t(ops->dimension);
...
void nsgs_local_problem_update(NSGSLocalProblem* local, const double* global_sol) {
  ...
  if (local->ops.update) {
    local->ops.update(local->global_problem, local->block_id, global_sol, local->local_rhs,
                      local->ops.dimension, local->workspace);
  }

  if (local->ops.extract) {
    local->ops.extract(local->global_problem, local->block_id, local->local_mat,
                       local->ops.dimension);
  }
}
...
int nsgs_local_problem_solve(NSGSLocalProblem* local, double* result, SolverOptions* options) {
  ...
  if (!local->solver) {
    numerics_error_log("nsgs_local_problem_solve", "No local solver configured");
...
static const NSGSLocalProblemOps fc3d_ops = {.update = default_update,
...
const NSGSLocalProblemOps* nsgs_fc3d_local_ops(void) { return &fc3d_ops; }

static const NSGSLocalProblemOps fc2d_ops = {.update = default_update,
                                             .extract = default_extract,
                                             .solve = NULL,
                                             .type = NSGS_LP_FC2D,
                                             .dimension = 2,
...
const NSGSLocalProblemOps* nsgs_fc2d_local_ops(void) { return &fc2d_ops; }

static const NSGSLocalProblemOps rfc3d_ops = {.update = default_update,
                                              .extract = default_extract,
                                              .solve = NULL,
                                              .type = NSGS_LP_RFC3D,
                                              .dimension = 5,
...
const NSGSLocalProblemOps* nsgs_rfc3d_local_ops(void) { return &rfc3d_ops; }

static const NSGSLocalProblemOps plasticity_2d_ops = {.update = default_update,
                                                      .extract = default_extract,
                                                      .solve = NULL,
...
const NSGSLocalProblemOps* nsgs_plasticity_2d_local_ops(void) { return &plasticity_2d_ops; }
