...
#include "NonSmoothSolvers/NonSmoothNewton.h"              // for nonSmoothNewton, nonSmoothN...
...
void mcp_old_FischerBurmeister_init(MixedComplementarityProblem_old* problem,
...
void mcp_old_FischerBurmeister_reset(MixedComplementarityProblem_old* problem,
                                     SolverOptions* options) {
...
void nablaFischerFunc_MCP(int size, double* z, double* nablaPhi, int dummy) {
  ...
  jacobianPhi_Mixed_FB(sizeEq, sizeIneq, z, localProblem->Fmcp, localProblem->nablaFmcp,
                       nablaPhi);
}

void mcp_old_FischerBurmeister(MixedComplementarityProblem_old* problem, double* z, double* w,
  ...
  *info = nonSmoothNewton(fullSize, z, &phi, &nablaPhi, options);
  ...
  if (*info > 0)
    fprintf(stderr,
            "Numerics, mcp_FB failed, reached max. number of iterations without convergence. "
            "Residual = %f\n",
  ...
  if (error > tolerance) {
    numerics_printf("mcp_old_FischerBurmeister : error = %e > tolerance = %e.", error,
                    tolerance);
    *info = 1;
  } else {
    numerics_printf("mcp_old_FischerBurmeister : error = %e < tolerance = %e.", error,
