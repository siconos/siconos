...
/*!\file SOCLCP_Solvers.h
  \brief Subroutines for the resolution of Second Order Cone Linear
  Complementarity Problem (SOCLCP).
...
typedef int (*ComputeError_soclcp_Ptr)(
...
/**
    ...
    SICONOS_ERROR_FULL_EVALUATION Complete error computation with v computation
    (Default) SICONOS_ERROR_LIGHT_EVALUATION for Light error computation with
    incremental values on r verification of absolute error at the end
    SICONOS_ERROR_LIGHT_EVALUATION_NO_UPDATE for light error computation,
    without update for v [out]iparam[7] = iter number of performed iterations
...
/* void soclcp_nsgs_v(SecondOrderConeLinearComplementarityProblem* problem,
 * double *r, double *v, int* info, SolverOptions* options); */
...
/*     \param options the solver options : */
/* *\/ */
/* void soclcp_proximal(SecondOrderConeLinearComplementarityProblem* problem,
 * double *r, double *v, int* info, SolverOptions* options); */
...
/*   \param options the solver options : */
/* *\/ */
/* void soclcp_TrescaFixedPoint(SecondOrderConeLinearComplementarityProblem*
 ...
 * maximum number of iteration is the line--search. */
/*  *   The internal (local) solver must set by the SolverOptions
...
/*     dparam[3] : rho >0 */
/* *\/ */
/* void soclcp_HyperplaneProjection(SecondOrderConeLinearComplementarityProblem*
 * problem, double *r, double *v, int* info, SolverOptions* options); */
