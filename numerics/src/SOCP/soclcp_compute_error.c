...
int soclcp_compute_error(SecondOrderConeLinearComplementarityProblem *problem, double *z,
  ...
  *error = 0.;
  ...
  for (ic = 0; ic < nc; ic++) {
    ...
    soclcp_unitary_compute_and_add_error(
        z + problem->coneIndex[ic], w + problem->coneIndex[ic], dim, mu[ic], error, worktmp);
    /* for (int i=0; i < dim; i++ ) */
    /* { */
...
int soclcp_compute_error_v(SecondOrderConeLinearComplementarityProblem *problem, double *z,
  ...
  *error = *error / (norm_q + 1.0);
  if (*error > tolerance) {
    /*      if (verbose > 0) printf(" Numerics - soclcp_compute_error_velocity failed: error =
     * %g > tolerance = %g.\n",*error, tolerance); */
    return 1;
  } else
    return 0;
