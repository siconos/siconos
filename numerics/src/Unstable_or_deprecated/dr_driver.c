...
int dr_driver(RelayProblem* problem, double* z, double* w, SolverOptions* options) {
  ...
  if (problem == NULL || z == NULL || w == NULL)
    numerics_error("dr_driver",
                   "null input for LinearComplementarityProblem and/or unknowns (z,w)");
  ...
  /****** NLGS algorithm ******/
  if (options->solverId == SICONOS_RELAY_NLGS) dr_nlgs(problem, z, w, &info, options);
