...
void lcp_qp(LinearComplementarityProblem *problem, double *z, double *w, int *info,
  ...
  ql0001(&m, &me, &mmax, &n, &nmax, &mnn, Q, p, A, b, xl, xu, z, lambda, &iout, info, &un, war,
         &lwar, iwar, &liwar, &tol);
#else
  (void)numerics_error("lcp_qp", "Fortran language is not enabled in siconos numerics");
  return;
#endif
#else
  (void)numerics_error("lcp_qp", "ql0001 is not available in siconos numerics");
  return;
#endif
  /* /    printf("tol = %10.4e\n",*tol);
...
REGISTER_SOLVER(SICONOS_LCP_QP, "LCP_QP", "Quadratic Programming solver for LCP",
                lcp_qp_init_wrap, lcp_qp_solve_wrap, lcp_qp_free_wrap,
                NULL,               /* error function */
