...
static int lcp_pgs_solve_wrap(void* problem, double* z, double* w, SolverOptions* options) {
  ...
  lcp_pgs_graph_permut((LinearComplementarityProblem*)problem, z, w, &info, options);
  return info;
}
...
REGISTER_SOLVER(SICONOS_LCP_PGS_GRAPH_PERMUT, "SICONOS_LCP_PGS_GRAPH_PERMUT",
                "SICONOS_LCP_PGS_GRAPH_PERMUT", lcp_pgs_init_wrap, lcp_pgs_solve_wrap,
