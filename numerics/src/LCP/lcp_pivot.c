...
void lcp_pivot_covering_vector(LinearComplementarityProblem* problem, double* restrict u,
  ...
  *info = 0;
  ...
  switch (pivot_selection_rule) {
    ...
    case SICONOS_LCP_PIVOT_PATHSEARCH:
      ...
      basis_init = 1;
      DEBUG_EXPR_WE(
          DEBUG_PRINT("basis after hot start: ") for (unsigned int i = 0; i < dim; ++i){
...
void lcp_pivot_set_default(SolverOptions* options) {
  options->iparam[SICONOS_LCP_IPARAM_PIVOTING_METHOD_TYPE] =
      SICONOS_LCP_PIVOT_LEMKE;  // default
