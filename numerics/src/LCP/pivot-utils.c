...
const char* basis_to_name(unsigned nb, unsigned n) {
  if (nb < n + BASIS_OFFSET)
    return "w";
  else if (nb > n + BASIS_OFFSET)
    return "z";
  else
    return "e";
}
...
unsigned basis_to_number(unsigned nb, unsigned n) {
  if (nb < n + BASIS_OFFSET)
    return nb + 1 - BASIS_OFFSET;
  else if (nb > n + BASIS_OFFSET)
    return nb - n - BASIS_OFFSET;
  else
    return 0;
}
...
int lexicosort_lumod(unsigned* vars, unsigned n, unsigned nb_candidates,
                     SN_lumod_dense_data* restrict lumod_data, unsigned* restrict basis,
                     double* restrict lexico_col, double* restrict driving_col,
                     double lexico_tol);
...
int lexicosort_lumod(unsigned* vars, unsigned n, unsigned nb_candidates,
  ...
  DEBUG_EXPR_WE(DEBUG_PRINT("lexicosort_lumod :: candidates: ");
                for (unsigned i = 0; i < nb_candidates; ++i) {
                  DEBUG_PRINTF("%s%d ", basis_to_name(basis[vars[i]], n),
                               basis_to_number(basis[vars[i]], n));
                } DEBUG_PRINT("\n"));
  ...
  for (unsigned i = 0; i < n; ++i) {
    ...
    if (basis[i] == i + BASIS_OFFSET)
      ...
      ++var_indx;
      for (unsigned next_var = vars[var_indx]; var_indx < nb_remaining_vars;
           next_var = vars[++var_indx]) {
        ...
        if (zero_current_lexico_col_element) {
          if (next_lexico_col_element == 0.) /* equality  */
          {
            ...
            DEBUG_PRINTF(
                "lexicosort_lumod :: adding var %s%d because of equality (both row elements "
                "are 0)\n",
                basis_to_name(basis[next_var], n), basis_to_number(basis[next_var], n));
          } else if (next_lexico_col_element <
                     -lexico_tol) /* delta_lexico > 0 (since pivot are always >0., => new
                                     lexicomin  */
          {
        ...
        else if ((next_lexico_col_element == 0.) &&
                 (current_lexico_col_element > lexico_tol)) {
          zero_current_lexico_col_element = 1;
          goto update_current_blocking_variable;
        } else /* we need to actually compute delta_block, both element of the lexico_col are
                  non-zero */
        {
          double candidate_pivot = driving_col[next_var];
          double delta_lexico = current_lexico_col_element * candidate_pivot -
                                next_lexico_col_element * current_pivot;
          if (delta_lexico > lexico_tol) /* Difference is significant */
          {
            DEBUG_PRINTF(
                "lexicosort_lumod :: lexicomin change var block changes to %s%d from %s%d, "
                "delta_lexico = %2.2e, new pivot = %e\n",
                basis_to_name(basis[next_var], n), basis_to_number(basis[next_var], n),
                basis_to_name(basis[vars[staying_var_indx]], n),
                basis_to_number(basis[vars[staying_var_indx]], n), delta_lexico,
                candidate_pivot);
            zero_current_lexico_col_element = 0;
            goto update_current_blocking_variable;
          ...
          {
            if ((current_pivot < SMALL_PIVOT) &&
                (candidate_pivot > MIN_INCREASE * current_pivot)) {
              DEBUG_PRINTF(
                  "lexicosort_lumod :: lexicomin small difference %2.2e, taking largest pivot "
                  "%e > %e (var %s%d vs %s%d)\n",
                  delta_lexico, candidate_pivot, current_pivot,
                  basis_to_name(basis[next_var], n), basis_to_number(basis[next_var], n),
                  basis_to_name(basis[vars[staying_var_indx]], n),
                  basis_to_number(basis[vars[staying_var_indx]], n));
              ...
              vars[++staying_var_indx] = next_var;
              DEBUG_PRINTF(
                  "lexicosort_lumod :: adding var %s%d because of inconclusive test: "
                  "delta_lexico = %2.2e\n",
                  basis_to_name(basis[next_var], n), basis_to_number(basis[next_var], n),
                  delta_lexico);
            }
      ...
      update_current_blocking_variable:
        DEBUG_PRINTF(
            "lexicosort_lumod :: lexicomin blocking variable changes to %s%d from %s%d\n",
            basis_to_name(basis[next_var], n), basis_to_number(basis[next_var], n),
...
static inline int lexicosort_rowmajor(unsigned var, int block, unsigned n, double* lexico_mat,
                                      double candidate_pivot, double current_pivot,
                                      double lexico_tol) {
  assert(lexico_tol >= 0.);
  ...
  for (unsigned j = 0; j < n; ++j) {
    ...
    DEBUG_EXPR_WE(if ((dblock != 0.) && (fabs(dblock) < LEXICO_TOL_DISPLAY)) {
      printf("lexicosort_rowmajor :: very small difference in lexicomin search: %2.2e\n",
             dblock);
      unsigned block_number = basis_to_number(basis_global[block], n);
      unsigned var_number = basis_to_number(basis_global[var], n);
      char* block_name = basis_to_name(basis_global[block], n);
      char* var_name = basis_to_name(basis_global[var], n);
      printf(
          "lexicomin: A[%s%d][j] / A[%s%d][drive] = %e / %e vs A[%s%d][j] / A[%s%d][drive] = "
          "%e / %e\n",
          block_name, block_number, block_name, block_number, lexico_row_block[j],
          current_pivot, var_name, var_number, var_name, var_number, lexico_row_var[j],
          candidate_pivot);
    });
    if (dblock < -lexico_tol)
      return block;               /* Unchanged blocking variable */
    else if (dblock > lexico_tol) /* Difference is significant */
    {
      DEBUG_PRINTF(
          "pivot_selection_lemke :: lexicomin change var block changes to %s%d from %s%d, "
          "dblock = %2.2e, new pivot = %e\n",
          basis_to_name(basis_global[var], n), basis_to_number(basis_global[var], n),
          basis_to_name(basis_global[block], n), basis_to_number(basis_global[block], n),
          dblock, candidate_pivot);
      return var;
    } else if (dblock != 0.) {
      if ((current_pivot < SMALL_PIVOT) && (candidate_pivot > MIN_INCREASE * current_pivot)) {
        DEBUG_PRINTF(
            "pivot_selection_lemke :: lexicomin small difference %2.2e, taking largest pivot "
            "%e > %e (var %s%d vs %s%d)\n",
            dblock, candidate_pivot, current_pivot, basis_to_name(basis_global[var], n),
            basis_to_number(basis_global[var], n), basis_to_name(basis_global[block], n),
            basis_to_number(basis_global[block], n));
        DEBUG_EXPR_WE(max_pivot_helped = 1;);
        return var;
      }
    }
  ...
  DEBUG_PRINTF(
      "lexicosort_rowmajor :: reaching end of function, which means that the lexicomin "
      "failed!, block = %d, var = %d\n",
      block, var);
  ...
  *t_indx = block;
  ...
  if (block >= 0) {
    ...
    if (t_min >= 1.0) {
      ...
      DEBUG_PRINTF(
          "pivot_init_pathsearch :: expected t value = %.*le; blocking variable indx = %d\n",
      ...
      if (candidate_ratio > ratio)
        ...
        if (block == (int)aux_indx || i == aux_indx) {
          ...
          for (unsigned j = 1; j <= dim; ++j) {
            ...
            DEBUG_EXPR_WE(
                if (fabs(dblock) < TOL_LEXICO) printf(
                    "pivot_selection_lemke :: very small difference in lexicomin: %2.2e\n",
                    dblock););
            if (dblock < 0.)
              break;
            else if (dblock > 0.) {
...
int pivot_selection_lemke2(unsigned n, double* restrict col_drive, double* restrict q_tilde,
                           double* restrict lexico_mat, unsigned aux_indx, double lexico_tol) {
  int block = -1;
    ...
    if (candidate_pivot > 0.) {
      ...
      if (candidate_ratio > ratio)
        ...
        if (block == (int)aux_indx || i == aux_indx) {
          ...
          if (nb_candidate < SIZE_CANDIDATE_PIVOT - 1)
            ...
            for (unsigned k = 0; k < 5; ++k) {
              ...
              block = lexicosort_rowmajor(var, block, n, lexico_mat, test_pivot, current_pivot,
                                          lexico_tol);
              if (block == (int)var) {
  ...
  DEBUG_EXPR_WE(if (max_pivot_helped) {
    DEBUG_PRINT("pivot_selection_lemke2 :: lexicomin MAX PIVOT HELPED!\n");
  });
  return block;
}

int pivot_selection_lemke3(unsigned n, double* restrict col_drive, double* restrict q_tilde,
                           double* restrict lexico_col, unsigned* restrict basis,
                           unsigned* restrict candidate_indx,
                           SN_lumod_dense_data* restrict lumod_data, unsigned aux_indx,
                           double lexico_tol) {
  int block = -1;
  ...
  if (nb_candidates > 1) {
    DEBUG_PRINTF("pivot_selection_lemke3 :: lexicomin %d candidates, ratio = %e\n",
                 nb_candidates, ratio);
    return lexicosort_lumod(candidate_indx, n, nb_candidates, lumod_data, basis, lexico_col,
                            col_drive, lexico_tol);
    DEBUG_EXPR_WE(if (max_pivot_helped) {
      DEBUG_PRINT("pivot_selection_lemke3 :: lexicomin MAX PIVOT HELPED!\n");
    });
  } else {
    return block;
  }
...
void init_M_lemke(double* restrict mat, double* restrict M, unsigned int dim,
                  unsigned int size_x, double* restrict q, double* restrict d) {
...
void init_M_bard(double* restrict mat, double* restrict M, unsigned int dim,
                 double* restrict q) {
  /* construction of mat matrix such that
   * mat = [ q | Id | -M ]
   */
...
void init_M_least_index(double* restrict mat, double* restrict M, unsigned int dim,
                        double* restrict q) {
  /* construction of mat matrix such that
   * mat = [ q | M ]
   */
  ...
  for (unsigned int i = 0; i < dim; ++i) {
    mat[i] = q[i];
    for (unsigned int j = 0; j < dim * dim; j += dim) mat[i + j + dim] = M[j + i];
  }
}

int init_M_lemke_warm_start(int n, double* restrict u, double* restrict mat,
                            double* restrict M, double* restrict q, int* restrict basis,
  ...
  double* d = &mat[(n + 1) * n];
  if (cov_vec)
    cblas_dcopy_msan(n, cov_vec, 1, d, 1);
  else
  ...
  for (int i = 0; i < n; ++i) {
    ...
    } else /* s[i] > 0.0 and if both s and u are nonbasic, we insert a column from the identity
            * matrix this choice could ne different M_bas[:, i] = -I[:, i] */
    {
      basis[i] = i + 1;
...
void do_pivot_driftless2(double* mat, unsigned int dim, unsigned int dim2, unsigned int block,
                         unsigned int drive) {
  double tmp;
  double pivot_inv = 1.0 / mat[block + drive * dim];
  ...
  for (unsigned int i = 0; i < block; ++i) {
    ...
    for (unsigned int j = drive + 1; j < dim2; ++j)
      mat[i + j * dim] -= tmp * mat[block + j * dim];
  }
  for (unsigned int i = block + 1; i < dim; ++i) {
    tmp = mat[i + drive * dim];
    for (unsigned int j = 0; j < drive; ++j) mat[i + j * dim] -= tmp * mat[block + j * dim];
...
void do_pivot(double* mat, unsigned int dim, unsigned int dim2, unsigned int block,
              unsigned int drive) {
  double tmp;
  double pivot_inv = 1.0 / mat[block + drive * dim];
...
void do_pivot_lumod(SN_lumod_dense_data* restrict lumod_data, NumericsMatrix* restrict M,
                    double* restrict q_tilde, double* restrict lexico_mat,
                    double* restrict col_drive, double* restrict col_tilde, unsigned* basis,
                    unsigned block, unsigned drive) {
  ...
  unsigned pos_leaving_var_in_factorized_basis =
      lumod_data->factorized_basis[leaving_var_indx];
  ...
  switch (leaving_type + entering_type) {
    ...
    case 1:
      ...
      SN_lumod_add_row_col(lumod_data, pos_leaving_var_in_factorized_basis - BASIS_OFFSET,
                           col_tilde);
      break;
    case 2: {
      ...
      if (index_row < (int)lumod_data->k) {
        unsigned changed_row =
            SN_lumod_find_arg_var(lumod_data->row_col_indx, lumod_data->k, n);
    ...
    case 3:
      ...
      assert(lumod_data->row_col_indx[entering_var_indx] >= 0);
      assert(lumod_data->row_col_indx[leaving_var_indx] == 0);
      SN_lumod_replace_row(lumod_data, lumod_data->row_col_indx[entering_var_indx],
                           pos_leaving_var_in_factorized_basis - BASIS_OFFSET);
...
void lcp_pivot_diagnose_info(int info) {
  switch (info) {
    ...
    case LCP_PATHSEARCH_LEAVING_T:
      printf(
          "lcp_pivot :: the pathsearch was not successful since the t variable was leaving\n");
      break;
