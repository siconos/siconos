...
int lcp_enum_getNbDWork(LinearComplementarityProblem* problem, SolverOptions* options) {
  ...
  if (options->iparam[SICONOS_LCP_IPARAM_ENUM_USE_DGELS]) {
    ...
    /* int info = 0; */
    /* double dgelsSize = 0; */
    /* DGELS(problem->M->size0, problem->size , 1, 0, problem->M->size0, 0, problem->M->size0,
     * &dgelsSize, LWORK, &info); */
    /* aux += (int) dgelsSize; */
    /* LWORK = (int) dgelsSize; */
  }
  return aux;
...
void lcp_enum(LinearComplementarityProblem* problem, double* z, double* w, int* info,
  ...
  *info = 1;
  ...
  if (verbose) numerics_printf("lcp_enum begin, size %d tol %e", size, tol);
  ...
  *info = 0;
  ...
  while (enum_next(zw_indices, size, enum_struct)) {
    ...
    if (useDGELS) {
      ...
      DGELS(LA_NOTRANS, size, size, NRHS, M_linear_system, size, q_linear_system, size,
            &LAinfo);
      if (verbose) {
    ...
    if (!LAinfo) {
      ...
      if (!check)
        ...
        if (verbose || multipleSolutions) {
          numerics_printf("lcp_enum find %i solution with scurrent = %ld!", numberofSolutions,
                          enum_struct->current - 1);
        }
        *info = 0;
