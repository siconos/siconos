  ...
  */
  ...
  while ((iter < itermax) && (hasNotConverged > 0)) {
    ...
    for (rowNumber = 0; rowNumber < blmat->blocknumber0; ++rowNumber) {
      ...
      options[0].iparam[SICONOS_LCP_IPARAM_NSGS_ITERATIONS_SUM] +=
          current_local_options->iparam[SICONOS_IPARAM_ITER_DONE];
      /* sum of local errors (output from local_driver)*/
      options[0].dparam[SICONOS_LCP_DPARAM_NSGS_LOCAL_ERROR_SUM] +=
          current_local_options->iparam[SICONOS_DPARAM_RESIDU];
      ...
      if (infoLocal > 0) {
        ...
        fprintf(stderr,
                "lcp_NSGS_SBM error: Warning local LCP solver  at global iteration %d.\n for "
    ...
    numerics_printf_verbose(1, "---- LCP - NSGS SBM  - Iteration %i  full error = %14.7e",
                            iter, error);
  }
