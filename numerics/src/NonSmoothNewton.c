...
void linesearch_Armijo(int n, double* z, double* dir, double psi_k, double descentCondition,
  ...
  */
  ...
  while (tk > tmin) {
    ...
    warning: (-tk) because we need to start from z0 at each step while z_k+1 is saved in place
    of z_k ...*/
  ...
  if (tk <= tmin)
    numerics_printf(
        "Non Smooth Newton:\t\t linesearch_Armijo warning, resulting tk < tmin, linesearch "
        "stopped.");
  else
    numerics_printf_verbose(
        2, "Non Smooth Newton:\t\tlinesearch_Armijo succeeded with tk = %e", tk);
}
...
int nonSmoothNewton(int n, double* z, NewtonFunctionPtr* phi, NewtonFunctionPtr* jacobianPhi,
  ...
  while ((niter < itermax) && (terminationCriterion > tolerance)) {
    ...
    cblas_dgemv(CblasColMajor, CblasTrans, n, n, 1.0, jacobianPhiMatrix, n, phiVector, incx,
                0.0, gradient_psi, incx);
    ...
    if (terminationCriterion < tolerance) break;
    ...
    numerics_printf(
        "Non Smooth Newton: iteration number %i, norm merit function = %e, norm grad. merit "
        "function = %14.7e .",
        niter, normPhi, norm_gradient_psi);
  }
...
int nonSmoothDirectNewton(int n, double* z, NewtonFunctionPtr* phi,
  ...
  while ((niter < itermax) && (terminationCriterion > tolerance)) {
    ...
    (*phi)(n, z, phiVector, 0);
    (*jacobianPhi)(n, z, jacobianPhiMatrix, 1);
    /* Computes the jacobian of the merit function, gradient_psi =
    ...
    */
    ...
    numerics_printf(
        "Non Smooth Newton: iteration number %i, norm of the merit function = %14.7e .", niter,
