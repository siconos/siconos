  ...
  *error = cblas_dnrm2(n, wtmp, incx);
  ...
  if (*error > tolerance) {
    numerics_printf_verbose(
        2, " Numerics - convexQP_compute_error: error = %g > tolerance = %g.\n", *error,
        tolerance);
    return 1;
  } else
    return 0;
...
int convexQP_compute_error(ConvexQP *problem, double *z, double *xi, double *w, double *u,
  ...
  *error = cblas_dnrm2(n, tmp_n, incx);
  ...
  if (fabs(relative_scaling) > DBL_EPSILON) *error = *error / relative_scaling;
  ...
  if (fabs(relative_scaling) > DBL_EPSILON) tmp = tmp / relative_scaling;
