...
void convexQP_ADMM_init(ConvexQP* problem, SolverOptions* options) {
  ...
  /* if  (options->iparam[SICONOS_CONVEXQP_ADMM_IPARAM_ACCELERATION] ==
...
void convexQP_ADMM(ConvexQP* problem, double* z, double* w, double* xi, double* u, int* info,
  ...
  while ((iter < itermax) && (hasNotConverged > 0)) {
    ...
    } else if (options->iparam[SICONOS_CONVEXQP_ADMM_IPARAM_ACCELERATION] ==
    ...
    numerics_printf_verbose(
        1,
        "---- ConvexQP - ADMM  - Iteration %i rho = %14.7e \t residual = %14.7e, tol = %14.7e",
        iter, rho, residual, tolerance);
    numerics_printf_verbose(1,
                            "---- ConvexQP - ADMM  -                            primal "
                            "residual = %14.7e, epsilon_primal = %14.7e",
                            r, epsilon_primal);
    numerics_printf_verbose(1,
                            "---- ConvexQP - ADMM  -                            dual residual "
    ...
    if (stopping_criterion) {
      ...
      if (error < dparam[SICONOS_DPARAM_TOL]) {
        ...
        tolerance =
            ...
            error;
        numerics_printf_verbose(
            1, "---- ConvexQP - ADMM  - We reduce the tolerance on the residual to %14.7e",
            tolerance);
        // getchar();
  ...
  convexQP_compute_error(problem, z, xi, w, u, tolerance, rho, options, norm_q, norm_b,
                         &error);
  numerics_printf_verbose(
      1, "---- ConvexQP - ADMM  - Iteration %i rho = %14.7e \t error = %14.7e", iter, rho,
      error);
  ...
  *info = hasNotConverged;
  ...
  if (iter == itermax) {
    ...
    convexQP_compute_error(problem, z, xi, w, u, tolerance, rho, options, norm_q, norm_b,
                           &error);
    numerics_printf_verbose(
        1, "---- ConvexQP - ADMM  - Iteration %i rho = %14.7e \t full error = %14.7e", iter,
        rho, error);
  }
  ...
  numerics_printf_verbose(1, "---- ConvexQP - ADMM - #Iteration %i Final Residual = %14.7e",
                          iter, error);
...
void convexQP_ADMM_set_default(SolverOptions* options) {
  options->iparam[SICONOS_CONVEXQP_ADMM_IPARAM_ACCELERATION] =
      SICONOS_CONVEXQP_ADMM_ACCELERATION_AND_RESTART; /* 0 Acceleration */
  options->iparam[SICONOS_CONVEXQP_ADMM_IPARAM_RHO_STRATEGY] =
      SICONOS_CONVEXQP_ADMM_RHO_STRATEGY_RESIDUAL_BALANCING;
